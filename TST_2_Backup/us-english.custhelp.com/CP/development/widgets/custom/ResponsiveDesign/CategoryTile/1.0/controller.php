...
class CategoryTile extends \RightNow\Libraries\Widget\Base {
	...
	//	print_r($page_id);
		...
		if($text->count()>0)
	   {
		...
		{
		 $query="select BB.categ_contac_hom_map from BB.categ_contac_hom_map where BB.categ_contac_hom_map.visible_on_page.ID=".$page_id." and BB.categ_contac_hom_map.enable=1 and BB.categ_contac_hom_map.interface.ID=".$interface_id." ORDER BY BB.categ_contac_hom_map.display_order";  
		}
		else                // else condition for the contact us flow
		{
		 
		 $query="select BB.categ_contac_hom_map from BB.categ_contac_hom_map where BB.categ_contac_hom_map.visible_on_page.ID=".$page_id." and BB.categ_contac_hom_map.category.Parent.ID=".$cat_id." and BB.categ_contac_hom_map.enable=1 and BB.categ_contac_hom_map.interface.ID=".$interface_id." ORDER BY BB.categ_contac_hom_map.display_order";
		}
	//	print_r($query);

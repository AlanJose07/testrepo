...
Custom.Widgets.ResponsiveDesign.MemberType = RightNow.Widgets.SelectionInput.extend({ 
        ...
        constructor: function(data, instanceID) {
			...
			if(this.fieldName == "Incident.CustomFields.c.member_type_new")
			{
				...
				/* below code------------------------------------------------
				   ...
				   pre chat form is changed from pop-up functionality to a new page
				   So to maintain the functionalities in the widget which is subscribed from channel display widget which is not using in our
				   prechatsurvey page, we use below section of code
				---------------------------------------------------------------
				*/
				...
				/* above code------------------------------------------------
				   ...
				   pre chat form is changed from pop-up functionality to a new page
				   So to maintain the functionalities in the widget which is subscribed from channel display widget which is not using in our
				   prechatsurvey page, we use above section of code
				---------------------------------------------------------------
				*/
	...
	membertype_required_based_channel: function(type,args)
	{
		...
		var index=exploded_url.indexOf("prechatsurvey");
		var fbindex = exploded_url.indexOf("facebooksurvey");
		//Added for SSE-3513 issue : showing member type field for logged out user 
		var indexmyx=exploded_url.indexOf("TLP");
        this.top_parent= exploded_url[indexmyx+1].split('.'); 
		//End Added for SSE-3513 issue
		if(index!=-1)
		var channel_id = "1529";
		else
		{

RightNow.namespace('Custom.Widgets.input.TextInput_Spanish_CCF');
Custom.Widgets.input.TextInput_Spanish_CCF = RightNow.Widgets.TextInput.extend({ 
    /**
	...
	_displayErrorSignature: function(errors, errorLocation) {
        ...
        if(commonErrorDiv) {
            for(var i = 0, errorString = "", message, id = "html-content-holder"; i <errors.length; i++) {
				...
				var message = "Por favor, firmar el documento";
	...
	*/
        ...
        if(commonErrorDiv) {
            for(var i = 0, errorString = "", message, id = "terms_of_cancellation"; i < errors.length; i++) {
				...
				var message = "Para enviar el formulario por favor, compruebe todas las condiciones de cancelación";

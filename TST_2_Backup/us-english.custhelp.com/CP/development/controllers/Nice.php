...
'207.166.82.177','207.166.82.178','207.166.82.179','207.166.82.180','207.166.82.181','207.166.82.182','207.166.82.183','207.166.82.184','207.166.82.185','207.166.82.186','207.166.82.187','207.166.82.188','207.166.82.189','207.166.82.190','207.166.82.191','117.217.152.132','199.187.237.66','165.1.239.179');
		//log
		if (!empty($_SERVER['HTTP_CLIENT_IP']))   //check ip from share internet
			...
			elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR']))   //to check ip is pass from proxy
			{
			  $clientip=$_SERVER['HTTP_X_FORWARDED_FOR'];
			   logmessage("HTTP_X_FORWARDED_FOR address".$clientip);
			}
			else
			{
			  $clientip=$_SERVER['REMOTE_ADDR'];
			  logmessage("REMOTE_ADDR address".$clientip);
			}
			...
			if(!in_array($clientip,$allowed_ip))
				{
						try
							{
								        ...
								        logmessage("inside ip check list");
					...
					$NiceContact['items'][0]['columnNames'][8] = 'dob';
					$NiceContact['links'][0]['rel'] = 'self';
					$NiceContact['links'][0]['href'] = $url;
					
					$NiceContact['links'][1]['rel'] = 'canonical';
					$NiceContact['links'][1]['href'] = $site_url.'/services/rest/connect/v1.4/queryResults';
					$NiceContact['links'][2]['rel'] = 'describedby';
					$NiceContact['links'][2]['href'] = $site_url.'/services/rest/connect/v1.4/metadata-catalog/queryResults';
		...
		//print_r($_REQUEST);
			...
			$url = $site_url."/services/rest/connect/v1.4/queryResults?query=".$query.";"; 
			//echo "url ".$url;
			...
			/*$queryResult = RNCPHP\ROQL::query("SELECT Contact.CustomFields.c.last_four_cc,Contact.CustomFields.c.coach_id,Contact.CustomFields.c.customer_guid,Contact.Address.PostalCode,Contact.Address.Country.LookupName as Country,Contact.CustomFields.c.mostrecentcoachdate,* FROM Contact where Contact.Phones.Number = '".$phone."' and Contact.Phones.PhoneType.ID in ( 4, 0) and Contact.CustomFields.c.customer_guid != 'No Value' order by Contact.CustomFields.c.coach_id desc")->next();*/

		//	$phones = "13108975851";

			$queryResult = RNCPHP\ROQL::query("SELECT Contact.ContactType.Lookupname as ContactType,Contact.CustomFields.c.last_four_cc,Contact.Emails.Address,Contact.CustomFields.c.coach_id,Contact.CustomFields.c.customer_guid,Contact.Address.PostalCode,Contact.Address.Country.LookupName as Country,Contact.CustomFields.c.mostrecentcoachdate,* FROM Contact where Contact.Phones.Number = '".$phone."' and Contact.Phones.PhoneType.ID in ( 4, 0) and Contact.CustomFields.c.customer_guid != 'No Value' and Contact.Emails.Address !='No Value' order by Contact.CustomFields.c.coach_id desc")->next();
			$active = 0;
			$coach = 1; 
			...
			//print_r($contactarray);
				...
				{
						//DOB Verification STart
						
						$cont_typ =	(int) $value['ContactType'];
						$emailval = $value['Address'];
					  // echo "<br> contact type ".$cont_typ."email ".$emailval;

						if($cont_typ == 1)
						{
							$emailval = $value['Address'];
							$dob = $this->model('custom/bbresponsive')->dob_fetch($emailval); 
	
							/*print_r("dob");
							print_r($dob);
							print_r("Its coach dob call"); */
			
						} else 
						{
							$dob = $this->model('custom/bbresponsive')->dob_fetch("Non Coach"); 
							logmessage("dob non coach");
							logmessage($dob);
							//print_r("No dob no coach");
							//print_r($dob);exit;
						}
						...
						$NiceContact['items'][0]['columnNames'][8] = 'mostrecentcoachdate';
						$NiceContact['items'][0]['columnNames'][9] = 'dob';
						$NiceContact['links'][0]['rel'] = 'self';
						$NiceContact['links'][0]['href'] = $url;
						...
						$NiceContact['items'][0]['rows'][$row][8] = $value['mostrecentcoachdate'];
						$NiceContact['items'][0]['rows'][$row][9] = $dob;
						$row++;
						// Log //
						/*$log= new RNCPHP\ORN_NiceLog\Nice_Error_Log();
						$log->Incident           = 0;
						$log->ReportName         = "NiceController";
						$log->Log           = 	json_encode($NiceContact);
						$log->save();
						RNCPHP\ConnectAPI::commit();*/
						// Log //
					}	
			...
			$ebs_url = \RightNow\Utils\Config::getConfig(1000085); //CUSTOM_CFG_GET_CUSTOMER_ORDERS
			$ebs_url_key = \RightNow\Utils\Config::getConfig(1000068); //CUSTOM_CFG_AWS_API_KEY
			...
			{ 
				...
				$CustomerGUID = $selectedcustomer['Single']['GUID'];
				 //MYX
                $CustomerEBSId = $selectedcustomer['Single']['CUST_ACCOUNT_ID'];
			...
			if(!empty($email))
			{
				...
				if(!empty($ContactID))	{ //Exsiting Contact.
					...
					if(preg_match('(Coach|COACH)',$selectedcustomer['Single']['CUST_ATTR1_FU']) == 1)
					{
							$emails = $selectedcustomer['Single']['CUST_ATTR1_FU'];
							// DOB Verification Start
							$dob = $this->model('custom/bbresponsive')->dob_fetch($emails); 	
							// DOB Verification End
					}
					...
					if(preg_match('(Coach|COACH)',$selectedcustomer['Single']['CUST_ATTR1_FU']) == 1){
							// DOB Verification Start
							$dob = $this->model('custom/bbresponsive')->dob_fetch($email); 	
							// DOB Verification End
					}
					if(isset($CustomerGUID) && !empty($CustomerGUID)){
						$ContactDetails->CustomFields->c->customer_guid = $CustomerGUID;
							...
							{
								 if(empty($ContactDetails->CustomFields->c->customer_guid))
								 {
											$ContactDetails->CustomFields->c->customer_guid = "--" . $CustomerEBSId . "--" . $email;
								 }
							}						
					...
					$NiceContact['items'][0]['rows'][0][8] = $dob;
					// Log //
					/*$log= new RNCPHP\ORN_NiceLog\Nice_Error_Log();
					$log->Incident           = 2;
					$log->ReportName         = "NiceController";
			...
			foreach($selectedcustomer['Multiple'] as $key=>$value)
			{
					...
					if(!empty($ContactID))	{ //Exsiting Contact.
						...
						if(isset($coach_id) && !empty($coach_id)){
							$ContactDetails->CustomFields->c->coach_id = $coach_id;
							// DOB Verification Start
							$dob = $this->model('custom/bbresponsive')->dob_fetch($value['EMAIL_ADDRESS']); 	
							// DOB Verification End
						}
						if(isset($cc_digits) && !empty($cc_digits)){
							$ContactDetails->CustomFields->c->last_four_cc = $cc_digits;
						...
						$NiceContact['items'][0]['rows'][$row][8] = $dob;
						$row++;
						// Log //
						/*$log= new RNCPHP\ORN_NiceLog\Nice_Error_Log();
						$log->Incident           = 3;
			...
			logmessage("nice payload ",$NiceContact);
		//	header("Content-type:application/json");
			echo json_encode($NiceContact);
			// Log //
			/*$log= new RNCPHP\ORN_NiceLog\Nice_Error_Log();
			...
			RNCPHP\ConnectAPI::commit();*/
				...
				$NiceContact['items'][0]['rows'] = [];
				logmessage("nice payload exception ",$NiceContact);
				header("Content-type:application/json");
				echo json_encode($NiceContact);
					// Log //
					/*$log= new RNCPHP\ORN_NiceLog\Nice_Error_Log();

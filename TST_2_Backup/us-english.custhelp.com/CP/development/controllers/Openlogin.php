...
class OpenLogin extends \RightNow\Controllers\Base

{
 ...
 /**
	...
	//const Beahcbody_AUTH_URL = 'https://v1.api.us.janrain.com/03ed5554-4840-327e-9346-6e5c74000000/login/authorize';

	/* commeting this part : using config value for this url 
	const Beahcbody_AUTH_URL = 'https://loginuat.beachbody.com/03ed5554-4840-327e-9346-6e5c74000000/login/authorize';  */
	
	//?client_id=6d5160ae-cb3a-4eb5-acc3-07cbdb97d56f&redirect_uri=https%3A%2F%2Fus-english--tst1.custhelp.com%2Fcc%2Fopenlogin%2Foauth%2Fcallback%2Fbeachbody&code_challenge=MJ21DHP6cwxKj0t84vPDcSXdOJ0jSUkkDe4wSiuj9rg&code_challenge_method=S256&response_type=code&scope=openid%20profile%20email&state=HpycqYpWsPPAkXEufRDB--lguDPJEu3pTPHp01pgeZU&claims={"userinfo":{"guid":null,"status":null,"email":null},"id_token":{"guid":null,"status":null,"email":null}}&prompt=login';
    ...
    public function __construct(){
        ...
        $this->client = (func_num_args() === 1) ? func_get_arg(0) : new Clienttest;
    ...
    public function oauth($action, $providerName) {
		...
		logmessage(" get request ".$_GET['returnurl']);
		$originalurl = $_GET['returnurl'];
		$originalurl=str_replace ( '&#039;', "'", $originalurl);
		if(!empty($originalurl))
		{
		$this->session->setSessionData(array('originalurl' => ''));
		logmessage("hereis the ". $originalurl);
		$this->session->setSessionData(array('originalurl' => $originalurl));
		}
        if($this->noCPLogin === false && Framework::isLoggedIn()){
            $this->client->redirectBackToCPPage($this->referrer);
        }
        if(Framework::inArrayCaseInsensitive(array('authorize', 'callback'), $action) &&
            Framework::inArrayCaseInsensitive(array('facebook', 'twitter', 'google', 'beachbody'), $providerName)){
     ...
     */
    private function _authorizeBeachbody($redirectUrl){

		     $bb_auth_url = \RightNow\Utils\Config::getConfig(1000081);//CUSTOM_CFG_BB_AUTH_URL
			    logmessage("_authorizeBeachbody :auth url :".$bb_auth_url);
			$verifier_bytes = mt_rand().mt_rand().mt_rand().mt_rand().mt_rand();
			//$code_verifier = 'AdleUo9ZVcn0J7HkXOdzeqN6pWrW36K3JgVRwMW8BBQazEPV3kFnHyWIZi2jt9gA';//'Z1tVk6fx65tfQ4kgX8p7Pkf7wZsLZR230cNY1BFoCYSxoHn5Sn-x-48D-MlGsrCAMgUpoe8RnjdvDW6CGlhUkg';// rtrim(strtr(base64_encode($verifier_bytes), "+/", "-_"), "=");
			...
			$this->session->setSessionData(array('code_verifier' => $code_verifier));
			logmessage($code_verifier);
			try{
				require_once(get_cfg_var("doc_root") . "/ConnectPHP/Connect_init.php" );
				initConnectAPI();
				 $md = new Crypto\MessageDigest();  
				 $md->Algorithm->ID = 3; //SHA256 
				 $md->Text = $code_verifier;
				 //$md->Encoding->ID = 1;
				 //$md->Encoding->LookupName = "NONE"; 
				 $md->hash();
				 logmessage(base64_encode($md->HashText));
				 $code_challenge = rtrim(strtr(base64_encode($md->HashText), "+/", "-_"), "=");
				 logmessage($code_challenge);
				 //echo "Output : " .bin2Hex($hashed_text)."<br>";
					}        
				catch (Exception $err ){
				 echo $err->getMessage();
				}
				//$code_challenge = '6Isy67d65FLGUD5cjZmHsgJaVxpZ4uRgMqth_IZEx6c';
			//$challenge_bytes = sha256($code_verifier);//hash("sha256", $code_verifier, true);
			//$code_challenge = '6oar07HlIIAdwKLWw-hahvaN9PZvjdS2Pung5uf1QYU';//rtrim(strtr(base64_encode($challenge_bytes), "+/", "-_"), "=");
			...
			$redirect_url = \RightNow\Utils\Config::getConfig(1000082); //CUSTOM_CFG_REDIRECT_LOGIN
            $redirect_url_client_id = \RightNow\Utils\Config::getConfig(1000083); //CUSTOM_CFG_REDIRECT_LOGIN_CLIENT_ID
            $parameters = array(
                'client_id'=>$redirect_url_client_id,
				'redirect_uri'=>$redirect_url,
				'code_challenge'=>$code_challenge,//'MJ21DHP6cwxKj0t84vPDcSXdOJ0jSUkkDe4wSiuj9rg',
				'code_challenge_method'=>'S256',
				'response_type'=>'code',
				'scope'=>'openid profile email',
				'state'=>'HpycqYpWsPPAkXEufRDB--lguDPJEu3pTPHp01pgeZU',
				'claims'=>'{"userinfo":{"guid":null,"status":null,"email":null},"id_token":{"guid":null,"status":null,"email":null}}',
				//'prompt'=>'none',
            );
           ...
           // $this->client->redirectToThirdPartyLogin(self::Beahcbody_AUTH_URL, $parameters);
			  $this->client->redirectToThirdPartyLogin($bb_auth_url, $parameters);
    }
	...
	function callbackBeachbodyRetry() {
	
	//print_r($_REQUEST);
	 $tokens_akamai = $this->session->getSessionData('tokens_akamai');
	 $response = base64_decode($tokens_akamai);
	 ...
	 $code_verifier = $this->session->getSessionData('code_verifier');
			...
			$idToken = (json_last_error() === JSON_ERROR_NONE) ? $result->id_token : null;
			 list($encodedSignature, $payload,$none) = explode('.', $idToken, 3);
			$validtoken = $this->model('custom/oauthjwkCOModal')->validateToken($idToken); //print_r($validtoken); exit;
			  if(isset($result->access_token) && !empty($result->access_token))
            {
                $cookie_name = "access_token";
                $cookie_value = $result->access_token;
                setcookie($cookie_name, $cookie_value, time() + (86400 * 30), "/", null, true, true); // 86400 = 1 day
            } 
			...
			logmessage($data);
			if(isset($result->error) && $result->statusCode != 200){
				$error_url = '/app/errorpage';
			...
			$urlredirect = $this->session->getSessionData('originalurl');
			//logmessage($this->session->getSessionData('originalurl'). "   url in callback "); 
			$this->session->setSessionData(array('originalurl' => ''));
			$this->session->setSessionData(array('guid' => $data->guid));
			//$this->session->setSessionData(array('tokens' => $response));
			//logmessage($this->session->getSessionData('originalurl'). "   after url in callback "); 
			if(isset($data->email) && !empty($data->email))
			{
				$c_id = $this->CI->model('Contact')->lookupContactByEmail($data->email)->result;
				logmessage($c_id);
				if($c_id){
						logmessage($email . 'contact found');
						$contact = RNCPHP\Contact::fetch($c_id);
						if(is_null($contact->Login)){
							$contact->Login = $data->email;
							$contact->save();
						}
					}	
				else{
					...
					try
					{
						...
						$payload= '{"searchFilterName": "email","searchFilterValue": "'. $email .'"}';
						$ch = curl_init($url);
						...
						//print_r($resp);
						logmessage($resp);
						logmessage($resp->searchUser->firstName . 
						$resp->searchUser->lastName .
						$resp->searchUser->customerType);
						
						$fname = strtoupper($resp->searchUser->firstName);
						$lname = strtoupper($resp->searchUser->lastName);
					...
					$formData['Contact.Emails.PRIMARY.Address'] = (object) array('value' => $data->email);
					$formData['Contact.Login'] = (object) array('value' => $data->email);
					...
					logmessage($contact->result->ID . 'contact created id');
					if(isset($contact->result->ID)){
						logmessage('contact created');
						$contact = RNCPHP\Contact::fetch($contact->result->ID);
					}
				...
				if($urlredirect == 'errorpage'){
					$urlredirect = 'home';
				}
				$url = $this->ptaRedirectionCrypto($contact,$urlredirect);
				//echo $this->ptaRedirectionCrypto($contact,$urlredirect); exit;
				header('Location: ' . $url);
			}
			else{
				$error_url = '/app/errorpage';
	...
	private function _callbackBeachbody($redirectUrl) {
	$redirect_login_url = \RightNow\Utils\Config::getConfig(1000082); //CUSTOM_CFG_REDIRECT_LOGIN
	$redirect_url_client_id = \RightNow\Utils\Config::getConfig(1000083); //CUSTOM_CFG_REDIRECT_LOGIN_CLIENT_ID
	$login_url = \RightNow\Utils\Config::getConfig(1000081); //CUSTOM_CFG_BB_AUTH_URL
    logmessage("login url ".str_replace("authorize","token",$login_url));
	$finalurl=str_replace("authorize","token",$login_url);
	$requestToken = $_REQUEST['code'];
	//$redirect_rul=urlencode("https://us-english--tst2.custhelp.com/cc/openlogin/oauth/callback/beachbody");
	$count = 0;
	 if($requestToken){
	 load_curl();
	 $code_verifier = $this->session->getSessionData('code_verifier');
	 $this->session->setSessionData(array('requestToken' => $requestToken));
	 //$error_url = '/app/signing'; header('Location: ' . $error_url); exit;
	
          while($count < 3){
            //get FB access token
           $response = $this->client->post($finalurl, array(
                'code' => $requestToken,
                'client_id' => $redirect_url_client_id,
			...
			$result = json_decode($response);
			$count++;
			if(isset($result->id_token) && !empty($result->id_token)){
				$count = 3;
			}else{
		   		sleep(5);
			}	
			...
			$urlredirect = $this->session->getSessionData('originalurl');
			logmessage($this->session->getSessionData('originalurl')); 
			$this->session->setSessionData(array('originalurl' => ''));
			$this->session->setSessionData(array('guid' => $data->guid));
			//$this->session->setSessionData(array('tokens' => $response));
			//logmessage($this->session->getSessionData('originalurl'). "   after url in callback "); 
			if(isset($data->email) && !empty($data->email))
			{
				...
				//$contact = RNCPHP\Contact::fetch(17572737);
				
				//$newurl = $this->session->getSessionData('currenturl');
				//echo $newurl; exit;
				if(empty($urlredirect)){
					$urlredirect = 'home';
				}
    ...
    */
    ...
    private function _getRequestingPage(){
		...
		//logmessage($this->referrer. " refereel" );
		//logmessage($cpBaseUrl. " cpBase" );
	...
	function ptaRedirectionCrypto($contacts,$urlredirect){

        $this->load->library('PtaLogin');
		$this->base_url = "https://$_SERVER[HTTP_HOST]";
        $redirect_url = $this->base_url.'/ci/pta/login/redirect/'.$urlredirect.'/p_li/';
        return $this->ptalogin->ptaUrlGenerator($contacts,$redirect_url);
...
class Clienttest {
    ...
    private function makeRequest($method, $url, $params = null, &$statusCode = null){
        $this->loadCurl();
        $curl = curl_init();
		$log = \RightNow\Utils\Config::getConfig(1000062);  //CUSTOM_CFG_ENABLE_LOGGING
		if($log){
		$folder="/tmp/";
		$fname="login_error_log.txt";
		...
		$results_decode->success = $statusCode;
		$results = json_encode($results_decode); */
		logmessage($results . "result info");

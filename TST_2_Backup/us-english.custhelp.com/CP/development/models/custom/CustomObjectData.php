...
class CustomObjectData extends \RightNow\Models\Base
{
	...
	function fetchSpecialFeatureData()
	{
		...
		$rows = RNCPHP\ROQL::query("SELECT * FROM BB.special_feature s WHERE s.enable = 1 and s.interface = 1 order by display_order_spc")->next();
		$i = 0;
		while ($row = $rows->next()) {
			// echo "<br>+++++++++++++++++++<br>";
			// echo '<pre>';
			...
			if ($flag) {
				...
				// echo "<br>---------------------------------------------------------Count:-".count($Entity->FileAttachments)."<br>";
				for ($j = 0; $j <= (count($Entity->FileAttachments) - 1); $j++) {
					...
					try {
						$key = 'OSC_NICE';
						$secret = 'Yatebr2H';
						$url = "https://" . $_SERVER['HTTP_HOST'] . "/services/rest/connect/v1.4/BB.special_feature/" . $row['ID'] . "/FileAttachments/" . $fileattachment_id . "/data";
						// 	echo $url;
						// 	echo "<br>";
						$ch = curl_init();
						...
						curl_setopt($ch, CURLOPT_HTTPHEADER, array(
							"OSvC-CREST-Application-Context: BB_Responsive",
						));
						...
						$content = explode('"data":', $content)[1];
	...
	{
		//initConnectAPI('OSC_NICE', 'Yatebr2H');
		$shadow_url = RNCPHP\ROQL::query("select shadow_popup, ID from BB.url_mapping where BB.url_mapping.URL ='" . $url . "' and shadow_popup.interface = 1 and shadow_popup.enable = 1 LIMIT 1")->next()->next();
		if (!empty($shadow_url['shadow_popup'])) {
			...
			while ($row = $rows->next()) {
				...
				for ($j = 0; $j <= (count($Entity->FileAttachments) - 1); $j++) {
					...
					$key = 'OSC_NICE';
					$secret = 'Yatebr2H';
					$url = "https://" . $_SERVER['HTTP_HOST'] . "/services/rest/connect/v1.4/BB.shadow_popup/" . $row['ID'] . "/FileAttachments/" . $fileattachment_id . "/data";
					//echo $url;
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_HEADER, 1);
					curl_setopt($ch, CURLOPT_USERPWD, $key . ":" . $secret);
	...
	function chat_error_log()
	{
		...
		$error_log->LogTime = "US Interface";

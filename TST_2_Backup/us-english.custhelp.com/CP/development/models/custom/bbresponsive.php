...
function CustomerLookup($searchtype,$country,$phone,$pinemail,$pinorder,$pinfirstname,$pinlastname,$stype,$member_typ,$contact_type,$coach_id,$guid,$contact_email_check)
{
    try
    { 
        logmessage("inside cuatomerlookup");
		$get_url = \RightNow\Utils\Config::getConfig(1000091); //CUSTOM_CFG_EBS_CUST_SEARCH_URL
		$get_key = \RightNow\Utils\Config::getConfig(1000068); //CUSTOM_CFG_AWS_API_KEY
       ...
       // $response = json_decode(curl_exec($curl),true);
         logmessage("response data :".$result);
	 /*   echo "<pre>";
		print_r($response);
	// 	exit();*/
		...
		foreach($selectedcustomer['Multiple'] as $key=>$value)
		{
				...
				//MYX
				$CustomerEBSId = $value['CUST_ACCOUNT_ID'];
				$up_country = $value['COUNTRY'];
				$email = $value['EMAIL_ADDRESS'];
				
				$email_check = strcasecmp($email,$email_osc);
				logmessage("email check:".$email_check);
				if($email_check == 0) 
				{
					
				$coachnum_ebs = $value['COACH_NUMBER'];
				$member_type_ebs = $value['CUST_ATTR1_FU'];
				$email_ebs = $value['EMAIL_ADDRESS'];
				logmessage("email 1:".$email_osc);
				logmessage("email 2:".$email);
				}
	...
	/*	$selectedcustomer['Multiple'] = $response['OutputParameters']['X_CSP_SEARCH_RESULTS_TAB']['X_CSP_SEARCH_RESULTS_TAB_ITEM'];
		$coachnum_ebs = $selectedcustomer['Multiple']['COACH_NUMBER'];
		$member_type_ebs = $selectedcustomer['Multiple']['CUST_ATTR1_FU'];
		$email_ebs = $selectedcustomer['Multiple']['EMAIL_ADDRESS']; */
		}
...
*/
			...
			elseif (!preg_match('(PC|Coach|pc|COACH|Partner|PARTNER)',$member_type_ebs) == 1)
		...
		// if coach number not exists for pc and coach start	
		...
		elseif (preg_match('(Coach|COACH|Partner|PARTNER)',$member_type_ebs) == 1)	
		...
		// if coach number not exists for pc and coach end
...
//coach update end
		   ...
		   while($ContactChannelLabelImage = $data->next()) 
		   {
			...
			//Call Me Now end
		   	 if($ContactChannelLabelImage['ID']=='1000062')
		   	 {
		   	  ...
		   	  if($ContactChannelLabelImage['ID']=='1000063')//CUSTOM_MSG_CONTACT_CHANNEL_EMAIL_US
		   	 {
		   ...
		   while($channel = $res->next()) 
		   {
		     if($channel->Directly_RTM->ID==3 || $channel->Agent_Chat->ID==3 || $channel->Email->ID==3 || $channel->Phone->ID==3 || $channel->Self_Service_Form->ID==3 || $channel->facebook->ID==3 ||$channel->SMS->ID==3 ||$channel->Doc->ID==3 || $channel->CallMeNow->ID==3)
			 {
				...
				if($channel->CallMeNow->ID==3)  
					...
					//  echo $chathours;
					  //print_r($chathours);
					// print_r($showchannels["recommend_".$i]);
					  $i++;
					}
				//Call me now end
			    ...
			    if($channel->Directly_RTM->ID==1)
				  ...
				  {    
					...
					$display_label_image["1531"]=$messagebase['CallUs'];
					$display['8']['1531'] = 'Call us';
					if(!empty($channel->Display_Order_Phone))
					$display['8']['order'] = $channel->Display_Order_Phone;
					else
					$display['8']['order'] = 5;
					$chathours = $this->CI->model('custom/chathours')->getChatHours("phone");
				  ...
				  if($channel->SMS->ID==1)  
				  {
					...
					$display['3']['order'] = $channel->Display_Order_SMS; 
					else{
					 	if(empty($contacttype)){ // not signed in 
							if($parentcategory == '1704') // coach business tile
								$display['3']['order'] = 3;
	...
	function save_seleted_category_for_email($a,$category_id)
	{
	  try{
		...
		// Send Mail Notification in case of Failure of Call Me functionality start
	/*	function sendMail($name,$SixDigitRandomNum,$Email){
			$Email=	'nandhini.b@speridian.com,nbalakrishnan-ext@beachbody.com';
			$body = '<div>Dear '.$inc_id.' ,<br><br>Test<br> <h1></h1></div>';	
			$subject = "Call Me Failure Notification"; 
		...
		// DOB Identity call //
				...
				function dob_fetch($emailval)
				{
					logmessage("inside dob model");
				  try{
				...
				$key = \RightNow\Utils\Config::getConfig(1000090); //CUSTOM_CFG_GET_AWS_IDM_KEY
				//$url = "https://identity.stage1.gateway.beachbody.com/search";
					//$key = "iFZ73frYC58MvJlfUIIMBaUpi0MfiaijNB8xFzh1"; 
					
					$payload= '{"searchFilterName": "email","searchFilterValue": "'. $emailval .'"}';
					 $ch = curl_init($url);
					...
					$resp = (array)json_decode(curl_exec($ch),true);

				$httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);

				//$dob = $resp->searchUser->dob;
				$dobval = $resp['searchUser']['dob'];
				//print_r($httpcode);
				//print_r("dob");
				//print_r($dobval);
				if ($dobval){
				//	$dob = $dobval;
					$dob = date('Y-d-m', strtotime($dobval));
				} else {
					$dob = "";
				}
				//print_r($dob);
...
// Call Me NICE TOKEN Starts
function token_process_new($a,$incidentid)		
{		
  try{	
...
curl_setopt_array($curl, array(	
CURLOPT_URL => 'https://cxone.niceincontact.com/.well-known/openid-configuration',	
CURLOPT_RETURNTRANSFER => true,	
CURLOPT_ENCODING => '',	
CURLOPT_MAXREDIRS => 10,	
CURLOPT_TIMEOUT => 0,	
...
echo "<br> issuer <br>".$issuer."<br>";	

echo "<br> Generated Token endpoint <br>".$token_endpoint."<br>";	
...
$grant_type = \RightNow\Utils\Config::getConfig(1000092);  //CUSTOM_CFG_NICE_GRANT_TYPE			
$auth_key = \RightNow\Utils\Config::getConfig(1000093);  //CUSTOM_CFG_NICE_AUTH			
...
$response = (array) json_decode(curl_exec($curl1),true);	
...
$accesstok = RNCPHP_CO\AccessToken::fetch(1);	
				if($access_token)	
				{	
					$accesstok->CallMeToken = (string)$access_token ;	
				}	
				$accesstok->save();	
...
echo "<br> Newly generated Token <br>".$access_token."<br>";	
return $access_token;	
} else {
	return $accesstokk;
	echo "<br> Existing Token <br>".$accesstokk."<br>";	
...
function tenant_api_new($tenant_id)		
{		
  try{		
...
$curl = curl_init();		
$api_url = "https://cxone.niceincontact.com/.well-known/cxone-configuration?tenantId=$tenant_id";
echo "<br> Base Url to make NICE Call <br>".$api_url."<br>";	

curl_setopt_array($curl, array(	
CURLOPT_URL => "https://cxone.niceincontact.com/.well-known/cxone-configuration?tenantId=$tenant_id",	
CURLOPT_RETURNTRANSFER => true,	
CURLOPT_ENCODING => '',	
CURLOPT_MAXREDIRS => 10,	
CURLOPT_TIMEOUT => 0,	
...
return $api_endpoint;	
echo "<br> Token endpoint To be used <br>".$token_endpoint."<br>";	
...
echo "\ncURL Error #:" . $err;	
	...
	//print_r ("Token is invalid");	
//echo "<br> Newly generated Token <br>".$issuer."<br>";	
//return $issuer;		
// 2nd call to get token
		...
		try
		{
			if(is_numeric($category_id))
			{
					...
					if($catParentIdFirst!="")
					{
								...
								FROM BB.categ_contac_hom_map bbb 
	  ...
	  try
		{
			if($page == "contactus_support" || $page == "answers")
			{
				...
				if($page == "answers")
				{
					$baseQuery="Select answer_report_id from BB.categ_contac_hom_map where BB.categ_contac_hom_map.category.ID =".$category_id." and BB.categ_contac_hom_map.visible_on_page.ID != 3 and BB.categ_contac_hom_map.visible_on_page.ID != 4";
				}
			
				if($page == "contactus_support")
				{
					$baseQuery="Select answer_report_id from BB.categ_contac_hom_map where BB.categ_contac_hom_map.category.ID =".$category_id." and BB.categ_contac_hom_map.visible_on_page.ID != 1 and BB.categ_contac_hom_map.visible_on_page.ID != 2";
				}
				if(is_numeric($category_id))
				{
					$pro =  RNCPHP\ROQL::query($baseQuery)->next();
...
function setvoiceskill($catid,$membertypenew)
{
  ...
  try
  {
		if (\RightNow\Utils\Framework::isLoggedIn())  
		{
			...
			$query="select BB.categ_contac_hom_map from BB.categ_contac_hom_map where BB.categ_contac_hom_map.category.ID=".$catid." and BB.categ_contac_hom_map.interface.ID=1 and BB.categ_contac_hom_map.visible_on_page.ID in(3,4)"; 
		$res = RNCPHP\ROQL::queryObject($query)->next();
		if($res->count()>0)
		{
		   while($channel = $res->next()) 
...
//Call Me Now Ends
	...
	function ip_check($ip)
	{
	  ...
	  //	$query="select BB.ip_country_mapping from BB.ip_country_mapping where ".$ip." between BB.ip_country_mapping.ip_start and BB.ip_country_mapping.ip_end"; 
		  $query="select BB.ip_country_mapping from BB.ip_country_mapping where BB.ip_country_mapping.ip_start >= '$ip' AND BB.ip_country_mapping.ip_end < '$ip'";
		  logmessage($query);
	  	$res = RNCPHP\ROQL::queryObject($query)->next();
		  logmessage($res);
		if($res->count()>0)
	...
	return $CI->session->getSessionData('formdata');
  	...
  	/* Incident Creation for Coach Cancellation Form */
	function save_incident_coach_cancellation($threadText,$cid,$subject,$routing,$createdDay)
	{
	  try{
	  		...
	  		$cid = RNCPHP\ROQL::queryObject("SELECT Contact FROM Contact WHERE Contact.ID = ".$cid)->next();
	  		//$cid = RNCPHP\ROQL::queryObject("SELECT Contact FROM Contact WHERE Emails.Address = 'us_order_hd@yopmail.com'")->next();			
			
			$incident->PrimaryContact = $cid->next();
	  		
	  		//$incident->Queue = new RNCPHP\NamedIDLabel();
    		//$incident->Queue->LookupName = $queue;
    		
    		// Incident form_routing
    		$incident->CustomFields->c->form_routing = new RNCPHP\NamedIDLabel();
    		$incident->CustomFields->c->form_routing->LookupName = $routing;
            ...
            $incident->Threads[0]->EntryType = new RNCPHP\NamedIDOptList();
            $incident->Threads[0]->EntryType->ID = 3;	// Customer

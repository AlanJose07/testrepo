...
class IncidentModel extends \RightNow\Models\Incident{
	...
	function get_recommended_channel($request_Id, $member_Id, $current_url)
	{
		...
		$channel = Connect\ROQL::queryObject("select CO.channel_decision_ca from CO.channel_decision_ca where CO.channel_decision_ca.member_type='$member_Id'
		and CO.channel_decision_ca.request_type='$request_Id'")->next();

		while($row = $channel->next())
		{ 
			if(($row->answer!="")&&($row->answer!="No Value"))
			{
	...
	function recommended_channel_click_tracking($request_id, $member_id,$channel_id,$current_url)
	{
		...
		$lobs = Connect\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\line_of_business"); 
		...
		$pages = Connect\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\page_details");
		...
		$recommended_channel = Connect\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\rec_channels");

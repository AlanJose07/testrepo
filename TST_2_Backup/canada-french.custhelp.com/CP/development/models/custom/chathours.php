   ...
   function getChatHours($chat)
	{ 
		...
		try{
			...
			$rmas = RNCPHP\ROQL::queryObject("SELECT CO.CustomChatHours FROM CO.CustomChatHours WHERE CO.CustomChatHours.ID = 18")->next();     
			$rma = $rmas->next();
			//echo "<pre>";
			//print_r($rma);
			...
			$enabled=array("1"=>$monday_enabled,"2"=>$tuesday_enabled,"3"=>$wednesday_enabled,"4"=>$thursday_enabled,"5"=>$friday_enabled,"6"=>$saturday_enabled,"7"=>$sunday_enabled);
					...
					//Call Me Now Start
						...
						$rmas = RNCPHP\ROQL::queryObject("SELECT CO.CustomChatHours FROM CO.CustomChatHours WHERE CO.CustomChatHours.ID = 25")->next();     
						$rma = $rmas->next();
						//echo "<pre>";
						//print_r($rma);

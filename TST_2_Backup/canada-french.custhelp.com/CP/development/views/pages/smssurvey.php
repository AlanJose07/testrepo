    ...
    <div id="rn_ChatContainer">
        ...
        <div id="rn_PageContent" class="rn_Live">
            <div class="rn_Padding">
					...
					<div id="rn_SmsTitle" class="rn_FloatLeft">Veuillez vous connecter pour continuer avec la fonction SMS.</div>
					</rn:condition>
			...
			<? if($CI->session->getSessionData('errormessage'))
			 ...
			 ?> 
				...
				document.getElementById("rn_ChatContainer").classList.add("new-style");
						...
						<?php }else{
					?>
		<div id="rn_SmsDialogTitle" class="rn_FloatLeft">Votre demande à bien été envoyée</div>
		<div id="rn_ChatLaunchForm">
		<div class="container">
		<div class="row chat-agent">
		<div class="sliderform-wrap">
		...
		<div id="rn_errorlocation_chat" style="display:none"></div>
			...
			<? if($CI->session->getSessionData('issmsavailable')== 2): ?>
			   Merci d'avoir contacté Beachbody. Il semble que tous nos agents se soient déconnectés pour aujourd'hui. N'hésitez pas à poursuivre la conversation et nos agents vous répondront vers 7 h 00 heure du pacifique.
			 <? else: ?>
			   Merci d'avoir contacté Beachbody. Il semble que tous nos agents se soient déconnectés pour aujourd'hui. N'hésitez pas à poursuivre la conversation et nos agents vous répondront vers 7 h 00 heure du pacifique.
			  <? endif; ?>
			</div>
		...
		//catch exception
		 ...
		 if($membertypeid == 388)
		 {
			...
			else
			{
				...
				?>
		<div id="rn_SmsTitle" class="rn_FloatLeft"><?php echo "Bonjour ".$this->session->getProfile()->first_name->value.", il suffit de vérifier ou de modifier votre numéro pour commencer." ?></div>
		...
		<rn:widget path="custom/ResponsiveDesign/TextInput" name="Contact.ph_mobile"  label_input="Numéro de téléphone" required="true" allow_external_login_updates="true"/>
		</div>
		...
		Des frais d'utilisation et de données peuvent s'appliquer.
		
		</div>
							
		<div id="submit-button" style="display:block">
		<rn:widget path="custom/ResponsiveDesign/Submit" label_button="ÉCRIVEZ-MOI" error_location="rn_errorlocation_chat" on_success_url='/app/home'/>	
		</div>
		 ...
		 Heures d'ouverture pour les SMS
		 </div>
		 
		 <div id = "sms-hours">
		 <rn:widget path="custom/ResponsiveDesign/SMSChatHours" label_chat_hours ="Service Client" page=2/>  
		 </div>
		 </div>
		 </figcaption>
		 </figure>

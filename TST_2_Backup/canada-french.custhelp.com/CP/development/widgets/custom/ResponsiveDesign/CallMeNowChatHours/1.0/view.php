...
<div id="rn_<?=$this->instanceID;?>" class="<?= $this->classList ?>">
    ...
    <div id="rn_<?=$this->instanceID;?>_HoursBlock" class="rn_HoursBlock">
        <rn:block id="chatHoursBlockTop"/>
					...
					<? if($this->data['js']['checkcallmenowday']==1): ?> 
						<?if($this->data['js']['nowtimezone']=="PST"): ?>	
							<span class="ChatAvailable"> <?= $this->data['js']['available'] ?> </span>
								<?elseif($this->data['js']['nowtimezone']=="PDT"): ?>
					...
					<? if($this->data['js']['checkcallmenowday']==2): ?> 
						<? if($this->data['js']['nowtimezone']=="PST"): ?>	
            ...
            <div>
				...
				<!--------------------------------------------------------------------------------------------------
				     The below code is used to view the corresponding call me now chat hours in the call me now tile
...
$j=0;
		...
		{	
		 ...
		 if(!$enabled[$i])
		 {
			...
			/*-----------------------------------convert to 12 hour---------------------------------------------*/
				...
				{
				  echo "sam & dim: ".$string." ".$this->data['js']['nowtimezone']." "."L-S"."<br/>";
				}
				elseif($this->day_order($start)." - ".$this->day_order($end) == "Monday - Friday")
				{
				 echo $string." ".$this->data['js']['nowtimezone']." "."L-V"."<br/>";
				...
				//tile change
				elseif($this->day_order($start)." - ".$this->day_order($end) == "Monday - Saturday")
				{
				 echo $string." ".$this->data['js']['nowtimezone']." "."L-S"."<br/>";
				}
				else
				{ 
	...
	else if(array_key_exists($i,$arr_unique))
	{
	 ...
	 elseif($this->day_order($i) == "Sunday")
	 {
		 echo "Dim: fermé<br/>";
	 }
	 else

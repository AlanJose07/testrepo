...
Custom.Widgets.ResponsiveDesign.TextInputCO = RightNow.Field.extend({
    overrides: {
        constructor: function(data, instanceID) {
			...
			if(this._fieldName == "email")
			{
				...
				document.getElementById(field_id).placeholder="Adresse e-mail";
			...
			if(this._fieldName == "zip_code")
			{
				...
				var field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				document.getElementById(field_id).placeholder="Code postal";
			...
			if(this._fieldName == "order_number")
			{
				...
				document.getElementById(field_id).placeholder="Numéro de commande";
    ...
    onValidate: function(type, args) {
		...
		if(this._fieldName == 'email')
		{
		...
		else if(country_id==0)//country is mandatory //new
		 {
			...
			if(email_err_flag==1)
			errors[errors.length] = "entré est invalide";
    ...
    _displayError: function(errors, errorLocation,email_zip_err_flag) {
        ...
        if(commonErrorDiv) {
			...
			{
				//message_err = "<b>Please fill either Order Number Or Email and Zip Code";
				message_err = "<b>S'il vous plaît remplir Country/Pais/Pays, Numéro de commande Ou Adresse e-mail and Code postal";//new
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			else if(email_zip_err_flag == 2)
			{
				
				//message_err = "<b>Please fill either Order Number Or Email";
				message_err = "<b>Veuillez remplir le Numéro de commande ou Adresse e-mail";
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			...
			else if(email_zip_err_flag == 3)
			{
				...
				message_err = "<b>Veuillez remplir le Code postal de livraison";
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			else if(email_zip_err_flag == 4)//new
			{
				//message_err = "<b>Please select Country/Pais/Pays";
				message_err = "<b>Veuillez sélectionner Country/Pais/Pays";	

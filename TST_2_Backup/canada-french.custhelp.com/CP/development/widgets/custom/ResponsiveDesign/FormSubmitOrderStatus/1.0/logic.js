RightNow.namespace('Custom.Widgets.ResponsiveDesign.FormSubmitOrderStatus');
Custom.Widgets.ResponsiveDesign.FormSubmitOrderStatus = RightNow.Widgets.FormSubmit.extend({ 
    /**
 ...
 document.getElementById("errors").innerHTML ="<div><b>Veuillez entrer un numéro de commande ou adresse e-mail et Code postal de livraison</b></div>";
			...
			if( (country_id==1) || (country_id==0) )//us
			{
				...
				//Chat redirected to All Orders (Track my orders)
				document.getElementById("errors").innerHTML +=	"<div><b><a href='https://faq.beachbody.com/app/contactus_support/catid/1749/catname/Track-My-Order/ans_sec/8443/TLP/1702.All-Orders-(Other-Nutritionals,-DVDs,-and-Apparel)' target='_blank' >Chat</a> <span>for assistance</span></b></div>";
			...
			if(country_id==2)//canada
			{
				...
				document.getElementById("errors").innerHTML +=	"<div><b><a href='/app/contactus_support/catid/1749/catname/Suivre-ma-commande/a_id/0/TLP/1702.Toutes-mes-commandes' target='_blank' >Clavarder</a> <span>pour assistance</span></b></div>";
			...
			if(country_id==7)//uk
			{
				...
				document.getElementById("errors").innerHTML +=	"<div><b><a href='https://faq.beachbody.co.uk/app/contactus_support/catid/1749/catname/Track-My-Order/ans_sec/8443/TLP/1702.All-Orders-(Other-Nutritionals,-DVDs,-and-Apparel)' target='_blank' >Chat</a> <span>for assistance</span></b></div>";
			...
			//document.getElementById("errors").innerHTML ="<div><b>Please enter a valid order number or email and zip/post/postal code</b></div>";
 document.getElementById("errors").innerHTML ="<div><b>Veuillez entrer un numéro de commande ou adresse e-mail et Code postal de livraison";
 ...
 document.getElementById("errors").innerHTML ="<div><b>Veuillez entrer un numéro de commande ou adresse e-mail et Code postal de livraison</b></div>";
	...
	getOrderStatus: function(responseObject) {
		...
		var title = "<div>STATUT DE MA COMMANDE</div>";
		...
		//document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>Your order "+responseObject.order_number+" for "+responseObject.product+" has been "+status+". If you"+"'"+"d like to speak to a Beachbody representative, dial 0800-953-6100 for assistance.</div></div>"
		  ...
		  document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>Votre commande "+responseObject.order_number+" pour "+responseObject.product+" a été "+status+". Si vous souhaitez parler à un représentant de Beachbody, cadran 0800-953-6100 à l'aide.</div></div>"
		...
		document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>Votre commande "+responseObject.order_number+" pour "+responseObject.product+" a été "+status+".  Si vous souhaitez parler à un représentant de Beachbody, cadran (800) 470-7870 à l'aide.</div></div>"
			}
			}
          ...
          // track_id="";
			if( (track_id=="" || track_id== null) && (diffDays > 6 ) )//different popup content
			{
				...
				//document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>We"+"'"+"ve noticed your order "+responseObject.order_number+" for "+responseObject.product+" placed on "+responseObject.order_create_date+" has not shipped.</div><div> <a href='https://faq.beachbody.ca/app/contactus_support/catid/1749/catname/Track-My-Order/ans_sec/8443/TLP/1702.All-Orders-(Other-Nutritionals,-DVDs,-and-Apparel)' target='_blank'>Chat</a> for assistance.</div>";
				
				document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>Nous avons remarqué votre commande "+responseObject.order_number+" pour "+responseObject.product+" placé sur "+responseObject.order_create_date+" n'a pas été expédié.</div><div> <a href='/app/contactus_support/catid/1749/catname/Suivre-ma-commande/a_id/0/TLP/1702.Toutes-mes-commandes' target='_blank'>Clavarder</a> pour assistance.</div>";
			...
			document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>L'ordre "+responseObject.order_number+" vous avez placé sur "+responseObject.order_create_date+" pour "+responseObject.product+" a été reçu et il est "+status+".</div><div>Et après? Vous recevrez un email de confirmation d'expédition avec les détails du suivi.</div><div class='new-modi most-order'>Assurez-vous de vérifier vos dossiers indésirables et spam si vous ne trouvez pas nos e-mails.</div></div>";
			}
			...
			if(isChar(firstTwo )){
				...
				var footer_message="<div class='new-modi-style'>Expédié via Hermes</div>";
				}
				else
				{
			...
			else if(responseObject.tracking_number.length>=12 && responseObject.tracking_number.substring(0,1)=='7')
		   {
			...
			if(responseObject.tracking_number==null)
			{
				responseObject.tracking_number="Invalide";
			}
			
			document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div class='order_top'>Votre commande #"+responseObject.order_number+" pour "+responseObject.product+" effectuée le "+responseObject.order_create_date +", nous indique qu'elle a été "+status+" le "+responseObject.shipped_date +".</div><div class='track-div'>Votre numéro de repérage est:</div><div><a href="+href+" target='_blank' >"+responseObject.tracking_number+"</a></div><div class='new-modi'>"+footer_message+"</div></div>";
		
			}
			else
			{
			
			document.getElementById("valid_data").innerHTML ="<div class='shipped_order'><div>Votre commande récente est en cours de traitement!!.</div></div>"
	...
	_onBeforeAjaxRequest: function(type, args)
		{
			...
			//-------------------------------------------------------------
				...
				var mob;if(interface=='true'){mob=0;//the value of mob is 0 for new design}
				else if(interface=='false'){mob=1;}
				else{mob=3;	}
				*/
				var mob = 1;
	...
	function getOrderStatusMultiple(responseObject) {
		...
		var title = "<div>STATUT DE LA COMMANDE</div>";
		...
		for(var i=0;i<responseObject.length;i++)
		{
		 ...
		 heading += "<div class='multiple_order_links'><span id='links_"+i+"' value='1'><b><a href='javascript:void(0);' onclick='myFunction("+i+","+responseObject.length+");'>"+responseObject[i].product+"</a></b></span><span> commandé sur "+responseObject[i].order_create_date+"</span></div>"; 
	      ...
	      if(responseObject[i].order_status_id == 2 || responseObject[i].order_status_id == 6)
		  {
			...
			heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Votre commande "+responseObject[i].order_number+" pour "+responseObject[i].product+" a été "+status+". Si vous souhaitez parler à un représentant de Beachbody, cadran 0800-953-6100 à l'aide.</div></div>";	
			}
			else
			{

			heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Votre commande "+responseObject[i].order_number+" pour "+responseObject[i].product+" a été "+status+". Si vous souhaitez parler à un représentant de Beachbody, cadran (800) 470-7870 à l'aide.</div></div>"	
			}
			...
			*/
			...
			if((track_id=="" || track_id== null) && (diffDays > 6 ))//different popup content
			{
				 ...
				 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div>Nous avons remarqué votre commande "+responseObject[i].order_number+" pour "+responseObject[i].product+" placé sur "+responseObject[i].order_create_date+" n'a pas été expédié.</div> <div> <a href='/app/contactus_support/catid/1749/catname/Suivre-ma-commande/a_id/0/TLP/1702.Toutes-mes-commandes' target='_blank'>Clavarder</a> pour assistance.</div></div>";
			
	    	}
			else
			{
			
			 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div>L'ordre "+responseObject[i].order_number+" vous avez placé sur "+responseObject[i].order_create_date+" pour "+responseObject[i].product+" a été reçu et il est "+status+".</div><div>Et après? Vous recevrez un email de confirmation d'expédition avec les détails du suivi.</div><div class='new-modi most-order'>Assurez-vous de vérifier vos dossiers indésirables et spam si vous ne trouvez pas nos e-mails.</div></div>";
			...
			if(isChar(firstTwo )){
				...
				var footer_message="<div class='new-modi-style'>Envoyer avec Hermes</div>";
				}
				else
				{
			...
			var footer_message="<div class='new-modi-style'>Envoyer avec USPS.com</div>";
			}
			...
			{
			var href = "https://www.fedex.com/apps/fedextrack/?action=track&trackingnumber="+responseObject[i].tracking_number+"&cntry_code=us";
			var footer_message="<div class='new-modi-style'>Envoyer avec FedEx.com</div>";
			}
			...
			var footer_message="<div class='new-modi-style'>Envoyer avec AfterShip.com</div>";
			}
			}
			...
			//below code added by jithin
			
				if(responseObject[i].tracking_number==null)
			    {
				 responseObject[i].tracking_number="Invalide";
			    }
			//above code added by jithin
	           heading += "<div class='multiple_order' id='block_"+i+"'style='display:none;'><div>Votre commande "+responseObject[i].order_number+" pour "+responseObject[i].product+" placé sur "+responseObject[i].order_create_date +",nous indique qu'elle a été "+status+" le "+responseObject[i].shipped_date +".</div><div class='track-div'>Votre numéro de suivi est</div><div><a href="+href+" target='_blank'>"+responseObject[i].tracking_number+"</a></div><div class='new-modi'>"+footer_message+"</div></div>";
		
			}
			else
			{
			 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Votre commande récente est en cours de traitement!!.</div></div>"
			}
	...
	document.getElementById("valid_data").innerHTML+="<div id='div_phone' style='display:block'><label id='phone_label'>S'il vous plait, entrez votre numéro de téléphone</label><input type='text' id='phone1' /> <div id='div_submit'><input type='submit' id='submit_phone' value='Submit' onclick='onSubmit("+value+");' /></div></div>";
...
RightNow.Ajax.makeRequest('/cc/AjaxCustom/getCheckboxValue', {checked_value:flag , order_no:value}, {
	...
	function onSubmit(order_number)
	{
		...
		if(phone_number == "")
		{
			...
			document.getElementById("invalid_data").innerHTML ="s'il vous plaît entrer un numéro de téléphone valide";

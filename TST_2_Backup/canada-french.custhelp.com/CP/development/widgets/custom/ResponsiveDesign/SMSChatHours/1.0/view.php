		 ...
		 if(!$enabled[$i])
		 {
			...
			/*-----------------------------------convert to 12 hour---------------------------------------------*/
				...
				if($this->day_order($start)." - ".$this->day_order($end) == "Samedi - Dimanche")
				{
				  ...
				  echo "Sam à Dim: ".$string." ".$this->data['js']['nowtimezone']."<br/>";
				}
				
				elseif($this->day_order($start)." - ".$this->day_order($end) == "Lundi - Vendredi")
				{
				  echo $string." ".$this->data['js']['nowtimezone']." ".$this->day_order($start)[0]."-".$this->day_order($end)[0]."<br/>";
				}
				elseif($this->day_order($start)." - ".$this->day_order($end) == "Lundi - Samedi")
				{
				  echo $string." ".$this->data['js']['nowtimezone']." ".$this->day_order($start)[0]."-".$this->day_order($end)[0]."<br/>";
				}
				elseif($this->day_order($start)." - ".$this->day_order($end) == "Lundi - Dimanche")
				{
		   ...
		   { 
			...
			$string2 = str_replace($hourswithspace, $hourswithoutspace, $string2); 
				...
				{
					if($this->day_order($start) == 'Dimanche')
						echo "Dim: fermé<br/>";
					else
	...
	if(array_key_exists($i,$arr_duplicates))
	{
		...
		{	
			...
			/*-----------------------------------convert to 12 hour---------------------------------------------*/
				...
				{
				echo $this->day_order($start)." - ".$this->day_order($end).":  "."Le texte n'est pas disponible<br/>";
				}
		   ...
		   else if($start)
		   {
			...
			$string2=rtrim($string2,', ');
				...
				{
				echo $this->day_order($start).": "."Le texte n'est pas disponible<br/>";
				}
		   }
		   else
		   {
		   echo $this->day_order($i).": "."Le texte n'est pas disponible<br/>";
		   }

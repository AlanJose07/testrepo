RightNow.namespace('Custom.Widgets.input.SelectionInput_CCF');
Custom.Widgets.input.SelectionInput_CCF = RightNow.Widgets.SelectionInput.extend({ 
    /**
    ...
    overrides: {      
        ...
        constructor: function(data, instanceID) {
			...
			if((fieldName == "Incident.CustomFields.c.ccf_shakeology")||(fieldName == "Incident.CustomFields.c.ccf_tbb_club_membership")||(fieldName == "Incident.CustomFields.c.ccf_other_continued_services") ||(fieldName == "Incident.CustomFields.c.ccf_pro_team"))
			{
				...
				RightNow.Event.subscribe("evt_none", this.none, this);//Check whether None option is selected
			}
				if(fieldName == "Incident.CustomFields.c.ccf_shakeology_boost")
			{
			...
			if(fieldName == "Incident.CustomFields.c.ccf_performance_uk")
			{
			...
			if(fieldName == "Incident.CustomFields.c.ccf_3_day_refresh")
			{
	...
	*/
	...
	displayErrorCheckbox: function(errors, errorLocation) {
        ...
        if(commonErrorDiv) {
            for(var i = 0, errorString = "", message, id = "terms_of_cancellation"; i < errors.length; i++) {
				...
				var message = "Please select at least one service which you would like to continue";
	...
	/*
	* Function to disable all the other options of "Other memberships and enrollments" when "none" is selected
	*/
	...
	/*
	* Function to display Shakeology Boost dropdown when Shakeology Boost checkbox is selected
	*/

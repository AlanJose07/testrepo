...
class SelectionInput extends \RightNow\Widgets\SelectionInput {
    ...
    function getData() {
       ...
       // $this->data['hideEmptyOption'] = true; // null option is not needed in life time rank dropdown
        parent::getData();
	...
	/*------------------------------------Diamond Line New Customization----------------------------------------------------------*/
		...
		$rmas = RNCPHP\ROQL::queryObject("SELECT CO.CustomChatHours FROM CO.CustomChatHours WHERE CO.CustomChatHours.ID = 8")->next();
		$rma = $rmas->next();

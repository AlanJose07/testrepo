					...
					<div class="card-list">
						...
						for ($i = 0; $i < $lessthan9count; $i++) {
							...
							if ($product['content_type_name'] == 'text') {
								?>
								<div class="card"> <a href="javascript:void(0);"
										onclick="document.location='<?= $product['content'] ?>';click_tracking('Service Tools - <?= $product['product_title']; ?>')">
										<div class="icon"><img
												src="/euf/assets/themes/responsive/images/home_selfservice/<?= $product['image_name']; ?>">
										</div>
										<h2>
							...
							<?
								...
								?>
								<div class="card"> <a href="javascript:void(0);"
										onclick="showPopup('<?= $product['product_title'] ?>');click_tracking('Service Tools - <?= $product['product_title']; ?>')">
										<div class="icon"><img
												src="/euf/assets/themes/responsive/images/home_selfservice/<?= $product['image_name']; ?>">
										</div>
										<h2>
					...
					<div class="card-list">
						...
						if ($list_count > 0) {
							...
							<div class="visible-xs text-center more-products">
								<button data-toggle="collapse" data-target="#balance-product" class="more-product"
									onclick="change()" id="more_prod_link"> More Tools </button>
	...
	<div class="modal-content Bb_modal">
		<div class="model-inner">
			...
			<div class="info-text" id="odrSub" style="display:block; visibility:visible">
				<p>*Coaches-Coach Online Office results are <br>updated more frequently. Please visit the Coach
					<br>Online Office for timely updates.</p>
			</div>
			...
			<form id="rn_QuestionSubmit" method="post">
				...
				<div class="form-dv">
					...
					<div class="left-box">
						<rn:widget path="custom/ResponsiveDesign/countrySelectionInput_OrderStatus"
							name="Contact.Address.Country" required="false" label_input="Country/Pais/Pays"
							default_value='7' />
					</div>
				...
				<div class="form-dv">
					...
					<div class="info-text">
						<p>*Enter email and zip / post / postal code of shipping address.</p>
					</div>
				...
				<div class="model-footer">
					...
					<rn:widget path="custom/ResponsiveDesign/FormSubmitOrderStatus" error_location="rn_ErrorLocation"
						interface_type="true" />
...
<div id="myModal_ccu" class="modal View_Modal pop-modal card_popup">
	...
	<div class="modal-content Bb_modal">
	 ...
	 <p>*Coaches-Coach Online Office results are <br>updated more frequently. Please visit the Coach <br>Online Office for timely updates.</p>
				...
				<form id="rn_QuestionSubmit_vv" method="post">
					...
					<div class="form-dv">
						...
						<div class="center-box">
							...
							<rn:widget path="custom/ResponsiveDesign/TextInputCOCreditCardUpdate"
								name="CO.order_tracking_new.email" label_input="Email" required="false" />
					...
					<div class="form-dv">
						...
						<div class="center-box">
							<div class="text-2">Step 2:</div>
							<rn:widget path="custom/ResponsiveDesign/TextInputCOCreditCardUpdate"
								name="CO.order_tracking_new.zip_code" label_input="Zip / Postal Code"
								required="false" />
					...
					<div class="model-footer">
						...
						<rn:widget path="custom/ResponsiveDesign/recordSearchSubmit"
							error_location="rn_ErrorLocation_vv" interface_type="true" />
					</div>
			...
			<div id="closeButton" style="display:none !important"> <input type="button" name="close" value="CLOSE"
			...
			<div id="formdisplay" style="display: none">
				...
				<form id="form" onsubmit="return false">
					...
					<div id="displaynote">
						<h4 style="color: red">Please expect these changes to take up to 24 hours to be applied to your
							account.</h4>
					</div>
					<br /><br />
					...
					<div id="firstName" style="display: none">
						<rn:widget path="input/TextInput" name="Contact.Name.First" required="true" label="First Name"
							label_input="First name" /><br />
					</div>
					<div id="lastName" style="display: none">
						<rn:widget path="input/TextInput" name="Contact.Name.Last" required="true" label="Last Name"
							label_input="Last name" />
					</div><br /><br /><br />
					<!---------------------------------Updated by jithin(below code) select credit card type based on country selection------------------------------------------>
					<div class="country">
						<rn:widget path="custom/ResponsiveDesign/countrySelectionInput_ccu"
							name="Contact.Address.Country" required="true" label_input="Country" default_value='7' />
					</div>

					<div class="card_type" id="test">
						<rn:widget path="custom/ResponsiveDesign/UpdateCreditCardFormInput"
							name="Update_Acnt.Update_Account_Info.credit_card_type" required="true"
							label_input="Credit Card Type" />
					</div>
					<!---------------------------------Updated by jithin(above code) select credit card type based on country selection------------------------------------------>
					<div>
						<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
							name="Update_Acnt.Update_Account_Info.full_name" required="true" label="Name on the card" />
						<br />
					</div>
					<div class="">
						<div class=""><!--credit_card_number-->
							<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
								name="Update_Acnt.Update_Account_Info.credit_card_number" required="true"
				...
				<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput" name="Update_Acnt.Update_Account_Info.security_code" required="true" label="CVV / CSC"/><br/>
					...
					<div class="credit_card_expiry">
						<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
							name="Update_Acnt.Update_Account_Info.exp_month" required="true" label="Expiration" />
						<div class="cvv year">
							<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
								name="Update_Acnt.Update_Account_Info.exp_year" required="true" label="" /><br />
						</div>
					</div><br /><br />
					...
					<div>
						<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
							name="Update_Acnt.Update_Account_Info.bill_to_address" required="true"
							label="Address line1" /><br />
					</div>
					<div>
						<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
							name="Update_Acnt.Update_Account_Info.apt_suite" label="Address line2" />
					</div>
					<div class="">
						<div class="city">
							<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
								name="Update_Acnt.Update_Account_Info.bill_to_city" required="true" label="City/Town" />
						</div>
						<div class="state" id="state_req">
							...
							<rn:widget path="custom/ResponsiveDesign/SelectionInputState"
								name="Contact.Address.StateOrProvince" label_input="State / Province" required="true"
								required="false" />
						</div>
					</div>
					...
					<div class="">
						<div class="zip">
							<rn:widget path="ResponsiveDesign/UpdateCreditCardFormInput"
								name="Update_Acnt.Update_Account_Info.bill_to_postal_code" required="true"
								label="Zip / Post / Postal code" />
						</div>
						<!--<div class="country">
...
<script>
	...
	span.onclick = function () {
		...
		document.getElementsByName("Contact.Address.Country")[0].value = 7;
	...
	function closePopUp() {
		...
		document.getElementsByName("Update_Acnt.Update_Account_Info.credit_card_type")[0].value = "";
		...
		document.getElementsByName("Contact.Address.Country")[1].value = 7;

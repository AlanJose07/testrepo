...
Custom.Widgets.ResponsiveDesign.UpdateCreditCardSelection = RightNow.Widgets.extend({ 
     ...
     */
	 ...
	 checkCountryOnLoad: function(c_id) {
		     
		var eventObject = new RightNow.Event.EventObject(this, {data: {country_id: 7}});
		RightNow.Ajax.makeRequest('/cc/AjaxCustom1/screenCardTypesByCountry', {country_id:7}, {
			successHandler: function(response) {
				var wid = this.baseSelector+"_";
				var selbox = this.Y.one(wid); 
				this.Y.one(wid).get('options').remove();

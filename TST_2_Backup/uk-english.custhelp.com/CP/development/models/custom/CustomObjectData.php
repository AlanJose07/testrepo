...
class CustomObjectData extends \RightNow\Models\Base {
	...
	function fetchSpecialFeatureData() {
		...
		$rows = RNCPHP\ROQL::query( "SELECT * FROM BB.special_feature s WHERE s.enable = 1 and s.interface = 3 order by display_order_spc" )->next();
		$i=0;
		while($row = $rows->next())
		{
			...
			if($row['Type'] == 2){
				...
				$timestamp1 = strtotime($start_date);
				$time1 = $timestamp1 - (7 * 60 * 60);
				$start_date = date("Y-m-d H:i", $time1);
				
				$timestamp2 = strtotime($end_date);
				$time2 = $timestamp2 - (7 * 60 * 60);
				$end_date = date("Y-m-d H:i", $time2);
			}
	...
	function fetchShadowpopup($url) {
		//initConnectAPI('OSC_NICE', 'Yatebr2H');
		$shadow_url = RNCPHP\ROQL::query( "select shadow_popup, ID from BB.url_mapping where BB.url_mapping.URL ='".$url."' and shadow_popup.interface = 3 and shadow_popup.enable = 1 LIMIT 1" )->next()->next();

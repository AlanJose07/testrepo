...
class IncidentModel extends \RightNow\Models\Incident{
     ...
     function get_recommended_channel($request_Id, $member_Id, $current_url)
	{
		...
		$channel = RNCPHP\ROQL::queryObject("select CO.channel_decision_uk from CO.channel_decision_uk where CO.channel_decision_uk.member_type='$member_Id'
		and CO.channel_decision_uk.request_type='$request_Id'")->next();
    ...
    public function create(array $formData, $smartAssist = false) {
        ...
        if ($contact = $this->getContact()) {
            ...
            if($formData['Incident.PrimaryContact'] instanceof RNCPHP\Contact){
    ...
    protected function getSmartAssistantResults(KnowledgeFoundation\SmartAssistantContentSearch $incidentContent, array $keyValueList) {
		...
		{
			 $ansobj=RNCPHP\Answer::fetch($results[suggestions][0]['list'][$i]['ID']);

...
class AjaxCustom extends \RightNow\Controllers\Base
{
	...
	public function sendSponsorForm()
    {
        ...
        $smartAssistant = $this->input->post('smrt_asst');
        //echo json_decode($this->model('custom/FieldModel')->sendSponsorForm($data, $listOfUpdateRecordIDs ?: array(), ($smartAssistant === 'true')));
		$result = $this->model('Field')->sendForm($data, $listOfUpdateRecordIDs ?: array(), ($smartAssistant === 'true'));
		//echo "<pre>";
		//print_r(json_decode($result->toJson()));
		//$inc_id = json_decode($result->toJson())->result->transaction->incident->value;
		/*echo "\n inc id is ".$inc_id;
		if($inc_id && intval($inc_id) > 0)
		{
			$incident = RNCPHP\Incident::fetch(intval($inc_id));
			$incident->Threads[1] = new RNCPHP\Thread();
			$incident->Threads[1]->EntryType = $incident->Threads[0]->EntryType;
			$incident->Threads[1]->EntryType->ID = 1;
			$incident->Threads[1]->Text = $incident->Threads[0]->Text;

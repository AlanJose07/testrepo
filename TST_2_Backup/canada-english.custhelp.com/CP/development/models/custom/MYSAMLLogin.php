...
class MYSAMLLogin extends \RightNow\Models\Base
{
	...
	function redirect(&$preHookData)
	{
		...
		elseif($CI->session->getSessionData("chat") == "sms")
		{
		$sms_redirect = $CI->session->getSessionData("url");
		$sms = array("chat"=>" ","url"=>" "); // destroying the session data
		$CI->session->setSessionData($sms);
		Framework::setLocationHeader("https://".$_SERVER['HTTP_HOST']."/app/smssurvey/".$sms_redirect);
		exit;
		}

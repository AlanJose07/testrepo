RightNow.namespace('Custom.Widgets.lob.customguidedassistance');
Custom.Widgets.lob.customguidedassistance = RightNow.Widgets.extend({ 
...
********************************/
    ...
    */
    _createImageHTML: function(question) {
        ...
        var questionID = question.questionID;
         var img=new EJS({text: this.getStatic().templates.imageResponse}).render({
            id: this._buildResponseID(questionID),
            guideID: this._currentGuideID,
            questionID: questionID,
            accessibleText: question.taglessText,
		...
		var img = img.replace(/<div>/g, "<div class=\"imagealign\">");
		console.log(img);
		return img;
    },
    ...
    */
    _createAnswersHTML: function(response) {
		...
		for(var i=0;i<response.childAnswers.length;i++)
		{    
			 ansid.push(response.childAnswers[i].id);
		}
	    var ajaxResult='';
        ...
        if (response.childAnswers) {
           /* answers = new EJS({text: this.getStatic().templates.answerResult}).render({
			...
			//var answers= "<div class="+"\"rn_Result rn_ResultLink\""+">"+ajaxResult[0]+"</div>";
			var answers="<div class="+"\"rn_Result rn_ResultLink\""+">";
			for(var i=0;i<response.childAnswers.length;i++)
			{
				answers+=ajaxResult[0];
			}
			answers+="</div>";
        }
        return answers;
    ...
    _createNewGuide: function(guide, parentGuideID) {
        ...
        var question = guide.questions[0],
            ...
            result = this.Y.Node.create("<div class='rn_Guide rn_Result'></div>")
                ...
                .append(this.Y.Node.create("<div class='rn_Question1 rn_Node1'></div>")

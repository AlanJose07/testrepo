...
Custom.Widgets.reports.Multiline = RightNow.ResultsDisplay.extend({
    overrides: {
    ...
    _onReportChanged: function(type, args) {
		...
		var newdata = args[0].data,
			...
			/*****************by jithin******************************/
		     ...
		     $.ajax({
			 url: '/cc/AjaxCustom/get_hide_report_id',
			 async:false,
			 dataType: "json", 
			 type: "POST",
			 ...
			 for(var i=0;i<args[0].data['data'].length;i++)
			 {
				 var str = args[0].data['data'][i][1];
                 str = str.replace('(ID:',' ');
                 str=str.replace(')',' ');
                 str=str.trim();
				  if(jQuery.inArray(str,ajaxResult) !== -1){n.push("false");}
				  else{n.push("true");}  
			 }
			 args[0].data["hideanswercheck"]=n;
		/***********************by jithin*****************************/	

 ...
 RightNow.namespace('Custom.Widgets.search.FilterCheckbox');
 Custom.Widgets.search.FilterCheckbox = RightNow.SearchFilter.extend({
     overrides: {
         constructor: function() {
             this.parent();
 
             this._eo = new RightNow.Event.EventObject(this);
             this._selectBox = this.Y.one(this.baseSelector + "_Checkbox");
             this._Checkbox = document.getElementById("rn_" + this.instanceID + "_Checkbox");
             this._selectBox.on("click", this._onSelectChange, this);
            ...
            // this._setSelectedDropdownItem(this.data.js.defaultValue);
             this._setFilter();
             this._setCheckBox(); // To keep the check box selected when coming back from detail page
     ...
     _onSelectChange: function(evt)
     {
         ...
         this._eo.event_data = evt.type;	
 
         RightNow.Event.fire("evt_selectedrequest", this._eo); //event fired to set the set the filter data

         /* Commented out by Varun
     ...
     _setCheckBox: function()
     {
         ...
         var sName = encodeURIComponent(this._eo.filters.searchName);
         var validURL = url.search(sName+"/1");
         if(validURL >=0)
         {
             var checkbox = document.getElementById("rn_" + this._eo.w_id + "_Checkbox");
             checkbox.setAttribute("checked", "checked");
             this._eo.filters.data.val = 1;

...
RightNow.namespace('Custom.Widgets.input.TextInputCO');
Custom.Widgets.input.TextInputCO = RightNow.Field.extend({
    overrides: {
        constructor: function(data, instanceID) {
			...
			if(this._fieldName === "cf_other_desc" || this._fieldName === "cf_meds_desc" || this._fieldName === 			"cf_allergies_desc" || this._fieldName === "cf_condition_desc" || this._fieldName === "cf_attention_desc"||this._fieldName === "cf_other_notes"||this._fieldName === "cf_injury_comp_desc")
			{
         ...
         RightNow.Event.subscribe("evt_EvaluateRequired", this._eraseInput, this);
			...
			if(this._fieldName === "cf_date_used"){ 
			    
				RightNow.Event.subscribe("change_label", this._fields_set_attribute, this);
				RightNow.Event.subscribe("reset_Field", this._fields_reset,this);
			}
			if(this._fieldName === "cf_lot_num"){ 
			    
				RightNow.Event.subscribe("change_label", this._fields_set_attribute, this);
				RightNow.Event.subscribe("reset_Field", this._fields_reset,this);
			}
			if(this._fieldName === "cf_date_bike_delivery"){ 
			...
			if(this._fieldName === "cf_osc_incident_number"){ 
			    
				RightNow.Event.subscribe("show_fields", this._displayBlock, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);

			}
			if(this._fieldName === "cf_injured_person_name"){ 
			    
				RightNow.Event.subscribe("show_fields", this._displayBlock, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);

			}
			if(this._fieldName === "cf_injury_description"){ 
			    
				RightNow.Event.subscribe("show_fields", this._displayBlock, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);

			}
				if(this._fieldName === "cf_doctors_details"){ 
				RightNow.Event.subscribe("show_fields", this._displayBlock, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);

			}
			if(this._fieldName === "cf_injury_type_other"){ 
				RightNow.Event.subscribe("Required_other_option", this._fields_required, this);
				RightNow.Event.subscribe("notRequired_other_option", this._fields_not_required, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);

			}
				if(this._fieldName === "cf_body_parts_other"){ 
				RightNow.Event.subscribe("Required_other_option_injury", this._fields_required, this);
				RightNow.Event.subscribe("notRequired_other_option_injury", this._fields_not_required, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);

			}
			if(this._fieldName === "cf_bike_parts_other"){ 
			    RightNow.Event.subscribe("Required_other_option_bike", this._fields_required, this);
				RightNow.Event.subscribe("notRequired_other_option_bike", this._fields_not_required, this);
				RightNow.Event.subscribe("hide_fields", this._displayNone, this);
	...
	{
		if(this.globalfieldname=="cf_date_bike_delivery")
		{
			
			 document.getElementById("bike_delivery").style.display = "none";
			  this._fields_not_required();

		}
		if(this.globalfieldname=="cf_osc_incident_number")
		{
			 document.getElementById("osc_incident").style.display = "none";
			 this._fields_not_required();
		}
		if(this.globalfieldname=="cf_injured_person_name")
		{
			 document.getElementById("injured_person").style.display = "none";
			 this._fields_not_required();
		}
				if(this.globalfieldname=="cf_injury_description")
		{
			 document.getElementById("injury_decs").style.display = "none";
			 this._fields_not_required();
		}
				if(this.globalfieldname=="cf_doctors_details")
		{
			 document.getElementById("doctors_details").style.display = "none";
			 this._fields_not_required();
		}
		if(this.globalfieldname=="cf_injury_type_other")
		{
			 document.getElementById("other_text").style.display = "none";
			 this._fields_not_required();
		}
			if(this.globalfieldname=="cf_body_parts_other")
		{
			 document.getElementById("other_text_injury").style.display = "none";
			 this._fields_not_required();
		}
				if(this.globalfieldname=="cf_bike_parts_other")
		{
	...
	{
					console.log(this.globalfieldname);
		if(this.globalfieldname=="cf_date_bike_delivery")
		{

			 document.getElementById("bike_delivery").style.display = "block";
			 this._fields_required();
			 this._set_checkbox();
		}
		if(this.globalfieldname=="cf_osc_incident_number")
		{
			 document.getElementById("osc_incident").style.display = "block";
			  this._fields_required();
		}
		if(this.globalfieldname=="cf_injured_person_name")
		{
			 document.getElementById("injured_person").style.display = "block";
			 this._fields_not_required();
		}
		if(this.globalfieldname=="cf_injury_description")
		{
			 document.getElementById("injury_decs").style.display = "block";
			var textHeight=document.getElementById("rn_" + this.instanceID + "_" + "Complaint"+"."+"cf_injury_description");
			textHeight.style.height = "40px";
			 this._fields_not_required();
		}
		if(this.globalfieldname=="cf_doctors_details")
		{
			 document.getElementById("doctors_details").style.display = "block";
			 var textHeight=document.getElementById("rn_" + this.instanceID + "_" + "Complaint"+"."+"cf_doctors_details");
			textHeight.style.height = "40px";
			 this._fields_not_required();
		}
	},	
	_fields_reset: function(type,args)
	{
				 if(this.globalfieldname=="cf_date_used")
				 {

					  this.data.attrs.label_input = "Date Used";
					  var labelnew = document.getElementById("rn_" + this.instanceID + "_" + "Label");
					  labelnew.innerHTML=this.data.attrs.label_input+"<span class='rn_Required'> *</span>";
					  labelnew.style.cssFloat = "left";
					  //var label2new= document.getElementById("rn_" + this.instanceID + "_" + "Label");
					   var nacheck_exsist= document.getElementById("rn_" + this.instanceID + "_" + "nacheck");
					   if(nacheck_exsist)
					   this._resetcheckbox(this);
					 
				 }
				 if(this.globalfieldname=="cf_lot_num")
				 {
					  this.data.attrs.label_input = "Lot#";
					  var labelnew = document.getElementById("rn_" + this.instanceID + "_" + "Label");
					  labelnew.innerHTML=this.data.attrs.label_input+"<span class='rn_Required'> *</span>";
					  labelnew.style.cssFloat = "left";
					  //var label2new= document.getElementById("rn_" + this.instanceID + "_" + "Label");
	...
	_fields_set_attribute: function(type,args)
	{ 
	        if(args[0]==3915)
			{
				 if(this.globalfieldname=="cf_date_used")
				 {
					  this.data.attrs.label_input = "Date of Injury";
					  ...
					  var nacheck_exsist= document.getElementById("rn_" + this.instanceID + "_" + "nacheck");
							...
							this._resetcheckbox(this);
					        if(this.data.attrs.showna && (!nacheck_exsist)){	
							var newdiv = document.getElementsByClassName ("label_input");
							var anotherDiv = document.createElement('div');
							anotherDiv.classList.add('nacheck');
							anotherDiv.style.fontWeight="bold";
							anotherDiv.innerHTML="<input type='checkbox' id='rn_" + this.instanceID + "_" + "nacheck' " + "name='N/A' value='N/A'>N/A</input>";
							labelnew.after(anotherDiv);
							this._field = this.Y.one("#rn_" + this.instanceID + "_nacheck");
							this._field.on("change",this._naChanged,this);
				 ...
				 if(this.globalfieldname=="cf_lot_num")
				 {
					  this.data.attrs.label_input = "Bike Serial No";
					  var labelnew = document.getElementById("rn_" + this.instanceID + "_" + "Label");
					  labelnew.innerHTML=this.data.attrs.label_input+"<span class='rn_Required'> *</span>";
					  labelnew.style.cssFloat = "left";
					   var nacheck_exsist= document.getElementById("rn_" + this.instanceID + "_" + "nacheck");

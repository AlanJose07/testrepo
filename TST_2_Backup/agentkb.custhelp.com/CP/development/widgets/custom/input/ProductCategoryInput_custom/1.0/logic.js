RightNow.namespace('Custom.Widgets.input.ProductCategoryInput_custom');
Custom.Widgets.input.ProductCategoryInput_custom = RightNow.Field.extend({ 
    /**
    ...
    overrides: {
        ...
        constructor: function(data, instanceID) {
			...
			this.globalresetarray=this.data.js.hierData;
            ...
            if(this.data.js.hierData[0].length)
				...
				if(this.data.attrs.p_id==1308)
				{
					
				}
				if(fieldName=="Incident.Category")
				{
					RightNow.Event.subscribe("set_category", this.reason_Set, this);
				    RightNow.Event.subscribe("reset_category", this.reason_reSet, this);
		...
		newheirValues=this.data.js.myxdata;
		this._buildMyxTree(newheirValues);
	
	
	 },
	 reason_reSet : function(){
		//for resetting the reason back to normal 
    ...
    _buildMenuPanel: function() {
        ...
        this._panel.on('visibleChange', function(e) {
            if (!e.newVal && this.data.attrs.hint && !this.data.attrs.always_show_hint) {
                // Now hiding
	 ...
	 /**
     *Ajax request to fetch ProductID from current selected product
     */
	  ...
	  _getProdIDfromCurrentIndex : function(){
        ...
        var reverse_array = hierValues.reverse();
        var prod_id = reverse_array.pop();
		var top_parent=reverse_array.pop(1);
		if(top_parent==3915 && this.globalfieldname=="Incident.Product")
		{
			RightNow.Event.fire("set_category",this);
			RightNow.Event.fire("change_label",top_parent);
			RightNow.Event.fire("show_fields",top_parent);
		}
		else if(this.globalfieldname=="Incident.Product" && top_parent!=3915)
		{
			RightNow.Event.fire("reset_category",this);
			RightNow.Event.fire("reset_Field",top_parent);
			RightNow.Event.fire("hide_fields",top_parent);
			RightNow.Event.fire("evt_reset_hint",top_parent);
			RightNow.Event.fire("removeNoteField",top_parent);
			RightNow.Event.fire("reset_menu_values",this);
		...
		{
	      RightNow.Ajax.makeRequest("/cc/ajaxCustom/getAutoFillFields", {prodID:prod_id}, {"successHandler": this._sendAutoFillFields, "scope":this});
		}
     ...
     /**
     *Sends response to populate the text fields
     */       
  
    _sendAutoFillFields : function(response){

        var results = RightNow.JSON.parse(response.responseText);
        
        this._eoFill = new RightNow.Event.EventObject();
        this._eoFill.data = {"type" : results[0],
                              "sku" : results[1]};
        RightNow.Event.fire("evt_FillValues", this._eoFill);
    },
    _resetTree : function()
    {
        ...
        if(treeDiv && gallery.TreeView)
        {
			...
			this.data.js.myxdata=this.data.js.hierData;
            ...
            new this.Y.Node(treeDiv).setStyle("display", "block");
            if(hasDefaultValue)
                this._displaySelectedNodesAndClose(false);
        }
    },
	 _buildMyxTree : function(newheirValues)
    { 
            ...
            for(i in this._flatTreeViewData)
            {
                if(this._flatTreeViewData.hasOwnProperty(i))
                {
                    ...
                    if(item.level === previousLevel)
                        htmlList += "</li>";
                    //print current node
                    htmlList += "<li>" + '<a href="javascript:void(0)" id="rn_' +  this.instanceID + '_AccessibleLink_' + item[1] + '" class="rn_AccessibleHierLink" data-hierList="' + item['hier_list'] + '">' + item[0] + '</a>';
    ...
    */
    _displaySelectedNodesAndClose: function(focus)
    {
        ...
        var hierValues, description, descText, pro_id ;

        this._eo.data.value = this._currentIndex;
		if(this._checkRequiredLevel() && this.data.attrs.label_input == "Product" || this.data.attrs.label_input == "Reason")
		
		/*Anoop  - code to get the product id so as to make the functionality dynamic*/
		
		pro_id  = this._getProdIDfromCurrentIndex();
        ...
        else
        {
			...
			if (this.data.attrs.label_input == "Reason"){
			...
			/*code by anoop to make the multiselect div visibility dynamic*/
			
				if (this.data.attrs.label_input == "Reason"){
                this._eoShowFields = new RightNow.Event.EventObject();
                this._eoShowFields.data = {"prod" : pro_id};
		
				
				RightNow.Event.fire("evt_PCchosen_menu", this._eoShowFields);
            }

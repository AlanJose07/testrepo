...
class Field_beachbody_custom extends \RightNow\Models\Field{
     ...
     */ 
    public function sendForm($formData,$FName, $LName, $phones_detail,$country_detail, $thread_text, $thread_text2, $listOfUpdateIDs = array(), $smartAssistant = false) {
        if ($tokenError = $this->verifyFormToken()) return $tokenError;
		...
		if($contactDetail ->Address->Country){
			...
			}else if($country_detail== 23){
				$contactDetail->Address->Country = RNCPHP\Country::fetch(23);
			}
		...
		$countMobPhone=0;
		$contactDetail->Phones = new RNCPHP\PhoneArray();
		for($i=0; $i<count($contactDetail->Phones); $i++)
		{
			
			if($contactDetail->Phones[$i]->PhoneType->ID == 1)
			{
				$countMobPhone++;
				if(!empty($phones_detail))
				{
					...
					$contactDetail->Phones[$totalPhones]->PhoneType->ID = 1;
		...
		if($countMobPhone == 0) 
		{
			...
			//print_r($totalPhones); echo "ji"
			//print_r($contactDetail->Phones[$totalPhones]->Number);

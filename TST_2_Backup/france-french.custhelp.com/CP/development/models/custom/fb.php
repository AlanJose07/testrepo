...
class fb extends \RightNow\Models\Base {
    ...
    public function sendForm($formData, $listOfUpdateIDs = array(), $smartAssistant = false) { 
        ...
        if ($presentFields['contact']) {
            //Contact update
          /*  if ($userIsLoggedIn) {
                $return = $this->CI->model('custom/fb_contact')->update($this->CI->session->getProfileData('contactID'), $formData);
				return "here i am ";
                $action = 'Updated';
            }*/
        ...
        if ($presentFields['incident'] && !$presentFields['socialquestion']) { 
            ...
            if($rawFields = $this->getRawFormFields('Socialquestion.Body')) {
                $formData['Socialquestion.Body']->value = $rawFields->value;
            }

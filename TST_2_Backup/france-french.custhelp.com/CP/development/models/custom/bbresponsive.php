	...
	function channel_details_result($query)
	{
	  ...
	  try
	  {
		  ...
		  if($res->count()>0)
	   {
		   ...
		   {
		     if($channel->Directly_RTM->ID==3 || $channel->Agent_Chat->ID==3 || $channel->Email->ID==3 || $channel->Phone->ID==3 || $channel->Self_Service_Form->ID==3 || $channel->facebook->ID==3)
			 {
...
//doc submit start
	...
	//email submit start
	function save_subject_email($a)
	{
	  try{
		if (\RightNow\Utils\Framework::isLoggedIn()) 	
			{
		logmessage("in logged inc save");
			$incident = RNCPHP\Incident::fetch($a);
			$incident->Subject = "Votre demande d´assistance BODi";
			$incident->CustomFields->c->account_verified = 1;
			$incident->save();
	} else {
		logmessage("in not logged inc save");
			$incident = RNCPHP\Incident::fetch($a);
			$incident->Subject = "Votre demande d´assistance BODi";
			$incident->save();
	}
...
function get_enabled_category($category_id,$contactus_support)
		   ...
		   if(is_numeric($category_id))
			{
					...
					if($catParentIdFirst!="")
					{
											...
											$enable = "SELECT bbb.enable 
												   FROM BB.categ_contac_hom_fra bbb 
	...
	function remove_bookmarked_url($category_id,$page)
	{
	  try
	  {
		...
		if($page == "answers"){
	    $baseQuery="Select answer_report_id from BB.categ_contac_hom_fra where BB.categ_contac_hom_fra.category.ID =".$category_id." and BB.categ_contac_hom_fra.visible_on_page.ID != 3 and BB.categ_contac_hom_fra.visible_on_page.ID != 4";
		}
		
	  	if($page == "contactus_support"){
	    $baseQuery="Select answer_report_id from BB.categ_contac_hom_fra where BB.categ_contac_hom_fra.category.ID =".$category_id." and BB.categ_contac_hom_fra.visible_on_page.ID != 1 and BB.categ_contac_hom_fra.visible_on_page.ID != 2";
		}
		if(is_numeric($category_id))
		{
			$pro =  RNCPHP\ROQL::query($baseQuery)->next();
	...
	// 		else
	// 			$logged_in="no";
	// 			$query="select BB.categ_contac_hom_fra from BB.categ_contac_hom_fra where BB.categ_contac_hom_fra.category.ID=".$catid." and BB.categ_contac_hom_fra.interface.ID=31 and BB.categ_contac_hom_fra.visible_on_page.ID in(3,4)"; 
	// 			$res = RNCPHP\ROQL::queryObject($query)->next();
	// 			if($res->count()>0)
	// 			{
	// 			   while($channel = $res->next()) 

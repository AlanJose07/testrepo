...
class SmartAsstChatModel extends \RightNow\Models\Incident{
    ...
    public function create(array $formData, $smartAssist = false) {
        ...
        if($smartAssist === true){
           ...
           // Connect\ConnectAPI::setSource(SRC2_EU_SMART_ASST);
            ...
            $smartAssistantResults = $this->getSmartAssistantResults($smartAssistantIncidentContent, $additionalFields);//issue here
...
/*Comment end*/
  ...
  print_r($keyValueList);*/
      ...
      /*  ActionCapture::record('incident', 'suggest');*/
		...
		$searchTerms = $incidentContent->Summary." ".$incidentContent->DetailedDescription;
		
		$searchTerms = substr($searchTerms,0,255);
        ...
        try{
			...
			/*echo "<pre>";
			foreach ($smartAssistantSuggestions->Suggestions as $item)
			{
...
//no issue
		...
		$CI = get_instance();
        //$tab = $CI->session->getSessionData('currentLob');//'team';
		$mainurl = $_SERVER['HTTP_REFERER'];
		$url = explode("/",$mainurl);
		$index = array_search("lob",$url);
		$tab = $url[$index+1];
		//echo "tab is ".$tab; 
		/*if($tab == 'beach')
		{
			$tab_lob = $ansobj->CustomFields->c->beachbody;
		}
		else if($tab == 'shake')
		{
			$tab_lob = $ansobj->CustomFields->c->shakeology;
		}
		else if($tab == 'coach')
		{
			$tab_lob = $ansobj->CustomFields->c->coo;
		}
		else if($tab == 'beachbodylive')//"certification";//new page set certification
		{
			$tab_lob = $ansobj->CustomFields->c->p90x_certification;
		}
		else if($tab == 'team')
		{
			echo "lob = team";
			$tab_lob = $ansobj->CustomFields->c->tbb;
		}
		else
		{
			$tab_lob = $ansobj->CustomFields->c->tbb;
		 ...
		 
		{
			...
			/*echo $i;
			print_r($results[suggestions][0]['list'][$i]);*/
			if($results[suggestions][0]['list'][$i]['ID'] == 3973)
			{
				$ansobj->CustomFields->c->tbb;
			}
			 $ansobj=Connect\Answer::fetch($results[suggestions][0]['list'][$i]['ID']);  
				...
				//echo "--=".$tab_lob."=--=".$cnt."=--";
				if($tab_lob==1 && $cnt<5 && $item != null)//if($cnt<5 && $item != null)//first field is getting null from contact_us page
                {
				//echo "____inn___";
		...
		/*print_r($lob_filtered_res['suggestions'][0]['list']);
		die("------------------ DIE HARD 1.0 -------------------");*/
	  //If there are no results and the user can escalate the post, send back whether or not we ran rules
            //to determine if we have to show an empty result set (patent issues) or if we can immediately escalate the incident

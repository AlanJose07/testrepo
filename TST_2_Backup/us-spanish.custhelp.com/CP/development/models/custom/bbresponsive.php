...
class bbresponsive extends \RightNow\Models\Base {
  ...
  echo $e->getMessage();   
	...
	$order_status_pos=strpos( $clicked_link, "Comprobar el estado del pedido" );
		
	$update_credit_card_pos=strpos( $clicked_link, "Actualizar tarjeta de crédito" );
	...
	if($url_home_pos=="")//not from home page.so need to save category details
	{
		...
		function get_string_between($string, $start, $end)
		{
			...
			//jithin added (if and else condition)
			if (strcmp($fullurl, 'https://us-spanish--tst2.custhelp.com/app') == 0 || strcmp($fullurl, 'https://us-spanish--tst2.custhelp.com/app/') == 0) {
             $category_details="From Home Page";
			...
			//vimal added
			$check_cat_present= strpos( $fullurl, "catname" );
	 ...
	 for($i=0;$i<sizeof($contact->Phones);$i++)
	  {
	  		...
	  		if($contact->Phones[$i]->PhoneType->LookupName == 'Teléfono móvil')
			{
				$flag = 1;
				$contact->Phones[$i] = new RNCPHP\Phone();
				$contact->Phones[$i]->PhoneType = new RNCPHP\NamedIDOptList();
				$contact->Phones[$i]->PhoneType->LookupName = 'Teléfono móvil';
	  ...
	  if($flag == 2)
	  {
				...
				$contact->Phones[0]->PhoneType->LookupName = 'Teléfono móvil';
					...
					if(is_numeric($category_id))
					{
						...
						if($catParentIdFirst!="")
						{
								...
								$enable = "SELECT bbb.enable 
									   FROM BB.categ_contac_hom_sp bbb 
	...
	function remove_bookmarked_url($category_id,$page)
	{
		  try
		  {
				if($page == "contactus_support" || $page == "answers")
				{
					...
					if($page == "answers")
					{
						$baseQuery="Select answer_report_id from BB.categ_contac_hom_sp where BB.categ_contac_hom_sp.category.ID =".$category_id." and BB.categ_contac_hom_sp.visible_on_page.ID != 3 and BB.categ_contac_hom_sp.visible_on_page.ID != 4";
					}
					
					if($page == "contactus_support")
					{
						$baseQuery="Select answer_report_id from BB.categ_contac_hom_sp where BB.categ_contac_hom_sp.category.ID =".$category_id." and BB.categ_contac_hom_sp.visible_on_page.ID != 1 and BB.categ_contac_hom_sp.visible_on_page.ID != 2";
					}
					if(is_numeric($category_id))
					{
						$pro =  RNCPHP\ROQL::query($baseQuery)->next();
...
function setvoiceskill($catid,$membertypenew)
{
  ...
  try
  {
		if (\RightNow\Utils\Framework::isLoggedIn())  
		{
			...
			$query="select BB.categ_contac_hom_sp from BB.categ_contac_hom_sp where BB.categ_contac_hom_sp.category.ID=".$catid." and BB.categ_contac_hom_sp.interface.ID=29 and BB.categ_contac_hom_sp.visible_on_page.ID in(3,4)"; 
		$res = RNCPHP\ROQL::queryObject($query)->next();
		if($res->count()>0)
		{
		   while($channel = $res->next()) 
...
function CustomerLookup($searchtype,$country,$phone,$pinemail,$pinorder,$pinfirstname,$pinlastname,$stype,$member_typ,$contact_type,$coach_id,$guid)
{
    ...
    { 
        ...
        $curl = curl_init();
			curl_setopt_array($curl, array(
			CURLOPT_URL => "https://ebs.stage.gateway.beachbody.com/getCustomerOrders",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,

    ...
    <div id="rn_ChatContainer">
        ...
        <div id="rn_PageContent" class="rn_Live">
            <div class="rn_Padding">
					...
					<div id="rn_SmsTitle" class="rn_FloatLeft">Inicie sesión para continuar con la función envíenos un mensaje de texto.</div>
					</rn:condition>
			...
			<? if($CI->session->getSessionData('errormessage'))
			 ...
			 ?> 
				...
				document.getElementById("rn_ChatContainer").classList.add("new-style");
						...
						<?php }else{
					?>
		<div id="rn_SmsDialogTitle" class="rn_FloatLeft">Su solicitud ha sido enviada</div>
		<div id="rn_ChatLaunchForm">
		<div class="container">
		<div class="row chat-agent">
		<div class="sliderform-wrap">
		...
		<div id="rn_errorlocation_chat" style="display:none"></div>
			...
			<? if($CI->session->getSessionData('issmsavailable')== 2): ?>
			   Parece que todos nuestros agentes se han desconectado por hoy. Siéntase libre de continuar con la conversación y nuestros agentes responderán a las 7 am PST del siguiente día hábil.
			 <? else: ?>
			   Parece que todos nuestros agentes se han desconectado por hoy. Siéntase libre de continuar con la conversación y nuestros agentes responderán a las 7 am PST del siguiente día hábil.
			 <? endif; ?>
			</div>
		...
		//catch exception
		 ...
		 if($membertypeid == 388)
		 {
			...
			else
			{
				...
				?>
		<div id="rn_SmsTitle" class="rn_FloatLeft"><?php echo "Hola ".$this->session->getProfile()->first_name->value.", ahora solo verifique o cambie su número para comenzar." ?></div>
		...
		<rn:widget path="custom/ResponsiveDesign/TextInput" name="Contact.ph_mobile"  label_input="Número de Teléfono" required="true" allow_external_login_updates="true"/>
		</div>
		...
		<rn:widget path="custom/ResponsiveDesign/Submit" label_button="Enviar Texto" error_location="rn_errorlocation_chat" on_success_url='/app/home'/>	
		</div>
		 ...
		 <div id = "sms-hours">
		 <rn:widget path="custom/ResponsiveDesign/SMSChatHours" label_chat_hours ="Atención al Cliente" page=2/>  
		 </div>
		 </div>
		 </figcaption>
		 </figure>

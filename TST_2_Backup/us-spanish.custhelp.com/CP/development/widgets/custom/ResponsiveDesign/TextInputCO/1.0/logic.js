RightNow.namespace('Custom.Widgets.ResponsiveDesign.TextInputCO');
Custom.Widgets.ResponsiveDesign.TextInputCO = RightNow.Field.extend({
    overrides: {
        constructor: function(data, instanceID) {
			...
			{
				if(document.getElementById("email_instance"))
				document.getElementById("email_instance").value = this.instanceID;
				
				
				var field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				document.getElementById(field_id).placeholder="correo electrónico";
				
				//document.getElementById("rn_" + this.instanceID+"_Label").style.display="none";
				
										
			}
			if(this._fieldName == "zip_code")
			{
				if(document.getElementById("zip_code_instance"))
				document.getElementById("zip_code_instance").value = this.instanceID;
				
				
				var field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				document.getElementById(field_id).placeholder="Código postal ";
				
				//document.getElementById("rn_" + this.instanceID+"_Label").style.display="none";				
			}
			if(this._fieldName == "order_number")
			{
				if(document.getElementById("order_number_instance"))
				document.getElementById("order_number_instance").value = this.instanceID;
				
			
				var field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				
				//alert("field_id--"+field_id);
				document.getElementById(field_id).placeholder="Número de orden";
    ...
    onValidate: function(type, args) {
		...
		var email_err_flag=0;
		var email_zip_err_flag=0;
				
		var email = (document.getElementsByName("CO.order_tracking_new.email")[0].value).trim();
		...
		if(this._fieldName == 'email')
		{
		 ...
		 var zip_code = (document.getElementsByName("CO.order_tracking_new.zip_code")[0].value).trim();
		 var order_number = (document.getElementsByName("CO.order_tracking_new.order_number")[0].value).trim();
		  
		 var country_id=document.getElementsByName("Contact.Address.Country")[0].value;//new
		  ...
		  */
		 if(zip_code == "" && order_number=="" && email == "" && country_id==0)//new
		 {
			 var email_zip_err_flag = 1;
		 }
		
		
		 else if((order_number=="" && email==""))
		 {
			var email_zip_err_flag = 2;
		 }
		
		else if(zip_code=="")
		 {
			var email_zip_err_flag = 3;
		 }
		else if(country_id==0)//country is mandatory //new
		 {
			var email_zip_err_flag = 4;
		 }
		
	
		
			if(!this.validate(errors) || (this.data.attrs.require_validation && !this._validateVerifyField(null, errors)) || !this._compareInputToMask(true)|| email_err_flag==1 ||email_zip_err_flag==1 ||email_zip_err_flag==2 || email_zip_err_flag==3 || email_zip_err_flag==4) 
			{
			
			if(email_err_flag==1)
			//errors[errors.length] = "entered is invalid";
			errors[errors.length] = "entrado es inválido";
			
            this._displayError(errors, args[0].data.error_location,email_zip_err_flag);
    ...
    _displayError: function(errors, errorLocation,email_zip_err_flag) {
        ...
        if(commonErrorDiv) {
			...
			if(this._fieldName == 'email')
			{
			if(email_zip_err_flag == 1)
			{
				
				//message_err = "<b>Please fill Country/Pais/Pays, Order Number Or Email and Zip / Post / Postal Code";//english
				message_err = "<b>Complete el país / Pais / Pays, Número de orden o el correo electrónico y Código postal ";
				
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			else if(email_zip_err_flag == 2)
			{
				
				//message_err = "<b>Please fill either Order Number Or Email";//english
				message_err = "<b>Rellene el Número de orden o el correo electrónico";
				
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			
			else if(email_zip_err_flag == 3)
			{
				//message_err = "<b>Please fill Zip / Post / Postal Code";	
				message_err = "<b>Complete el Código postal";
				
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			else if(email_zip_err_flag == 4)//new
			{
				//message_err = "<b>Please select Country/Pais/Pays";	
				message_err = "<b>Complete el Country/Pais/Pays";
				document.getElementsByName("Contact.Address.Country")[0].className='rn_Text rn_ErrorField';
			}
			
			
			 errorString += message_err;
		}

    ...
    _confirmOnNavigate : function(result){
		...
		if(index!=-1)		
		 ...
		 if(checked_channel=="1530")
		 {
				...
				div.innerHTML += "<div class='email-conf-title'><h2>Su Email ha sido enviado</h2></div><div class='email-conf-cont'><div>Gracias por comunicarse con Beachbody. Hacemos todo lo posible para responder a los correos electrónicos dentro de las 4 horas.</div><div>Si su solicitud llega después de las 5 p.m. PT, le responderemos a la mañana siguiente.</div><div> Puede usar este número de referencia para seguimiento:&nbsp;<b>"+incident_ref_no+"</b></div></div>";
		...
		if(checked_channel=="1885")
		 {
				...
				if(document.getElementById('hide-for-email-confirm-popup').style.display=="block")
				{
					...
					var div = document.getElementById('after-submit-email');
					div.innerHTML += "<div class='email-conf-title'><h2>Su documento ha sido enviado</h2></div><div class='email-conf-cont'><div>Revisaremos su documento y le enviaremos un correo electrónico dentro de los dos días hábiles.</div><div> Puede usar este número de referencia para seguimiento:&nbsp;<b>"+incident_ref_no+"</b></div></div>";
		...
		if(checked_channel=="1621")//SMS
			{
				...
				if(args[0].url == "/ci/ajaxRequest/sendForm")
				{
					...
					args[0].url = "/cc/bbresponsivecontroller/sms/"+this.catid; 
					this._toggleLoadingIndicators(true);
					//window.location="https://faq.beachbody.com/app/responsivebb/home";
				}
			}	

...
Custom.Widgets.ResponsiveDesign.recordSearchSubmit = RightNow.Widgets.FormSubmit.extend({ 
    ...
    overrides: {
        ...
        if (!responseObject) {
			...
			document.getElementById("rn_ErrorLocation_vv").innerHTML ="<span>No pudimos ubicar su cuenta con la información a continuación, llámenos para actualizar su tarjeta de crédito.<br/>US and Canada: 800-470-7870 UK: 0800-953-6100</span>";
...
getOrderStatusMultiple: function(responseObject) {
		...
		heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Su pedido "+responseObject[i].order_number+" para "+responseObject[i].product+" ha sido "+status+". Si tu"+"'"+"Me gustaría hablar con un representante de Beachbody, marque (800) 470-7870 para obtener ayuda.</div></div>"	
			...
			*/
			...
			heading += "<br/><div class='multiple_order' id='block_"+i+"'style='display:none;'><div>Su pedido "+responseObject[i].order_number+" para "+responseObject[i].product+" colocado en "+responseObject[i].order_create_date +", muestra que "+status+" en "+responseObject[i].shipped_date +".</div><div class='track-div'>Su número de seguimiento es</div><div><a href="+href+" target='_blank'>"+responseObject[i].tracking_number+"</a></div><div class='new-modi'>"+footer_message+"</div></div>";
			...
			heading += "<hr class='clearfix'><h1 class='ccu_next_h1'>Haga clic en Siguiente para continuar</h1><br/>"
			$("div #heading").empty();
			//$("div #heading").html("We found your account. Here are your recent orders<hr/>");
			$("div #heading").html("Encontramos tu cuenta Aquí están sus pedidos recientes<hr/>");
			//heading+= '<button id="next" class="btn btn-primary" onclick="javascript:redirectNextPage_vv()" >Next</button>';	
			heading+= '<button id="next" class="btn btn-primary" onclick="javascript:redirectNextPage_vv()" >Siguiente</button>';	 
	...
	getOrderStatus: function(responseObject) {
		...
		if(document.getElementById("heading"))
		{
		   ...
		   if(responseObject.order_status_id == 2 || responseObject.order_status_id == 6)
			...
			heading="<div class='shipped_order'><div>Su pedido "+responseObject.order_number+" para "+responseObject.product+" ha sido "+status+". Si tu"+"'"+"Me gustaría hablar con un representante de Beachbody, marque (800) 470-7870 para obtener ayuda.</div></div>";
          ...
          // track_id="";
			...
			heading="<div class='shipped_order'><div>El orden "+responseObject.order_number+" lo pones en "+responseObject.order_create_date+" para "+responseObject.product+" fue recibido y es "+status+".</div><div>¿Que sigue? Recibirá un correo electrónico de confirmación de envío con detalles de seguimiento.</div><div class='new-modi most-order'>Asegúrese de revisar sus carpetas de correo no deseado y correo no deseado si no puede encontrar nuestros correos electrónicos.</div></div>";
			...
			else if(responseObject.tracking_number.length>=12 && responseObject.tracking_number.substring(0,1)=='7')
		   {
			...
			heading="<div class='shipped_order'><div class='order_top'>Su pedido "+responseObject.order_number+" para "+responseObject.product+" colocado en "+responseObject.order_create_date +", muestra que "+status+" en "+responseObject.shipped_date +".</div>";			
			//heading += "<hr class='clearfix'><h1 class='ccu_next_h1'>Click next to continue</h1><br/>"	
			heading += "<hr class='clearfix'><h1 class='ccu_next_h1'>Haga clic en Siguiente para continuar</h1><br/>"
			$("div #heading").empty();
			//$("div #heading").html("We found your account. Here are your recent orders<hr/>");

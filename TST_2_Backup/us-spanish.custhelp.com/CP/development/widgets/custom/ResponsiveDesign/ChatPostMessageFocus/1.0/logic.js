...
Custom.Widgets.ResponsiveDesign.ChatPostMessageFocus = RightNow.Widgets.ChatPostMessage.extend({ 
    ...
    overrides: {
		...
		{  
			document.title = "Asistencia en directo";
				...
				if(this.data.attrs.focus_on_incoming_messages)
				{
					...
					else
					{
						...
						this.change_title = setInterval(function() {
							...
							{
								if (counter++ % 2) {
									document.title = "Beachbody dice..."; // Call Function 1
								} else {
									document.title = "Asistencia en directo"; // Call Function 2
								}
							}
						}, 500);
						try
	...
	checkFocus: function() {
		this.abc =setInterval(function(){
			try
			{
				...
				if(title != "Asistencia en directo" && title != "Beachbody dice...")
				{
					document.title = "Asistencia en directo";
				}
			}

...
Custom.Widgets.ResponsiveDesign.TextInputCOCreditCardUpdate = RightNow.Field.extend({
    overrides: {
        constructor: function(data, instanceID) {
			...
			if(this._fieldName == "zip_code")   
			{
				...
				document.getElementById(zip_field_id).placeholder="Código postal de la dirección de envoi";
    ...
    _displayError: function(errors, errorLocation,email_zip_err_flag) {
        ...
        if(commonErrorDiv) {
			...
			{
				message_err = "<b>Complete el correo electrónico y el código postal";
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			else if(email_zip_err_flag == 2)
			{
				message_err = "<b>Por favor complete el correo";	
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			
			else if(email_zip_err_flag == 3)
			{
				message_err = "<b>Por favor rellene el código postal";	
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			} else {
				message_err="";
			}

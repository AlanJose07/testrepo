...
RightNow.namespace('Custom.Widgets.order_status.TextInputCONew');
Custom.Widgets.order_status.TextInputCONew = RightNow.Field.extend({
    overrides: {
        constructor: function(data, instanceID) {
			...
			if(this._fieldName == "zip_code")
			{
				...
				//added by Vimal on 10/18/2016
				var field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				document.getElementById(field_id).placeholder="Zip/Postal Code";
    ...
    onValidate: function(type, args) {
		...
		if(this._fieldName == 'email')
		{
		  ...
		  */
		 if(zip_code == "" && order_number=="" && email == "")
		 {
			 var email_zip_err_flag = 1;
		 }
		
		// else if((zip_code==""&&email!=""))//Old commented by Vimal on 10/06/2016
		 else if((order_number=="" && email==""))//Enter either order no or email id .Added by Vimal on 10/06/2016
		 {
			var email_zip_err_flag = 2;
		 }
		//xtra condition added by Vimal on 10/06/2016
		else if(zip_code=="")//Zip code is mandatory added by Vimal on 10/06/2016
		 {
			...
			if(!this.validate(errors) || (this.data.attrs.require_validation && !this._validateVerifyField(null, errors)) || !this._compareInputToMask(true)|| email_err_flag==1 ||email_zip_err_flag==1 ||email_zip_err_flag==2 || email_zip_err_flag==3) 
			{
			if(email_err_flag==1)
			errors[errors.length] = "eneterd is invalid";
    ...
    _displayError: function(errors, errorLocation,email_zip_err_flag) {
        ...
        if(commonErrorDiv) {
			...
			{
				message_err = "<b>Please fill Zip Code";	
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}

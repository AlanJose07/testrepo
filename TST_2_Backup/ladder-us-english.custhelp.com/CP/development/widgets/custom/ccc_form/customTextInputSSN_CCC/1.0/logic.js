...
Custom.Widgets.ccc_form.customTextInputSSN_CCC = RightNow.Widgets.TextInput.extend({ 
	...
	{	
	   ...
	   if(document.getElementsByName("Contact.Address.Country")[0].value!="")
		...
		else if(memb_id == 760 && countryid == 2)//Member Type = Coach French    Les 4 derniers chiffres du numéro de carte de crédit
		{	
		...
		else if(memb_id == 760 && countryid == 23)//Member Type = Coach French    Les 4 derniers chiffres du numéro de carte de crédit
		{	
			//this._makeRequired("Les 4 derniers chiffres du numéro de carte de crédit",true,req_fre);
			this._makeRequired("Derniers quatre chiffres de votre carte de crédit enregistrée pour BSF",true,req_france);

...
class SMSChatHours extends \RightNow\Widgets\ChatHours {
    ...
    function getData() {
		 ...
		 //$smslabel = RNCPHP\ROQL::query( "SELECT Value FROM MessageBase WHERE ID = 1000079")->next()->next(); //CUSTOM_MSG_SMS_LABEL
		// print_r($smslabel['Value']);
	...
	/*------------------------------------Facebook Chat Customization Start-------------------------------------------------*/
	 	 ...
	 	 $rmas = RNCPHP\ROQL::queryObject("SELECT CO.CustomChatHours FROM CO.CustomChatHours WHERE CO.CustomChatHours.ID = 13")->next();     
		 $rma = $rmas->next();
	//echo "<pre>";
	//print_r($rma);
	...
	$this->data['array4'] = $enabled;
		...
		//  print_r($chat_count_phone);
		  ...
		  if($chat_count_phone > 0)//if array is not null
				...
				//$this->data['js']['closed']= $smslabel['Value']; 

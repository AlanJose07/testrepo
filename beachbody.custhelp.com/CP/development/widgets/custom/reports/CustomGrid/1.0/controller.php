...
class CustomGrid extends \RightNow\Widgets\Grid {
   ...
   function getData()
    {
        ...
        $this->data['js'] = array(
		 ...
		 foreach($this->data['tableData']['data'] as &$record){
           
            $url = $this->get_string_between($record[1],"href='", "'>");
             $string_param = strip_tags($this->get_string_between($record[1],"'>","</a>"));
             ...
             $record[1] = str_replace($url, $new_url, $record[1]);
        }
    }

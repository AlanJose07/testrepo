    ...
    protected function getMenuItems(){
		...
		if($this->fieldName === 'member_type_new')
		  ...
		  if($this->fieldName === 'Country')
		 {
				...
				if($items)
				{
					...
					foreach ($items as $item)
					{
						if(($item->ID == 1)||($item->ID == 2)||($item->ID == 70))
						{
							//$menuItems[$item->ID] = $item->LookupName ?: $item->Name;
		  ...
		  //added for country dropdown ends
          		  ...
          		  //added for gdpr_us_states dropdown starts
		  if($this->fieldName === 'gdpr_us_states')
          {
                 $field = explode('.', $this->data['inputName']);
                 $object = array_shift($field);
                 $items = Connect::getNamedValues($object, implode('.', $field));
                 ...
                 if($items)
                 {
                     ...
                     foreach ($items as $item)
                     {
                         if(($item->ID == 1964)||($item->ID == 1965)||($item->ID == 1966))
                         {
                             $menuItems[$item->ID] = $item->LookupName ?: $item->Name;
                             //$menuItems[$item->ID] =  $item->Name;
                         }
        ...
        if (!($items = $this->fieldMetaData->named_values)) {
            ...
            /*    if($this->data['value'] !== null){
                ...
                $items = array();
                $stateItems = $this->CI->model('Country')->getAll()->result;
                // we want the full names and not the ISO codes for countries
                foreach ($stateItems as $stateItem)
                    $items[] = (object)array('ID' => $stateItem->ID, 'LookupName' => $stateItem->Name);
        ...
        if($items){
            foreach ($items as $item) {
				...
				if($item->ID == 1505){
                    ...
                    $menuItems[$item->ID] = 'Right to Know/Access  – Categories of Personal Information/Specific Pieces of Information';
                    }
                    else if($item->ID == 1963){
                    //$menuItems[$item->ID] = 'Right of Access/Portability / Right to Know';
                    $menuItems[$item->ID] = 'Right to Opt-Out of Sharing/Opt-Out of Targeted Advertising ';
                     } else if($item->ID == 1962) {
                    //$menuItems[$item->ID] = 'Right of Access/Portability / Right to Know';
                    $menuItems[$item->ID] = 'Data Portability Request/Copy of My Personal Information';
                     } else {
                    $menuItems[$item->ID] = $item->LookupName ?: $item->Name;
            }
        }

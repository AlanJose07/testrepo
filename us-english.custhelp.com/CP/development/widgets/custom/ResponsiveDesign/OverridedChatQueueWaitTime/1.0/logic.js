RightNow.namespace('Custom.Widgets.ResponsiveDesign.OverridedChatQueueWaitTime');
Custom.Widgets.ResponsiveDesign.OverridedChatQueueWaitTime = RightNow.Field.extend({ 
    /**
     * Widget constructor.
     */
	overrides: {	 
		...
		this._queuePositionLabel = this.Y.one(this.baseSelector + "_QueuePositionLabel");
    ...
    _onChatStateChangeResponse: function(type, args)
    {
           ...
           args[0].data.currentState === RightNow.Chat.Model.ChatState.REQUEUED)
        {
            ...
            if(this._displayQueuePosition)
            {
				...
				RightNow.UI.show(this._queuePositionLabel);
            }

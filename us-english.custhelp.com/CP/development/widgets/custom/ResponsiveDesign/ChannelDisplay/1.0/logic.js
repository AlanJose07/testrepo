        ...
        constructor: function(){
			...
			if(FieldName=='Incident.CustomFields.c.recommended_channel')			
				{   
					...
					{
						    console.log(displaychannel_id_only.length);
							var k=1;   
...
-----------------------------------------------------------------------------------------------------------------------------------------------------
 */						
					console.log(encodeURIComponent(this.url_parameter));	   			  
					RightNow.Ajax.makeRequest('/cc/bbresponsivecontroller/loginsession_fb/sms', {text:login_url}, {
                    successHandler: function(response) {
								//var site = actual_link.replace("https://"+window.location.hostname+"/app/contactus_support/"," ");

								if(response['loggedin'] ==  'no')
								{
								window.location = "https://"+this.site_url+"/cc/openlogin/oauth/authorize/beachbody?returnurl=smssurvey/"+encodeURIComponent(this.url_parameter);
								console.log(window.location );
								//window.location = 'https://us-english--tst2.custhelp.com/cc/openlogin/oauth/authorize/beachbody?returnurl=smssurvey/'+encodeURIComponent(this.url_parameter);
								}else{
		 ...
		 */						
							...
							successHandler: function(response) {
										...
										if(response['loggedin'] ==  'no')
										{
										window.location = "https://"+this.site_url+"/cc/openlogin/oauth/authorize/beachbody?returnurl=callmenow/"+encodeURIComponent(this.url_parameter);
...
*/			
			RightNow.Ajax.makeRequest('/cc/bbresponsivecontroller/loginsession', {text:login_url}, {
                    successHandler: function(response) {
								...
								if(response['loggedin'] ==  'no')
								{ 
							    window.location = "https://"+this.site_url+"/cc/openlogin/oauth/authorize/beachbody?returnurl=chat/prechatsurvey/"+encodeURIComponent(this.url_parameter);
								}else{
...
*/			
			RightNow.Ajax.makeRequest('/cc/bbresponsivecontroller/loginsession_fb/facebook', {text:login_url}, {
                    successHandler: function(response) {
								...
								if(response['loggedin'] ==  'no')
								{ 
							        window.location = "https://"+this.site_url+"/cc/openlogin/oauth/authorize/beachbody?returnurl=chat/facebooksurvey/"+encodeURIComponent(this.url_parameter);
								}else{
...
*/
			 ...
			 else if(checked_channel=='1885')//Document 
		      {
						...
						if(this.data.js.isloggedin=="loggedin")
						{
					     var displayBlock =["coachid","language","submit-button","hide-for-email-confirm-popup","file-upload","bsf"];
						var displayNone =["agent-help","phone-us","regular-chat-hours","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available","remove-callus-email","chat-title","phone-title","ask-title","after-submit-email","rn_errorlocation_chat","email-conf-title","email","first-name","last-name","order-no","bike-serial","tablet-id","zipcode","lifetimerank"];
							}
						else
						{
							var displayBlock =["coachid","email","first-name","last-name","language","submit-button","hide-for-email-confirm-popup","file-upload","bsf"];
						var displayNone =["agent-help","phone-us","regular-chat-hours","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available","remove-callus-email","chat-title","phone-title","ask-title","after-submit-email","rn_errorlocation_chat","email-conf-title","order-no","bike-serial","tablet-id","zipcode","lifetimerank"];
						}

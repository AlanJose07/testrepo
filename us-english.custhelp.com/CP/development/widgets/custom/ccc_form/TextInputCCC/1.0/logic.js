...
Custom.Widgets.ccc_form.TextInputCCC = RightNow.Widgets.TextInput.extend({
	...
	validationReset: function (type, args) {
		console.log(args);
		let inputFirstName = document.getElementsByName(`${that.data.attrs.name}`)[0];
	...
	_set_required_attribute: function (type, args) {
		...
		if (membertypeid == 771) { // YES
			if (this.data.js.name == "Incident.CustomFields.c.ccc_transfer_coach_name") {
				...
				this._makeRequired("BODi Partner Name", true, req_eng);

			}
			if (this.data.js.name == "Incident.CustomFields.c.ccc_transfer_coach_email") {
				//this._makeRequired("Coach Email Address",true,req_eng);
				this._makeRequired("BODi Partner Email Address", true, req_eng);

			}
		} else {
			if (this.data.js.name == "Incident.CustomFields.c.ccc_transfer_coach_name")
				//	this._makeRequired("Coach Name",false,req_eng);
				this._makeRequired("Partner Name", false, req_eng);

			if (this.data.js.name == "Incident.CustomFields.c.ccc_transfer_coach_email")
				//	this._makeRequired("Coach Email Address",false,req_eng);
				this._makeRequired("Partner Email Address", false, req_eng);

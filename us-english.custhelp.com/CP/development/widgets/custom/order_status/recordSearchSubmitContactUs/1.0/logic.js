RightNow.namespace('Custom.Widgets.order_status.recordSearchSubmitContactUs');
Custom.Widgets.order_status.recordSearchSubmitContactUs = RightNow.Widgets.FormSubmit.extend({ 
    /**
    ...
    overrides: {   
        ...
        if (!responseObject) { 
           ...
           // this._displayErrorDialog(RightNow.Interface.getMessage('ERROR_REQUEST_ACTION_COMPLETED_MSG'));
		   	document.getElementById("errors").innerHTML ="<div><b> 1. Please enter a valid order number or email and zip/postal code</b></div>";
			...
			document.getElementById("rn_ErrorLocation1").innerHTML ="Please enter a valid email and Zip <br/> <a href='/app/contact_us/lob/team'>Contact us</a> for assistance";
			//alert("errors");
		...
		for(var i=0;i<responseObject.length;i++)
		{
		 ...
		 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Your order "+responseObject[i].order_number+" for "+responseObject[i].product+" has been "+status+". If you"+"'"+"d like to speak to a Beachbody representative, dial (800) 470-7870 for assistance.</div></div>"	
			...
			*/
			...
			heading += "<hr><h1>Click next to continue</h1><br/>"	
	...
	getOrderStatus: function(responseObject) {
		...
		if(document.getElementById("heading"))
		{
		   ...
		   if(responseObject.order_status_id == 2 || responseObject.order_status_id == 6)
			...
			var status = "cancelled";	
		document.getElementById("valid_data_ccu").innerHTML ="<div class='shipped_order'><div>Your order "+responseObject.order_number+" for "+responseObject.product+" has been "+status+". If you"+"'"+"d like to speak to a Beachbody representative, dial (800) 470-7870 for assistance.</div></div>"	
          ...
          // track_id="";
			...
			else if(responseObject.tracking_number.length>=12 && responseObject.tracking_number.substring(0,1)=='7')
		   {
			...
			document.getElementById("valid_data_ccu").innerHTML ="<div class='shipped_order'><div class='order_top'>Your order "+responseObject.order_number+" for "+responseObject.product+" placed on "+responseObject.order_create_date +", shows that it "+status+" on "+responseObject.shipped_date +".</div><div class='track-div'>Your tracking number is</div><div><a href="+href+" target='_blank' >"+responseObject.tracking_number+"</a></div><div class='new-modi'>"+footer_message+"</div></div>";

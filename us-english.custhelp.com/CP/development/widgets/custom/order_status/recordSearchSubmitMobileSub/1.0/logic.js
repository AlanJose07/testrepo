RightNow.namespace('Custom.Widgets.order_status.recordSearchSubmitMobileSub');
Custom.Widgets.order_status.recordSearchSubmitMobileSub = RightNow.Widgets.FormSubmit.extend({ 
    /**
    ...
    overrides: {   
		...
		//LOB NOT OBEYED issue fix - start
		var lob = "team";
		var url = window.location.href;
		var url_split = url.split('/');
		var index=url_split.indexOf("lob");
		lob = url_split[index+1];
		
		//LOB NOT OBEYED issue fix - end
        ...
        if (!responseObject) { 
           ...
           // this._displayErrorDialog(RightNow.Interface.getMessage('ERROR_REQUEST_ACTION_COMPLETED_MSG'));
		   	document.getElementById("errors").innerHTML ="<div><b> 1. Please enter a valid order number or email and zip/post/postal code</b></div>";
			if(responseObject.display_chat == 1)
			{
			document.getElementById("errors").innerHTML +=	"<div><b><a href='/app/chat/chat_launch/lob/"+lob+"' target='_blank' >Chat</a> <span>for assistance</span></b></div>";
			}
        }
		   
		 
        else if (responseObject.errors) {
        	document.getElementById("rn_ErrorLocation1Sub").style.display="block";
			//document.getElementById("rn_ErrorLocation1Sub").innerHTML ="Please enter a valid email and Zip <br/> <a href='https://faq.beachbody.com/app/chat/chat_launch/lob/"+lob+"'>Contact us</a> for assistance";//CONTACT US TO CHAT
			//alert("errors");
			
			document.getElementById("rn_ErrorLocation1Sub").innerHTML ="<span style='color:red'>We were unable to locate your account with the information below, please call us to update your credit card.<br/>US and Canada: 800-470-7870 UK: 0800-953-6100</span>";
			
			document.getElementById("errors").style.display="block";
			//document.getElementById("errors").innerHTML ="<div><b>2. Please enter a valid order number or email and zip/postal code</b></div>";
			if(responseObject.display_chat == 1)
			{
			document.getElementById("errors").innerHTML +=	"<div><b><a href='/app/chat/chat_launch/lob/"+lob+"' target='_blank'>Chat</a> <span>for assistance</span></b></div>";
			}
			...
			document.getElementById("rn_QuestionSubmit1Sub").style.display = "none";
		 ...
		 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Your order "+responseObject[i].order_number+" for "+responseObject[i].product+" has been "+status+". If you"+"'"+"d like to speak to a Beachbody representative, dial (800) 470-7870 for assistance.</div></div>"	
			...
			*/
			...
			if((track_id=="" || track_id== null) && (diffDays > 6 ))//different popup content
			{
				 ...
				 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div>We"+"'"+"ve noticed your order "+responseObject[i].order_number+" for "+responseObject[i].product+" placed on "+responseObject[i].order_create_date+" has not shipped.</div> <div> <a href='/app/chat/chat_launch/lob/"+lob+"' target='_blank'>Chat</a> for assistance.</div></div>";
			...
			heading += "<hr class='clearfix'><h1>Click next to continue</h1><br/>"	
			//document.getElementById("heading").innerHTML = "<p>test</p>";
			$("div #heading").empty();
			$("div #heading").html("We found your account. Here are your recent orders<hr/>");
			heading+= '<button id="next" class="btn btn-primary" onclick="javascript:redirectNextPageMobileSub()" >Next</button>';	 
			document.getElementById("valid_data_ccuMobileSub").innerHTML = heading;	
	...
	getOrderStatus: function(responseObject) {
		...
		if(document.getElementById("heading"))
		{
		   ...
		   if(responseObject.order_status_id == 2 || responseObject.order_status_id == 6)
			...
			//var status = "cancelled";	
			//document.getElementById("valid_data_ccuMobileSub").innerHTML ="<div class='shipped_order'><div>Your order "+responseObject.order_number+" for "+responseObject.product+" has been "+status+". If you"+"'"+"d like to speak to a Beachbody representative, dial (800) 470-7870 for assistance.</div></div>"	
			var status = "cancelled";	
          ...
          // track_id="";
			if( (track_id=="" || track_id== null) && (diffDays > 6 ) )//different popup content
			{
				...
				heading="<div class='shipped_order'><div>We"+"'"+"ve noticed your order "+responseObject.order_number+" for "+responseObject.product+" placed on "+responseObject.order_create_date+" has not shipped.</div><div> <a href='/app/chat/chat_launch/lob/"+lob+"' target='_blank'>Chat</a> for assistance.</div>";
			...
			else if(responseObject.tracking_number.length>=12 && responseObject.tracking_number.substring(0,1)=='7')
		   {
			...
			//document.getElementById("valid_data_ccuMobileSub").innerHTML ="<div class='shipped_order'><div class='order_top'>Your order "+responseObject.order_number+" for "+responseObject.product+" placed on "+responseObject.order_create_date +", shows that it "+status+" on "+responseObject.shipped_date +".</div><div class='track-div'>Your tracking number is</div><div><a href="+href+" target='_blank' >"+responseObject.tracking_number+"</a></div><div class='new-modi'>"+footer_message+"</div></div>";

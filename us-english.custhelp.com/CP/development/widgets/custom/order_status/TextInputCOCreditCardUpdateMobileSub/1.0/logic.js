...
RightNow.namespace('Custom.Widgets.order_status.TextInputCOCreditCardUpdateMobileSub');
Custom.Widgets.order_status.TextInputCOCreditCardUpdateMobileSub = RightNow.Field.extend({
    overrides: {
        constructor: function(data, instanceID) { 
			...
			if(this._fieldName == "email")
			{
				...
				var email_field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				document.getElementById(email_field_id).placeholder="Email";
			...
			if(this._fieldName == "zip_code")
			{
				...
				var zip_field_id = "rn_" + this.instanceID + "_CO.order_tracking_new"+"."+this._fieldName;
				document.getElementById(zip_field_id).placeholder="Zip / Post / Postal code of Shipping Address";
    ...
    onValidate: function(type, args) {
		...
		//console.log("email " + email_err_flag);
		//console.log("zip " + email_zip_err_flag);
		if(!this.validate(errors) || (this.data.attrs.require_validation && !this._validateVerifyField(null, errors)) || !this._compareInputToMask(true)||email_zip_err_flag==1 ||email_zip_err_flag==2 || email_zip_err_flag==3) 
		{
			if(email_err_flag == 1)
        	...
        	if(email_err_flag==1){
				...
				document.getElementById("rn_ErrorLocation1Sub").style.display = "none";

			}
        
        }
        /*if (email_zip_err_flag  == 0 && email_err_flag == 0) {
        	document.getElementById("rn_ErrorLocation1").style.display = "none";
        } */
    ...
    _displayError: function(errors, errorLocation,email_zip_err_flag) {
        ...
        if(commonErrorDiv) {
            for(var i = 0, errorString = "", message, id = this.input.get("id"); i < errors.length; i++) {
                ...
                errorString += "<div><b><a href='javascript:void(0);' style='color:red' onclick='document.getElementById(\"" + id +
                    "\").focus(); return false;'><span style='color:#e51e1e'>" + message + "</span></a></b></div> ";
            }
			...
			{
				message_err = "<b><span style='color: red'>Please fill Email and Zip / Post / Postal Code</span>";
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			else if(email_zip_err_flag == 2)
			{
				message_err = "<b><span style='color: red'>Please fill Email</span>";	
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			}
			
			else if(email_zip_err_flag == 3)
			{
				message_err = "<b><span style='color: red'>Please fill Zip / Post / Postal Code</span>";	
				document.getElementsByName("CO.order_tracking_new.zip_code")[0].className='rn_Text rn_ErrorField';
			} else {
				message_err="";
			}

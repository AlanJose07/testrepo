...
$authtoken = $this->model('custom/bbresponsive')->token_process_new($a,$incidentid);
//$authtoken_gen = $this->model('custom/bbresponsive')->token_process_new();

$tok = base64_decode($authtoken);
$token_array = preg_split("/[,]/", $tok);
$arrlength=count($token_array);

for($x=0;$x<$arrlength;$x++)
  {
  $check = $token_array[$x];
  if(stristr($check,"tenantId")){
  $check1 = preg_split("/[:]/", (stristr($check,"tenantId")));
  $tenant_id = trim($check1[1], '"');
  }
  }
//print_r ($authtoken);
echo "<br> Tenant ID <br>".$tenant_id."<br>";	
$tenant_tok = $this->model('custom/bbresponsive')->tenant_api_new($tenant_id);
echo "<br> Api Endpoint <br>".$tenant_tok."<br>";	

//exit;
//$skillID = 10483214; //temp
...
if(!empty($skillID_url)){
  ...
  $skillID =  \RightNow\Utils\Config::getConfig(1000080); //CUSTOM_CFG_NICE_CHAT_POC
}
//print_r("skill");
//print_r ($skillID);
//print_r ($skillID_url);
...
$url_script = "$tenant_tok/incontactapi/services/v25.0/scripts/search?scriptName=$scriptname&includeInactive=false";
				echo "<br> URL to fetch the script details <br>".$url_script."<br>";		

curl_setopt_array($ch, array(
  CURLOPT_URL => "$tenant_tok/incontactapi/services/v25.0/scripts/search?scriptName=$scriptname&includeInactive=false",
...
//$urlcall ="https://api-na1.niceincontact.com/inContactAPI/services/v25.0/scripts/$scripdetails/start?skillId=$skillID";
$urlcall ="$tenant_tok/inContactAPI/services/v25.0/scripts/$scripdetails/start?skillId=$skillID";
$postParameter = ["parameters" => $phnumber. '|' . $incidentid. '|' .$skillID. '|' .$coachdate. '|' .$interface];	
//print_r($postParameter);
				echo "<br> URL to make final NICE call <br>".$urlcall."<br>";		
...
//$mail = $this->model('custom/bbresponsive')->sendMail($name,$SixDigitRandomNum,$Email);

        ...
        if($smartAssist === true){
		  ...
		  /*die('-------------------------------1-------------------------------------');*/
		   ...
		   /*die('-------------------------------2-------------------------------------');*/
            ...
            $resolution->ID = 4; //***Abandoned***KF_API_SA_RESOLUTION_TYPE_ESCALATED Change made by anoop from value 3 to value 1 to correct questions avoided report
          ...
          //  Hooks::callHook('pre_register_smart_assistant_resolution', $hookData);
			...
			//print_r($hookData);
			//echo "<br>";
			//echo($smartAssistantToken);
			//echo "<br>";
		//	print_r($resolution);
			//echo "<br>";
			
			
           // if ($hookData['shouldRegister']){ commented by anoop since there is no data from hooks
...
/*Comment end*/
  ...
  print_r($keyValueList);*/
	...
	//	die('------------------------------------------');	
			...
			/*$results['canEscalate'] = 0;*/
            $results['token'] = $smartAssistantSuggestions->Token;
		/*	echo"<pre>";
			print_r($smartAssistantSuggestions->Suggestions);
			die('---------------------------------------------------');*/
		...
		
		{
			 $ansobj=Connect\Answer::fetch($results[suggestions][0]['list'][$i]['ID']);    
				...
				foreach($accesslvl as $items) {
				 ...
				 if($ansobj->CustomFields->c->$arrtab==1 && $cnt<100 && $item != null)
             {
			 
			 	
				$lob_filtered_res['suggestions'][0]['type']= $results['suggestions'][0]['type'];
				$lob_filtered_res['canEscalate']= $results['canEscalate'];
	  ...
	  //If there are no results and the user can escalate the post, send back whether or not we ran rules
            ...
            if(is_null($lob_filtered_res)){
				...
				$lob_filtered_res['canEscalate'] = $smartAssistantSuggestions->CanEscalate;

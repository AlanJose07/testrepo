 ...
 */
class Field_beachbody_custom extends \RightNow\Models\Field{
     ...
     */ 
    public function sendForm($formData,$FName, $LName, $phones_detail,$country_detail, $state_detail, $thread_text, $thread_text2, $listOfUpdateIDs = array(), $smartAssistant = false) {
        if ($tokenError = $this->verifyFormToken()) return $tokenError;
		...
		$incident = RNCPHP\Incident::fetch($result->result->ID);
		
		
		
		//print_r(count($contactDetail->Phones));
		...
		$incident->Threads[0]->Text = $thread_text;
		if($thread_text2 != ""){
			...
			$incident->Threads[1]->Text = $thread_text2;
		}
        if($state_detail != ""){
            $incident->CustomFields->c->gdpr_us_states = $state_detail;
        }
		$incident->save();
		
		
		
		$contactDetail = RNCPHP\Contact::fetch($incident->PrimaryContact->ID);
		
		//added by sriram for contact first and last name starts
		if($contactDetail->Name->First =="" && $contactDetail ->Name->Last =="")
		{
			$contactDetail->Name->First = $FName;
			$contactDetail->Name->Last = $LName;
		}else{
			
			if($contactDetail->Name->First =="")
			{
				$contactDetail->Name->First = $FName;
			}
			if($contactDetail->Name->Last =="")
			{
				$contactDetail->Name->Last = $LName;
		...
		//added by sriram for contact first and last name ends
		if($contactDetail ->Address->Country){
			if($country_detail== 1){
				$contactDetail->Address->Country = RNCPHP\Country::fetch(1);
			}
			else if($country_detail== 2){
				$contactDetail->Address->Country = RNCPHP\Country::fetch(2);
			}else if($country_detail== 70){
				$contactDetail->Address->Country = RNCPHP\Country::fetch(70);
			}
		...
		for($i=0; $i<count($contactDetail->Phones); $i++)
		{
			
			if($contactDetail->Phones[$i]->PhoneType->LookupName == 'Mobile Phone')
			{
				$countMobPhone++;
				if(!empty($phones_detail))
				{
					/*$contactDetail->Phones[$i]->PhoneType = new RNCPHP\NamedIDOptList();*/
					//$contactDetail->Phones[$i]->Number = 666666666666666666;\
					
					
					$contactDetail->Phones[$i] = new RNCPHP\Phone();
		            $contactDetail->Phones[$i]->PhoneType = new RNCPHP\NamedIDOptList();
		            $contactDetail->Phones[$i]->PhoneType->LookupName = 'Mobile Phone';
		            $contactDetail->Phones[$i]->Number =$phones_detail;
		...
		if($countMobPhone == 0 && $phones_detail!=NULL)
		{
		
			$totalPhones = count($contactDetail->Phones);
			
			//print_r($totalPhones);
			//print_r($countMobPhone->Phones[$totalPhones]->Number);

			//exit;
			
			$contactDetail->Phones[$totalPhones] = new RNCPHP\Phone();
            $contactDetail->Phones[$totalPhones]->PhoneType = new RNCPHP\NamedIDOptList();
            $contactDetail->Phones[$totalPhones]->PhoneType->LookupName = 'Mobile Phone';
            $contactDetail->Phones[$totalPhones]->Number = $phones_detail;
			$contactDetail->save();
		}

...

{
	...
	*/
	...
	{
						$opa=\RightNow\Utils\Config::getConfig(1000084); //CUSTOM_CFG_OPA_URL
        ...
        $url = \RightNow\Utils\Config::getConfig(1000085); //CUSTOM_CFG_GET_CUSTOMER_ORDERS
        $getCustomerOrders_key=\RightNow\Utils\Config::getConfig(1000068); //CUSTOM_CFG_AWS_API_KEY
        
		$ch = curl_init($url);
		//post call
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
													'api-key: AGENT:US:'.$guid,
													'x-api-key:'.$getCustomerOrders_key,
	...
	*/
...

	{
		...
		$add_url=\RightNow\Utils\Config::getConfig(1000086); //CUSTOM_CFG_ADDRESS_VALIDATION_URL
		$address_validation_key=\RightNow\Utils\Config::getConfig(1000068); //CUSTOM_CFG_AWS_API_KEY
        $url =$add_url.$_SERVER['argv'][0];
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
													...
													'x-api-key:'.$address_validation_key,
													'Content-Type: application/json'));
        curl_setopt($ch, CURLOPT_TIMEOUT, 30);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
	...
	*/
...

	{
		...
		$apiPassword=\RightNow\Utils\Config::getConfig(1000043);
				load_curl();
        $url=\RightNow\Utils\Config::getConfig(1000087); //CUSTOM_CFG_GET_ALL_ITEMS_FOR_COUNTRY
	...
	*/
...

	{
		...
		{		
				...
				logmessage($session_count);
				$soapUser="uadstageuser";
				$soapPassword="Beachbody123";	
				...
				//load_curl(); 
				$url=\RightNow\Utils\Config::getConfig(1000088); //CUSTOM_CFG_ORDER_IMPORT_URL
				//$url = 'https://transportpsuat.beachbody.com/api/soap/orderimport';
				$ch = curl_init($url);
...

	{
		...
		if (\RightNow\Utils\Framework::isLoggedIn()) 		
		{
			...
			if($_GET["GUID"] === $guid){
				...
				$tbb_url=\RightNow\Utils\Config::getConfig(1000089); //CUSTOM_CFG_TBB
					$url = $tbb_url."GUID=".$_GET["GUID"];

...

{
	   ...
	   public function sendPlacementForm()
		{
			...
			$ps_coach_new_position = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Incident", "customFields.c.ps_coach_new_position");
			$leg_placement = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Incident", "customFields.c.leg_placement");
			foreach($data as $each_data)
			{
 ...
 public function sendChatForm()
	...
	function track_shk_pop_up_clicks($url,$mob,$prev_url)
	{
	
	  $link = urldecode($url);  
	  $prevlink=urldecode($prev_url);  
      $CI = get_instance();
	  $CI->model('custom/Answer_hit_model')->track_shk_pop_up_clicks($link,$mob,$prevlink);
...
$this->_renderJSON(array('errors' =>'error','display_chat' => $display_chat));
    ...
    $result = $this->model('custom/Answer_hit_model')->getOrderStatus($data,$link,$mob);//old one data read from CO        
...
$this->_renderJSON(array('errors' =>'error','display_chat' => $display_chat));
	...
	//===get life time rank based on the coach id
	
	function getLifeTimeRank()
	{
	$coach_Id = $this->input->post('coach_id');
	
		$result = $this->model('custom/IncidentModel')->get_lifetime_rank($coach_Id);	 
		/* var_dump($result);
	...
	//====added  for new form=====
	    ...
	    $results = $this->model('custom/CustomOrder')->shakecountrybasedlanguage($id); 
	    ...
	    public function sendFormGDPR()
		{
			...
			$thread_text = "Consumer Privacy Rights Request Form \n\n";
			foreach ($data as $items)
			{
				if(strpos($items->name,"gdpr_us_states"))
				{
					$mem = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Incident", "customFields.c.gdpr_us_states"); 
					if($items->value){
					foreach($mem as $each_mem)
					{
						...
						{
							$state_detail = $each_mem->ID;
							$thread_text .= "State Of Residence: ".$each_mem->LookupName."\n";
						}
				...
				{
					$mem = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Incident", "customFields.c.member_type_new"); 
					foreach($mem as $each_mem)
					{
						$mem_id = intval($items->value);
						if($each_mem->ID == $mem_id)
				...
				{
					$object_data1 = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Contact", "Address.Country"); 
					foreach($object_data1 as $each_mem1)
					{
						...
						if($each_mem1->ID == $obj_data_id1)
						{
							...
							if($each_mem1->ID ==70){
								$country_detail = $each_mem1->ID;
								$thread_text .= "Country: European Union\n";
				...
				{
					$right = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Incident", "customFields.c.dsrm_right_type"); 
					foreach($right as $each_mem)
					{
						...
						//	$thread_text .= "Data Request: ".$each_mem->LookupName."\n";
							if($each_mem->ID == 1505){
								$thread_text .= "Data Request: Right to Know/Access  – Categories of Personal Information/Specific Pieces of Information \n";
							}
							else if($each_mem->ID == 1963){
								$thread_text .= "Data Request: Right to Opt-Out of Sharing/Opt-Out of Targeted Advertising \n";
							}
							else if($each_mem->ID == 1962){
								$thread_text .= "Data Request: Data Portability Request/Copy of My Personal Information \n";
							} else {
								$thread_text .= "Data Request: ".$each_mem->LookupName."\n";
							}
						}
				...
				{
					$object_data = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_4\\Incident", "customFields.c.dsrm_object_data"); 
			...
			foreach ($data as $items)
			{
				if(strpos($items->name,"search_first_name"))
				{
					if($items->value)
					{
				...
				if(strpos($items->name,"search_last_name"))
				{
					if($items->value)
					{
					$thread_text .= "Last Name: ".$items->value."\n";
				...
				if(strpos($items->name,"search_email"))
				{
					if($items->value)
					{
					$thread_text .= "Email Address: ".$items->value."\n";
			...
			$result = $this->_echoJSON($this->model('custom/Field_beachbody_custom')->sendForm($data, $FName, $LName, $phones_detail,$country_detail, $state_detail, $thread_text, $thread_text2,$listOfUpdateRecordIDs ?: array(), ($smartAssistant === 'true'))->toJson());

...
class bbresponsive extends \RightNow\Models\Base
{
	...
	{
		try {
			// call to contact model is not required as did in US english interface since sms is only for US and CA
			$res = RNCPHP\ROQL::queryObject($query)->next();
			$i = 0;
			...
			$data = RNCPHP\ROQL::query("SELECT ID, Value FROM MessageBase WHERE ID in(1000060,1000061,1000062,1000063,1000064,1000069,1000117)")->next();
			...
			if ($res->count() > 0) {
				while ($channel = $res->next()) {
					...
					if ($channel->Directly_RTM->ID == 3 || $channel->Agent_Chat->ID == 3 || $channel->Email->ID == 3 || $channel->Phone->ID == 3 || $channel->Self_Service_Form->ID == 3 || $channel->facebook->ID == 3 || $channel->Doc->ID == 3) {
			...
			if (is_numeric($category_id)) {
				...
				if ($catParentIdFirst != "") {
					...
					$enable = "SELECT bbb.enable 
												   FROM BB.categ_contac_hom_uk bbb 
	...
	function prechatsurveydata($data)
	{

		//print_r($data);
		$log = \RightNow\Utils\Config::getConfig(1000054);  //CUSTOM_CFG_BLANK_CHAT_LOG

		if ($log) {
			$error_log = new RNCPHP\ORN_NiceLog\Nice_Error_Log();
			$error_log->ReportName = "Prechat";
			$error_log->LogTime = "UK Interface";
			$error_log->Remarks = $data;
	...
	function ghostchatsubmit($data)
	{

		$log = \RightNow\Utils\Config::getConfig(1000054);  //CUSTOM_CFG_BLANK_CHAT_LOG
		if ($log) {
			$error_log = new RNCPHP\ORN_NiceLog\Nice_Error_Log();
			$error_log->ReportName = "SubmitLogic";
			$error_log->LogTime = "UK Interface";
			if ($data != " ")
				$error_log->Remarks = $data;
			else
				$error_log->Remarks = $_SERVER['HTTP_USER_AGENT'];
			$contact = get_instance()->model('Contact')->get()->result;
			$error_log->Incident = $contact->Emails[0]->Address;
			$error_log->save(RNCPHP\RNObject::SuppressAll);
			return 0;
	...
	function remove_bookmarked_url($category_id, $page)
	{
		try {
			if ($page == "contactus_support" || $page == "answers") {
				...
				if ($page == "answers") {
					$baseQuery = "Select answer_report_id from BB.categ_contac_hom_uk where BB.categ_contac_hom_uk.category.ID =" . $category_id . " and BB.categ_contac_hom_uk.visible_on_page.ID != 3 and BB.categ_contac_hom_uk.visible_on_page.ID != 4";
				}

				if ($page == "contactus_support") {
					$baseQuery = "Select answer_report_id from BB.categ_contac_hom_uk where BB.categ_contac_hom_uk.category.ID =" . $category_id . " and BB.categ_contac_hom_uk.visible_on_page.ID != 1 and BB.categ_contac_hom_uk.visible_on_page.ID != 2";
				}
				if (is_numeric($category_id)) {
					$pro = RNCPHP\ROQL::query($baseQuery)->next();
					$product = $pro->next();
	...
	function setchatskill($catid, $membertypenew)
	{
		...
		try {
			if (\RightNow\Utils\Framework::isLoggedIn()) {
				...
				$logged_in = "no";
			$query = "select BB.categ_contac_hom_uk from BB.categ_contac_hom_uk where BB.categ_contac_hom_uk.category.ID=" . $catid . " and BB.categ_contac_hom_uk.interface.ID=3 and BB.categ_contac_hom_uk.visible_on_page.ID in(3,4)";
			$res = RNCPHP\ROQL::queryObject($query)->next();
			if ($res->count() > 0) {
				while ($channel = $res->next()) {

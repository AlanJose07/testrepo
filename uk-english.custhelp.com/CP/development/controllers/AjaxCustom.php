...
$this->_renderJSON(array('errors' =>'error','display_chat' => $display_chat));
 ...
 else if(count($result)==1)
 {
	...
	$sku_desc=$result[0]['sku_descriptions'];
  ...
  $data = array('first_name' => $result[0]['first_name'],'last_name' => $result[0]['last_name'],'order_number' => $result[0]['order_number'],'order_status_id' => $result[0]['order_status']->ID,'order_status_lookup' => $result[0]['order_status']->LookupName,'shipped_date' => gmdate("m/d/Y", $result[0]['shipped_date']),'tracking_number' => $result[0]['tracking_number'],'order_create_date' =>gmdate("m/d/Y", $result[0]['order_create_date']),'product'=>$sku_desc,'sms_enabled'=>$result[0]['sms_subscription_enabled'],'zip_code' => $result[0]['zip_code']);

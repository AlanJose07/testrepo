RightNow.namespace('Custom.Widgets.input.SelectionInputRadioButton');
Custom.Widgets.input.SelectionInputRadioButton = RightNow.Field.extend({ 
    /**
     * Widget constructor.
     */
   overrides: {       
       ...
       constructor: function() {
			...
			*/
			...
			if((FieldName == "Incident.CustomFields.c.ccf_reason_for_cancellation")||(FieldName == "Incident.CustomFields.c.ccf_reason_for_cancellation_fr")||(FieldName == "Incident.CustomFields.c.ccf_reason_for_cancellation_sp"))
				{
					this.input.on("click", this.getReasonForCancellation, this);//Checks if click event occurs for Reason For Cancellation field
    ...
    onValidate: function(type, args) {
		var reasons = document.getElementsByName(this.data.js.name);
				var reasons_value;
				for(var i = 0; i < reasons.length; i++){
				if(reasons[i].checked){
				reasons_value = reasons[i].value;
				}
			}
    ...
    displayError: function(errors, errorLocation) {
        ...
        if(commonErrorDiv) {
            for(var id = ((this.input instanceof this.Y.NodeList) ? this.input.item(0).get("id") : this.input.get("id")),
                ...
                message = errors[i];
				if(this.data.attrs.label_error)
				{
                message = (message.indexOf("%s") > -1) ? RightNow.Text.sprintf(message, this.data.attrs.label_error) : this.data.attrs.label_error + " " + message;
				}
				else
				{
    ...
    /**
     * Gets the value selected for reason for cancellation and
	 * fires an event
     */
	 getReasonForCancellation: function() {
			 ...
			 var eventObject = new RightNow.Event.EventObject(this, {data: {other_reason: rate_value}});
        	RightNow.Event.fire("evt_getreasonforcancellation", eventObject); 	
			//subcribed in custom/input/TextInput_Other

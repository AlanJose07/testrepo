RightNow.namespace('Custom.Widgets.order_status.recordSearchSubmit');
Custom.Widgets.order_status.recordSearchSubmit = RightNow.Widgets.FormSubmit.extend({ 
    /**
    ...
    overrides: {   
        ...
        if (!responseObject) {
           ...
           // this._displayErrorDialog(RightNow.Interface.getMessage('ERROR_REQUEST_ACTION_COMPLETED_MSG'));
			...
			//document.getElementById("rn_ErrorLocation1").innerHTML ="Please enter a valid email and Zip <br/> <a href='https://faq.beachbody.com/app/contact_us/lob/team'>Contact us</a> for assistance";
			//alert("errors");
						
			document.getElementById("rn_ErrorLocation1").innerHTML ="<span>We were unable to locate your account with the information below, please call us to update your credit card.<br/>US and Canada: 800-470-7870 UK: 0800-953-6100</span>";
			...
			document.getElementById("rn_QuestionSubmit1").style.display = "none";
		...
		for(var i=0;i<responseObject.length;i++)
		{
		 ...
		 heading +="<div class='multiple_order' id='block_"+i+"'style='display:none;'><div> Your order "+responseObject[i].order_number+" for "+responseObject[i].product+" has been "+status+". If you"+"'"+"d like to speak to a Beachbody representative, dial (800) 470-7870 for assistance.</div></div>"	
			...
			*/
			...
			heading+= '<button id="next" class="btn btn-primary" onclick="javascript:redirectNextPage()" >Next</button>';	 
			document.getElementById("valid_data_ccu").innerHTML = heading;	
		...
		if(document.getElementById("heading"))
		{
          ...
          // track_id="";
			if( (track_id=="" || track_id== null) && (diffDays > 6 ) )//different popup content
			{
				...
				/*
				document.getElementById("valid_data_ccu").innerHTML ="<div class='shipped_order'><div>We"+"'"+"ve noticed your order "+responseObject.order_number+" for "+responseObject.product+" placed on "+responseObject.order_create_date+" has not shipped.</div><div> <a href='/app/chat/chat_launch/lob/team' target='_blank'>Chat</a> for assistance.</div>";
				*/
				
				heading ="<div class='shipped_order'><div>We"+"'"+"ve noticed your order "+responseObject.order_number+" for "+responseObject.product+" placed on "+responseObject.order_create_date+" has not shipped.</div><div> <a href='/app/chat/chat_launch/lob/team' target='_blank'>Chat</a> for assistance.</div>";
			}
			else//old popup content
			{
			
			/*
			document.getElementById("valid_data_ccu").innerHTML ="<div class='shipped_order'><div>The order "+responseObject.order_number+" you placed on "+responseObject.order_create_date+" for "+responseObject.product+" was received and it is "+status+".</div><div>What's next? You'll receive a shipping confirmation email with tracking details.</div><div class='new-modi most-order'>Make sure to check your junk and spam folders if unable to find our emails.</div></div>";
			*/

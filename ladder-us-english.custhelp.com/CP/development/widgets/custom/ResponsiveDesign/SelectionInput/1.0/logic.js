...
Custom.Widgets.ResponsiveDesign.SelectionInput = RightNow.Widgets.SelectionInput.extend({ 
	...
	diamond_chat_hour_details: function(type, args)
	{
		...
		{
			if(this.top_parent[0]!=='1706' || ((this.top_parent[0]=='1706') && (membertype=='388')))// not bblive instructor
				{
				...
				var displayNone =["language","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available","bblive-chat-hours","diamond-chat-hours"];
			   ...
			   else
			   {
					...
					var displayNone =["diamond-chat-hours","language"];  
					this._displayBlock(displayBlock);
					this._displayNone(displayNone);
		...
		else if(channel=="1530" || channel=="1532")
		{
				...
				RightNow.Event.fire("make_remove_required_language_based_on_lifetimerank",eo);
				var hide_chat_hour =["regular-chat-hours","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available","diamond-chat-hours","bblive-chat-hours","bblive-chat-status"];   

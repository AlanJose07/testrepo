RightNow.namespace('Custom.Widgets.ResponsiveDesign.AnswerFeedback');
Custom.Widgets.ResponsiveDesign.AnswerFeedback = RightNow.Widgets.extend({ 
     /**
     * Widget constructor.
     */
    constructor: function() {
     ...
     /*   if (!this.Y.one(this.baseSelector + "_FeedbackTextarea")) {
		...
		//RightNow.Event.subscribe("evt_answerFeedbackRequest", this._appendData, this);
	...
	/*  _onClickNo: function(event, rating)
    {
		//alert("on click");
        this._rate = rating;
        this._submitAnswerRating();
...
//alert("after no ---");
    ...
    _showDialog: function()
    {
        ...
        if (!this._dialog)
        {
            ...
            this._dialog = RightNow.UI.Dialog.actionDialog(this.data.attrs.label_dialog_title, dialogForm, {"buttons" : buttons, "close" :  false,  "dialogDescription" : this.baseDomID + "_DialogDescription", "width" : this.data.attrs.dialog_width || ''});
    ...
    */
    ...
    _validateDialogData: function()
    {
		...
		if(document.getElementsByName('reason'))
		{
			...
			this._addErrorMessage(RightNow.Text.sprintf("Please select the reason",''));
			
			/*if(returnValue == true && document.getElementsByName('add_feedback')[0].value=="")
			{
			this._addErrorMessage(RightNow.Text.sprintf("Please select the Additional Feedback",''));   // as per the requirement no need to make require the additional feedback
				returnValue = false;
			}*/
    ...
    _closeDialog: function(cancelled)
    {
        ...
        if (this._dialog)
		{
		   ...
		   document.getElementsByClassName("yui3-widget-mask")[0].style.display="none";  
    ...
    _submitFeedback: function()
    {
		...
		var additionalfeedback= document.getElementsByName('add_feedback');
		var reason_selected;
		for(var i = 0; i < radios.length; i++){
			if(radios[i].checked){
				reason_selected = radios[i].value;     
			}
		}
        ...
        var eventObject = new RightNow.Event.EventObject(this, {data: {
				...
				//lob: document.getElementById('lob').value,
				addition_feedback: additionalfeedback[0].value
        }});
		//alert(this._rate);
        if(RightNow.Event.fire("evt_answerFeedbackRequest", eventObject)){
            RightNow.Ajax.makeRequest(this.data.attrs.submit_feedback_ajax, eventObject.data, {successHandler: this._onResponseReceived, scope: this, data: eventObject, json: true});
    ...
    _onResponseReceived: function(response, originalEventObj)
    {
		...
		*/
		this._fbId=response['Feedback Tracking ID'];
    ...
    */},
    ...
    _submitAnswerRating: function()
    {
		...
		//alert(document.getElementById('lob').value);
        var eventObject = new RightNow.Event.EventObject(this, {data: {
                a_id: this.data.js.answerID,
                rate: this._rate,
                options_count: this.data.attrs.options_count,
		  ...
		  var eventObjectYes = new RightNow.Event.EventObject(this, {data: {
                a_id: this.data.js.answerID,
                rate: this._rate,
                options_count: this.data.attrs.options_count,
				//lob: document.getElementById('lob').value,
        ...
        if(RightNow.Event.fire("evt_answerRatingRequest", eventObject)){
			...
			if(this._rate==2)
			{
			  RightNow.Ajax.makeRequest(this.data.attrs.submit_feedback_ajax, eventObjectYes.data, {successHandler: this._onResponseReceived, scope: this, data: eventObjectYes, json: true});
    ...
    _replaceRatingElementsWithMessage: function(){
        ...
        if(ratingElement){
            ...
            successMessage.set('innerHTML', this.data.attrs.label_feedback_thanks).set('tabIndex', -1);
    ...
    */
    ...
    _onFormTokenUpdate: function(type, args) {
	 ...
	 _appendData: function(type, args) {},

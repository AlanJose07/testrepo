...
class AjaxOrder extends \RightNow\Controllers\Base
{
		...
		foreach($data as $item)
		{
			if($item->name == "Incident.CustomFields.c.new_shipping_date")
			{
				if($item->value)
				{
					...
					$shipping_date_list = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\Incident.CustomFields.c.new_shipping_date");
	...
	function sendorderform_customer_coach()
	{
	 try{
		...
		} // when it is umcommented please check whether this model exist in all the interfaces
		echo json_encode($response);
		...
		error_log($message,3, '/cgi-bin/us_english.cfg/scripts/cp/customer/development/views/pages/error_log/exceptioncoachlog.txt');
	  }
	}
}

...
<? if(($item["DirectlyRTM"]=="null") and ($item["SelfServiceForm"]=="null") and ($item["AgentChat"]=="null") and ($item["Email"]=="null") and ($item["Phone"]=="null") and ($item["SMS"]=="null") and ($item["Doc"]=="null") and ($item["CallMeNow"]=="null") and ($item["Facebook"]=="null") and ($item["text"]=="null")) :?>
...
<? for($i = 0; $i < count($this->data["attrs"]["category_details"]); $i++) : ?>
		...
		<? if($item['visible_on_page_id']=="1" || $item['visible_on_page_id']=="2") :?> 
				...
				<? if($item['visible_on_page_id']=="1" and $title==0) :?>
						<h1>Consulter la FAQ par sujet</h1>
						<? $title++;?>
...
<? if(count($this->data['attrs']['more_category_details'])>0):?>
	...
	<div id="parent_more" class="more_categ_display">
		...
		<div id="more_link"><a  href="javascript:void(0);" id="<?= $id;?>">#rn:msg:CUSTOM_MSG_MORE_CATEGORY#</a></div>
...
<!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------                                The below code will display the most popular answer link if the selected category or subcategory contains[For CANADA only] ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------->


<? if($this->data['attrs']['page']=="2" and $this->data['attrs']['interface']=="30") :?>

 
		<? $categ_id=$this->data['categ_id']; ?>
		<? $cat_id_name=$this->data['categ_id'].".".$this->data['cat_nme'];?>
		
		<rn:widget path="custom/ResponsiveDesign/Popular_answer_count" static_filter="c=#rn:php:$cat_id_name#" report_id="143700"/> 

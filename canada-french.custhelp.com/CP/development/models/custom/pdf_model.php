...
class pdf_model extends \RightNow\Models\Incident{
	...
	*/
	...
	{		
		...
		*/
		...
		$y+=15;
		/*$pdf_obj->SetFont('Arial','B',10);
		$pdf_obj->Cell(1,$y,$comment_label,0,0,'L');*/
		...
		if($comments)
		{
			if($comment1)
			{
				$pdf_obj->Cell(1,$y,$comment1,0,0,'L');//MAX LEN 104
			...
			if($comment2)
			{
				...
				$pdf_obj->Cell(1,$y,$comment2,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment3,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment4,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment5,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment6,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment7,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment8,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment9,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment10,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment11,0,0,'L');
			...
			{$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment12,0,0,'L');
			...
			if($comment13)
			{
				$y+=7;
				$pdf_obj->Cell(-1,$y,'',0,0,'L');
				$pdf_obj->Cell(1,$y,$comment13,0,0,'L');
				$line_count++;
			}
		}
		else
		{
			$pdf_obj->SetFont('Arial','',10);
			$pdf_obj->Cell(1,$y,"No Comments",0,0,'L');
		}*/
	 ...
	 $y-=30;
		...
		*/
		...
		{
		$pdf_obj->SetFont('Arial','',10);
		$pdf_obj->Cell(-2,$y,$performance,0,0,'L');
		}
		else
		{
		$pdf_obj->SetFont('Arial','',10);
		$pdf_obj->Cell(-2,$y,'No Value',0,0,'L');
		}*/
		...
		{
		$pdf_obj->SetFont('Arial','',10);
		$pdf_obj->Cell(-2,$y,$shakeology_boost,0,0,'L');
		}
		else
		{
		$pdf_obj->SetFont('Arial','',10);
		...
		{
		$pdf_obj->SetFont('Arial','',10);
		$pdf_obj->Cell(-1,$y,$refresh,0,0,'L');
		}
		else
		{
		$pdf_obj->SetFont('Arial','',10);
		$pdf_obj->Cell(-1,$y,'No Value',0,0,'L');
		}*/
	...
	*/
	...
	{		
		...
		$ssn_label_two=utf8_decode("Coach OU 4 dernier chiffre de votre Assurance Social*");
...
//		$new_word=utf8_decode($word);//urldecode($word);
		...
		if($formData['Incident_CustomFields_c_ccf_reason_for_cancellation_fr'])
		{
			/*if($flag==1)
			{
				$other_reason=$formData['Incident_CustomFields_c_ccf_reason_for_cancellation_fr']."* : ".$formData['Incident_CustomFields_c_ccf_other_reason'];
				$pdf_obj->Cell(-1,190,utf8_decode($other_reason),0,0,'L');
			}
			else
			{*/
		...
		$pdf_obj->Cell(-1,$y,'',0,0,'L');
		$y+=5;
		$pdf_obj->Cell(1,$y,$comment_label2,0,0,'L');
		$pdf_obj->SetFont('Arial','',7);*/
		$y+=17;
		$inc=7;
		$line_count=0;
	...
	*/
	...
	{		
		...
		$ssn_label_one=utf8_decode("Los últimos 4 dígitos de la tarjeta de crédito que es utilizada para el pago de la tarifa de ");
		$ssn_label_two=utf8_decode("Servicio de Negocios O últimos 4 dígitos de SSN, EIN o SIN*");

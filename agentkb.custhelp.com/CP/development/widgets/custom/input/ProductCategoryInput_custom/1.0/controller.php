...
class ProductCategoryInput_custom extends \RightNow\Widgets\ProductCategoryInput {
    ...
    function getData()
    {
        $dataType = $this->data['attrs']['data_type'] = (Text::stringContains(strtolower($this->data['attrs']['data_type']), 'prod'))
            ? self::PRODUCT
            : self::CATEGORY;
//echo $this->data['attrs']['data_type'];
		...
		$defaultreason = $this->getDefaultreason(); //Added for setting injuary cat dynamically
		...
		$defaultHierreason=$this->CI->model('Prodcat')->getFormattedTree($dataType, $defaultreason, false, null, $maxLevel)->result;//Added for setting injuray dynamically
		array_unshift($defaultHierreason[0], array('id' => 0, 'label' => $this->data['attrs']['label_all_values']));
		$this->data['js']['myxdata'] = $defaultHierreason;
		}
        ...
        //$this->data['js']['hierData'] = $defaultHierMap;
		        //$this->data['js']['hierData'] = $defaultHierMap;
		/*Added By Varun to exclude products ids
		provided via widget attribute
		Code block starts here*/
		$defaultHier = array();
		$defaultHiers = array();
		$pids = explode(",",$this->data['attrs']['p_id']);
		if ($this->data['attrs']['selection'] == "exclude"){
		foreach($defaultHierMap as $k=>$v){
			foreach($defaultHierMap[$k] as $values){
             if (!in_array($values[id],$pids)){
				array_push($defaultHier,$values);	  
					  }
				}
			$defaultHiers[$k]=$defaultHier;
			$defaultHier = array();
			}
		}
		else{
		...
		//($defaultHiers, $defaultHier );
		//array_unshift($defaultHier, array('id' => 0, 'label' => $this->data['attrs']['label_all_values']));
		array_unshift($defaultHiers[0], array('id' => 0, 'label' => $this->data['attrs']['label_all_values']));
		//Code block ends here

		$this->data['js']['hierData'] = $defaultHiers;
    }
    ...
    */
    protected function getDefaultChain() {
		...
		protected function getDefaultreason() {
		  $defaultValue = 1345;
		  $dataType='Category';
		   if($defaultValue) {
            $defaultChain = explode(',', $defaultValue);

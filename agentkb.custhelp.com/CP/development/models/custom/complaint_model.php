...
use RightNow\Connect\v1_2\QA_Complain as RNCPHP_COM;
use RightNow\Connect\v1_2\CO as RNCPHP_CO;

require_once( get_cfg_var( 'doc_root' ).'/include/ConnectPHP/Connect_init.phph' );
initConnectAPI();

class complaint_model extends \RightNow\Models\Base
{
    ...
    {	
	    try
		{
			$Organization = new RNCPHP_COM\Complaint();
	...
	function getBlankCF($Orgs,$field)
    {
	    try
		{
		$Organization = RNCPHP_COM\Complaint::getMetadata();
 ...
 function formatOrgs($Organization, $field)
    {		
		try
		{
		 $Organization_meta = RNCPHP_COM\Complaint::getMetadata();
	...
	try{
	        ...
	        $complaint = new RNCPHP\QA_Complain\Complaint();
			$plastic_texture = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_plastic_texture");
			...
			$plastic_color = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_plastic_color");
			...
			$plastic_size = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_plastic_size");
			...
			$unknown_filth_color = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_filth_color");
			...
			$unknown_filth_appearance = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_filth_appearance");
			...
			$insect_box_or_prod = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_pest_insect_bug");
			...
			foreach($insect_box_or_prod as $items) 
			{
				...
				//die("------------------------------");
			$pest_size = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_pest_size");
			...
			$product_damage = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_product_damage");
			...
			//pcf change 17-05-2022 : Menu items
			
			$injury_type = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_injury_type");
			foreach($injury_type as $items) 
			
			{
				if($items->ID == $CBO_Array['cf_injury_type'])
				{
				$complaint->injury_type = $items;
				}
			}
			$injury_part = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_injury_occured");
			foreach($injury_part as $items) 
			{
				if($items->ID == $CBO_Array['cf_injury_part'])
				{
				$complaint->injury_part = $items;
				}
			}
			$bike_parts = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_bike_parts");
			foreach($bike_parts as $items) 
			{
				if($items->ID == $CBO_Array['cf_bike_parts'])
				{
				$complaint->bike_parts = $items;
				}
			}
				$requested_qca = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_requested_qca");
			foreach($requested_qca as $items) 
			{
				if($items->ID == $CBO_Array['cf_requested_qca'])
				{
				$complaint->requested_qca = $items;
				}
			}
			$medical_attention = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_medi_attention");
			foreach($medical_attention as $items) 
			{
				if($items->ID == $CBO_Array['cf_medical_attention'])
				{
				$complaint->medical_attention = $items;
				}
			} 
			$sought_medical_attention = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\CO\\pcf_sought_medi_attn");
			foreach($sought_medical_attention as $items) 
			{
				if($items->ID == $CBO_Array['cf_sought_medical_attention'])
				{
				$complaint->sought_medical_attention = $items;
				}
			} 
			//End pcf menu change
			...
			$complaint->date_bike_delivery = ($CBO_Array['cf_date_bike_delivery']) ? mktime(0,0,0,substr($CBO_Array['cf_date_bike_delivery'],0,2),substr($CBO_Array['cf_date_bike_delivery'],3,2),substr($CBO_Array['cf_date_bike_delivery'],6,4)) : null;
		   ...
		   /*End*/
			...
			// PCF change : text fields
			$complaint->osc_incident_number = $CBO_Array['cf_osc_incident_number'];
			$complaint->injury_type_other = $CBO_Array['cf_injury_type_other'];
			$complaint->body_parts_other = $CBO_Array['cf_body_parts_other'];
			$complaint->bike_parts_other = $CBO_Array['cf_bike_parts_other'];
			$complaint->injured_person_name = $CBO_Array['cf_injured_person_name'];
			$complaint->injury_description = $CBO_Array['cf_injury_description'];
			$complaint->doctors_details = $CBO_Array['cf_doctors_details'];
			
			//End Here: text fields
...
//$message="\n Inside complaint_model Reference no: #".$ref_no." Custom object ID#".$complaint->ID;
...
return $complaint->ID;	
	...
	///$message=$err->getMessage()."\n Reference no: #".$results->result['transaction']['incident']['value'];
	//error_log($message,3, '/cgi-bin/agentkb.cfg/scripts/cp/customer/development/views/pages/error_log/agentkb_error_log_'.date('d-m-Y-H-i-s').'_log.txt');
	//$message= $err->getMessage();
	//error_log($message,3, 'https://agentkb.custhelp.com/cgi-bin/agentkb.cfg/scripts/cp/customer/development/views/pages/error_log/inventory_error_log_'.date('d-m-Y').'_log.txt');
	/*$folder="/tmp/";
  $fname="DS3_".$date.".txt";

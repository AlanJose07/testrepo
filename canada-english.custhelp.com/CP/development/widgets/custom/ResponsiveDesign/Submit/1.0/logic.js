...
Custom.Widgets.ResponsiveDesign.Submit = RightNow.Widgets.FormSubmit.extend({ 
    ...
    overrides: {
        ...
        constructor: function(data,instanceID,Y) {
			...
			var index_top=exploded_url.indexOf("TLP"); 
            this.top_parent= exploded_url[index_top+1].split('.'); 
			console.log(this.top_parent[0]);
   ...
   _onFormValidated: function() {
    ...
    _confirmOnNavigate : function(result){
		 ...
		 if(checked_channel=="1530")
		 {
				...
				div.innerHTML += "<div class='email-conf-title'><h2>Your Request Has Been Submitted</h2></div><div class='email-conf-cont'><div>Thanks for reaching out to Beachbody. We make every effort to reply to emails within 4 hours. If your request comes in after 5pm PT we will reply the next morning.</div><div> You may use this reference number for follow up:&nbsp;<b>"+incident_ref_no+"</b></div></div>";

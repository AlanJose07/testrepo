...
Custom.Widgets.ResponsiveDesign.Language = RightNow.Widgets.SelectionInput.extend({ 
	...
	show_regular_chathours_details: function(type, args)
	{
		...
		{
			//This is for CANADA. There is a config setting CUSTOM_CFG_PREF_RES_LANG_CANADA to show or hide the preferred response language. If it is 'YES'->'show' and if it is 'NO'->'hide'. This Language field will be hidden only in chat and not in email
			if(this.data.js.prefResLangDisplay=="YES")
				{
				...
				{  
				   var hide_chat_hour_details_based_on_prefResLangDisplay=["language","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available"];
				   this._displayNone(hide_chat_hour_details_based_on_prefResLangDisplay);
			...
			if(language_id==1182)//english    
			{
				...
				var hide_chat_hour_details=["spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available"];
				this._displayBlock(regular_chat_hour);
				this._displayNone(hide_chat_hour_details);
			}
			else if(language_id==1183 && this.data.js.prefResLangDisplay=="YES")//french  
			{
				var display_chat_hours =["regular-chat-hours","spanish-chat-hours"];
				this._displayBlock(display_chat_hours);
				
				
				    if(this.data.js.check_lang_display=="1")
					{
						if(this.data.js.check_lang_display_spanish=="1")
						{
							var button_details=["submit-button"];
							this._displayBlock(button_details);
						}
						else
						{
							var button_details=["submit-button"];
							this._displayNone(button_details);
							var message=["spanish-message-outbound-time"];
					...
					else
					{
							var button_details=["submit-button"];
							this._displayNone(button_details);
							var message=["spanish-message-chat-not-available"];
							this._displayBlock(message); 
					}
			}
			else if(language_id==1184)//spanish     
			{
			...
			{
				var regular_chat_hour_hide =["regular-chat-hours","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available","submit-button"];
		...
		else if(channel_id=="1530")//email us
		{
			...
			//This is for CANADA. There is a config setting CUSTOM_CFG_PREF_RES_LANG_CANADA to show or hide the preferred response language. If it is 'YES'->'show' and if it is 'NO'->'hide'. This Language field will be hidden only in chat and not in email
				...
				var regular_chat_hour =["regular-chat-hours","spanish-chat-hours","spanish-message-outbound-time","spanish-message-chat-not-available"];

...
Custom.Widgets.ResponsiveDesign.ChatPostMessageFocus = RightNow.Widgets.ChatPostMessage.extend({ 
    ...
    overrides: {
		...
		{  
			document.title = "Live Assistance";
				...
				if(this.data.attrs.focus_on_incoming_messages)
				{
					...
					else
					{
						...
						this.change_title = setInterval(function() {
							...
							{
								if (counter++ % 2) {
									document.title = "Beachbody says..."; // Call Function 1
								} else {
									document.title = "Live Assistance"; // Call Function 2
								}
							}
						}, 500);
						try
	...
	checkFocus: function() {
		this.abc =setInterval(function(){
			try
			{
				...
				if(title != "Live Assistance" && title != "Beachbody says...")
				{
					document.title = "Live Assistance";
				}
			}

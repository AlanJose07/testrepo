...
function CustomerLookup($searchtype,$country,$phone,$pinemail,$pinorder,$pinfirstname,$pinlastname,$stype,$member_typ,$contact_type,$coach_id,$guid)
{
    ...
    { 
        ...
        $curl = curl_init();
			...
			CURLOPT_URL => "https://ebs.stage.gateway.beachbody.com/customerSearch",
...
//Incident created time			
		  ...
		  if(is_numeric($category_id))
			{
					...
					if($catParentIdFirst!="")
					{
											...
											$enable = "SELECT bbb.enable 
												   FROM BB.categ_contac_hom_ca bbb 
	...
	function remove_bookmarked_url($category_id,$page)
	{
	  try
	  {
		...
		if($page == "answers"){
	    $baseQuery="Select answer_report_id from BB.categ_contac_hom_ca where BB.categ_contac_hom_ca.category.ID =".$category_id." and BB.categ_contac_hom_ca.visible_on_page.ID != 3 and BB.categ_contac_hom_ca.visible_on_page.ID != 4";
		}
		
	  	if($page == "contactus_support"){
	    $baseQuery="Select answer_report_id from BB.categ_contac_hom_ca where BB.categ_contac_hom_ca.category.ID =".$category_id." and BB.categ_contac_hom_ca.visible_on_page.ID != 1 and BB.categ_contac_hom_ca.visible_on_page.ID != 2";
		}
		if(is_numeric($category_id))
		{
			$pro =  RNCPHP\ROQL::query($baseQuery)->next();
...
function setvoiceskill($catid,$membertypenew)
{
  ...
  try
  {
		if (\RightNow\Utils\Framework::isLoggedIn())  
		{
			...
			$query="select BB.categ_contac_hom_ca from BB.categ_contac_hom_ca where BB.categ_contac_hom_ca.category.ID=".$catid." and BB.categ_contac_hom_ca.interface.ID=2 and BB.categ_contac_hom_ca.visible_on_page.ID in(3,4)"; 
		$res = RNCPHP\ROQL::queryObject($query)->next();
		if($res->count()>0)
		{
		   while($channel = $res->next()) 

...
$(window).load(function() {
 ...
 //alert("window is loaded");
		var zurl = "http://dataservices.fig.prods.ca.intelsat.com/v1/flights?status=scheduled&order-by=scheduled-time&order-asc=true&detail=true&registration-number=N504AE";
		if(zurl)
		{		 
		$.ajax({
		url:zurl,
...
$.ajax({
	url:"https://care.inflightinternet.com/cc/AjaxCustom/gettailsnew", 
	type: "POST",
	dataType:'text', 
...
function fetchdatafromapi(regnumbersingle=null,states,lpcnt=0)
{
	...
	{	
		...
		{
		var xurl = "http://dataservices.fig.prods.ca.intelsat.com/v1/flights?status="+states[lpcnt]+"&order-by=scheduled-time&order-asc=true&detail=true&registration-number="+regnumbersingle;
		if(xurl)
		{		 
		$.ajax({
		url:xurl,
		...
		success: function(data){
			if(data!="[]")
			{
				...
				for(var k=0;k<1;k++)
				{
					...
					if(parsedDatasch[k]['departure'].airport.time_zone_offset!=undefined)
					{
				  ...
				  if(parsedData[i]['arrival'].time.scheduled!=undefined)
					{
						...
						{
							if(parsedData[i]['arrival'].airport.time_zone_offset!=undefined)
							{			
					...
					if(parsedData[i]['departure'].time.scheduled!=undefined)
					{
						...
						{
							if(parsedData[i]['departure'].airport.time_zone_offset!=undefined)
							{
							...
							if(parsedData[j]['departure'].airport!=undefined)
							{
								...
								if(parsedData[j]['departure'].airport.iata==arvl_st && parsedData[j]['aircraft'].registration_number==tail)
								{
									...
									if(parsedData[j]['departure'].time.scheduled!=undefined)
									{
										...
										if(parsedData[j]['departure'].airport.time_zone_offset!=undefined)
										{
...
// $.ajax({
	...
	// {				
	 ...
	 // {  //alert(temp[l]);
		...
		// {
		// $.ajax({
		// url:"http://dataservices.fig.prods.ca.intelsat.com/v1/flights?status=scheduled&order-by=scheduled-time&order-asc=true&detail=true&registration-number="+temp[l], 
		// type: "GET",
		// dataType: 'text',
		// success: function(data){
		...
		// }
			...
			// {  
				// (function(l){        
					...
					// xhr[l].open("GET", "http://dataservices.fig.prods.ca.intelsat.com/v1/flights?status=scheduled&page-size=150&order-by=scheduled-time&order-asc=true&detail=true&registration-number="+temp[l]);

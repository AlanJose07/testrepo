...
$.ajax({
	...
	if(regnumber.length > 0)
	{
		...
		//console.log(100/regnumber.length);
...
var tail;	
	...
	myTable+="<td style='width: 50px;border: 1px solid black;  text-align: center;background: #545454;padding: 5px;color:#fff;font-size:14px;'>S/N ON</td>";
	myTable+="<td style='width: 50px;border: 1px solid black;  text-align: center;background: #545454;padding: 5px;color:#fff;font-size:14px;'>Flight States</td></tr>";
				  ...
				  if(parsedData[i]['arrival'].time.scheduled!=undefined)
					...
					if(cmptime>=date_selected && cmptime<=cmpdate)
					{
						...
						while(j<length)
						{
							...
							{
								//console.log(parsedData[j]['departure'].airport.iata+"--"+arvl_st);

...
<rn:container report_id="101195" per_page="20">
    <div id="page" align="left" style="font-size:18px; margin-left:15px;"></div>
    <?php 
    if(getUrlParm('tail'))
    {
...
var tod=new Date();
  ...
  var m1=chk1.getMonth();
  var d1=chk1.getDate();
  var y1=chk1.getFullYear();
  var cDate=m1+"/"+d1+"/"+y1;
  var m2=chk2.getMonth();
  var d2=chk2.getDate();
  ...
  {
	  count++;
	  var da1=parsedData[i]['departure'].time.scheduled;
	  var da2=parsedData[i]['arrival'].time.scheduled;
	  var dta1 = new Date(da1);
	  var dta2 = new Date(da2);
	  ...
	  var dife= ho + ':' + mi;
		...
		if(parsedData[i]['flight'].flight_state=="COMPLETED")
		{
			if(parsedData[i]['departure'].time.actual!=undefined)
			{
				var dep_datetime=new Date(parsedData[i]['departure'].time.actual);
				var dep_loc=parsedData[i]['departure'].airport.time_zone_offset;
			...
			{
				var arr_datetime=new Date(parsedData[i]['arrival'].time.actual);
				var arr_loc=parsedData[i]['arrival'].airport.time_zone_offset;
		...
		myTable+="<td style='width: 100px; border: 1px solid black;text-align: center;'>" + dep_date[0] + "</td>";
		...
		else if(parsedData[i]['flight'].flight_state=="IN_AIR")
		{
			if(parsedData[i]['departure'].time.actual!=undefined)
				{
					var dep_datetime=new Date(parsedData[i]['departure'].time.actual);
			...
			var arr_datm="-";
		myTable+="<td style='width: 100px; border: 1px solid black;background-color:#C2FFC1;text-align: center;'>" +dep_date[0] + "</td>";
		myTable+="<td style='width: 100px; border: 1px solid black;background-color:#C2FFC1;text-align: center;'>" +dep_time + "</td>";
		//myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" +arr_date[0] + "</td>";
		myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" +arr_time + "</td>";
		myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" +parsedData[i]['departure'].airport.iata + "</td>";
		...
		if(parsedData[i]['flight'].flight_state=="IN_AIR")
	   {
			myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" +parsedData[i]['arrival'].airport.iata+ "</td>";
			 myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1;text-align: center;'>" + parsedData[i]['flight'].flight_identifier + "</td>";
			 myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" +dife + "</td>";
			
			myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1;text-align: center;'>" +parsedData[i]['arrival'].gate + "</td>";
	 ...
	 if(parsedData[i]['flight'].flight_state=="COMPLETED")
		...
		{
			if(count<=1)
				myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'> </td>";
			else
			{
			...
			if(dt2!='undefined')
			{
			  ...
			  m = m < 10 ? '0' + m : m;
			  var dif= h + ':' + m;
			  myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" +dif + "</td>";
		  	}
		    else
			  myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>-</td>";
			}
	...
	if(parsedData[i]['flight'].flight_state=="IN_AIR")
	myTable+="<td style='width: 100px;border: 1px solid black;background-color:#C2FFC1; text-align: center;'>" + parsedData[i]['flight'].flight_state + "</td></tr>";
	else
...
xhr.send();
            ...
            <div class="w_40">
                <div class="column"><iframe src="https://care.inflightinternet.com/cgi-bin/aircell.cfg/php/custom/op.php?tail=<?php echo $tail?>" width="100%" height="400" ></iframe></div>
                <div class="column"><iframe src="https://care.inflightinternet.com/cgi-bin/aircell.cfg/php/custom/snow-closed.php?tail=<?php echo $tail?>" width="100%" height="400" ></iframe></div>
            </div>
        </div>
        <div class="w_100">

...
<html lang="#rn:language_code#" >
    ...
    <?php 
        if(getUrlParm('tail')) {
			...
			$CI = get_instance();
			if(getUrlParm('aline')){
				$aline = getUrlParm('aline');
			}
			else {
				$aline=$CI->model('custom/language_model')->getaline($tail);
				$aline=$aline[0]; //dynamic //u_operator
			}
			if(getUrlParm('tech')){
				$tech = getUrlParm('tech');
			}
			else {
				$techdata=$CI->model('custom/language_model')->getTech($tail);
				$tech = $techdata[0]; //dynamic //u_access_technology_code	
			}
			if($tech=="2KU")
				$tech="2Ku";
			if($tech=="KU")
			...
			$tmoip=$tmoip[0]; //not used anymore it seems
	
			$resultsurl = $this->model('custom/fetch_url')->fetchUrl();	//no dependency with tail
			
			$resultMisc = $this->model('custom/fetch_misc_url')->fetchUrl();
			$optionMisc = "";
			if($resultMisc != false){
				foreach($resultMisc as $data){
					$url = $data['url'];
					$url = str_replace('{TailNo}',$tail,$url);	
					$optionMisc .="<option  value='".$data['display']."****".$url."' >".$data['name']."</option>"; 
				}
			}
        ...
        <div id="rn_Container" class="rn_Gogo_Container rn_Gogo_Container-loginU">
            <div class="login-container">
                ...
                <div id="rn_Header" role="banner" class="rn_Gogo_Header">
                    ...
                    <div class="header_logo">
						...
						if(getUrlParm('tail')){
							...
							<input type="button" id="mnt_button" target="_blank" onClick="window.open('http://<?php echo $tail?>.<?php echo $aline?>.abs-ops.com:8084/maintenance/'); window.open('http://<?php echo $tail?>-gate.<?php echo $aline?>.abs-ops.com:8084/maintenance/');" value="Maintenance Page" />&nbsp;
							...
							<select style="width: 100px;" id="miscDetails"  onchange = "miscDetailsShow(this)" width="1%" height="10%">
				...
				<?php 	} ?>
						...
						<rn:widget path="search/amccKeyword" label_text="𝗧𝘆𝗽𝗲 𝗧𝗮𝗶𝗹 𝗻𝘂𝗺𝗯𝗲𝗿 / 𝗡𝗼𝘀𝗲 𝗻𝘂𝗺𝗯𝗲𝗿 / 𝗦𝗲𝗿𝗶𝗮𝗹 𝗻𝘂𝗺𝗯𝗲𝗿" initial_focus="true"/>
									<rn:widget path="custom/search/amccSearch"/>
									
									
						<?php
							//all below given paramters are dynamic & from api
								
							if(getUrlParm('owner'))
							{
								$owner=getUrlParm('owner');
								?>
								<span id="nose" style="float: left;margin-left: 25px;margin-top: 16px;font-size: large;"><?php echo "Owner: <label class='text-select-all'>". $owner; "</label>"?></span>
							...
							<?php } 
							if(getUrlParm('serialNo')){
								$serialNo = getUrlParm('serialNo');

								?>
								<span id="serial_No" style="float: left;margin-left: 25px;margin-top: 16px;font-size: large;"><?php echo "Serial No: <label class='text-select-all'>". $serialNo; "</label>"?></span>
							...
							<?php
							if(getUrlParm('videoCap')){
								?>
								<span id="videoCap" style="float: left;margin-left: 25px;margin-top: 16px;font-size: large;">
								<?php echo "Video Capability: <label class='text-select-all'>". getUrlParm('videoCap'); "</label>"?>
							</span>
								<?php
							}
...
<br>
	...
	<script type="text/javascript">
		...
		function miscDetailsShow()
		{
			console.log("Selected Element");
			const miscUrl = document.getElementById('miscDetails').value;
			if(miscUrl){
				document.getElementById('miscDetails').value=1;
				const url	=	miscUrl.split('****');
				const[display,displayurl]=url;
				if(display){
					if(display === "New Tab"){
						openInNewTab(displayurl);
					}
					if(display === "New Window"){
						window.open(displayurl,"_blank",'location=yes,height=490,width=1200,top=80,left=60,scrollbars=yes,status=yes');
					}
				}
			}			
		}

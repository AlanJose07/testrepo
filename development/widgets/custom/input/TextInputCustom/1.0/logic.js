...
Custom.Widgets.input.TextInputCustom = RightNow.Widgets.TextInput.extend({ 
	...
	{	
		...
		{  
            for(var i = 0, errorString = "", message, id = this.input.get("id"); i < errors.length; i++) 
			{
               ...
               message = errors[i];
			   /*Moncy--Modified the message with custom message*/
			   if(this.data.js.name == "Incident.Subject")
			   {
				message ="Please enter a %s of discussion";
			   }
			  else if(this.data.js.name == "Contact.Emails.PRIMARY.Address")
			   {
				message ="Please enter a valid %s address";
			   }
					...
					{
						var label = this.data.attrs.label_input;
						/*Moncy--Converting  the custom message to lowercase*/
						var new_label = label.toLowerCase();
						if(new_label == "your email")
						{ /*Moncy--Getting only the email to use in custom message*/
						var label_part = new_label.split(" ");
						new_label = label_part[1];
						}
						/*Moncy--Replacing with custom message*/
						message = (message.indexOf("%s") > -1) ? RightNow.Text.sprintf(message, new_label) : new_label + " " + message;
					}
    ...
    */
    ...
    _checkExistingAccount: function() {
        ...
        if (this._value === "" || massagedNewValue === this._seenValue || (this._previousValue && this._value === this._previousValue))
            return;
    ...
    _onAccountExistsResponse: function(response, originalEventObject) {
        if (RightNow.Event.fire("evt_accountExistsResponse", response, originalEventObject)) {
            if(response !== false && this._isFormSubmitting === false) {
                ...
                warnDialog = RightNow.UI.Dialog.actionDialog(RightNow.Interface.getMessage("WARNING_LBL"), this.Y.Node.create("<div>" + response.message + "</div>").addClass("rn_AccountExistsWarnDialog"), {
                    buttons: [ { text: RightNow.Interface.getMessage("OK_LBL"), handler: {fn: handleOK, scope: this}, isDefault: true } ]
                });

...
<div id="rn_<?=$this->instanceID;?>" class="<?= $this->classList ?>">
    ...
    <div class="rn_HierList rn_HierList_<?=$key;?> <? if($index % 2) echo 'rn_FloatLeft'; else echo 'rn_FloatRight';?>">
	 <? if ($this->data['aline']=="CPA"){?>
    <h3><a href="<?=$this->data['attrs']['report_page_url'] . $this->data['appendedParameters'] .\RightNow\Utils\Url::sessionParameter(). "/{$this->data['type']}/" .$value['id']."/ko/1";?>"><?=htmlspecialchars($value['label'], ENT_QUOTES, 'UTF-8');?></a></h3>
	<?php } else { ?>
	<h3><a href="<?=$this->data['attrs']['report_page_url'] . $this->data['appendedParameters'] .\RightNow\Utils\Url::sessionParameter(). "/{$this->data['type']}/" .$value['id'];?>"><?=htmlspecialchars($value['label'], ENT_QUOTES, 'UTF-8');?></a></h3>
	<?php }

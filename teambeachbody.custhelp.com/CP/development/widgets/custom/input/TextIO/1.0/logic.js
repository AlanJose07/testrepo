...
RightNow.namespace('Custom.Widgets.input.TextIO');
Custom.Widgets.input.TextIO = RightNow.Field.extend({
    overrides: {
        constructor: function() {
			...
			this._widgetContainer = this.Y.one(this.baseSelector);
			// save state of required for later use
			this._initiallyRequired = this.data.attrs.required;
			// and always initially set required as false
			this.data.attrs.required = false;
			// set base constraints
			this._baseConstraints = this.data.js.constraints;
			
			// Subscribe to event fired from Email widget.. 
			RightNow.Event.on("evt_contactEmailExistsResponse", this._onEmailCheckResponse, this);
        }
    },
	
	/////////////////////////////////////////////////////////////////////////////////////////////
	// Anuj Feb 22, 2014 - CP3 Migration
	// Custom Functions below this line
	/////////////////////////////////////////////////////////////////////////////////////////////
	
	/**
     * Event handler executed when ajax fired from email widget returns some response
     *
	...
	_onEmailCheckResponse: function(type, args) {
		...
		if(response) {
			...
			if(eventArgs.data.show_first_last_name) {
			...
			else {	// we don't want to show the name to user
				// hide the widget completely
				this.data.attrs.hidden = true;
				this.data.attrs.editable = false;
			}
			// make it not required, value already present
	...
	_setRequired: function(value) {
		...
		// update required attribute only when it was initially required, otherwise will always be false
		if(this._initiallyRequired)
			this.data.attrs.required = value;
	},
	
	/**
     * appends constraints to input's attribute string
     */
    _updateConstraints: function() {
		
		this.data.js.constraints = this._baseConstraints;
		
		if(this.data.attrs.required)
			this.data.js.constraints += " required";
		if(!this.data.attrs.editable)
			this.data.js.constraints += " readonly";
    },
	
	/**
     * Update widget element's reqferences after re-render
     */
    _updateReferences: function() {
		
		// update reference to element as it was redrawn again from ejs, so old reference won't work
		this.input = this.Y.one(this._inputSelector);
		this.label = this.Y.one(this.baseSelector + "_Label");
    },
	
	/**
     * Hide/Show widget elements as per new values
     */
    _updateVisibility: function() {
		
		// Hide/Show widget from the attribute
		if(this.data.attrs.hidden)
			RightNow.UI.hide(this._widgetContainer);
		else
			RightNow.UI.show(this._widgetContainer);
		
		// Hide input if not editable
		if(this.data.attrs.editable)
			RightNow.UI.show(this.input);
		else
			RightNow.UI.hide(this.input);
    },
	
	/**
     * Redraws the widget's view with values returned from ajax request
     */
    renderView: function() {
		
		this._updateConstraints();
		var inputType = this.data.js.displayType.toLowerCase();
		...
		// set Content
		this._widgetContainer.set('innerHTML', content);
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	// Anuj Feb 22, 2014 - CP3 Migration - end
	// Standard TextInput functions below this line
	/////////////////////////////////////////////////////////////////////////////////////////////

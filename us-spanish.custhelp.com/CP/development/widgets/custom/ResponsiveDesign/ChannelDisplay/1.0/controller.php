   ...
   function __construct($attrs) {
    ...
    function getData() {
		...
		if (!empty($cat_id))
		{
		 ...
		 $query="select BB.categ_contac_hom_sp from BB.categ_contac_hom_sp where BB.categ_contac_hom_sp.category.ID=".$cat_id." and BB.categ_contac_hom_sp.interface.ID=29 and BB.categ_contac_hom_sp.visible_on_page.ID in(3,4) and BB.categ_contac_hom_sp.enable=1";  
		}
	...
	if($display ==0){
		$this->data['$recommendchannel_id']= $key;
		  if($key=="1527")//ask an expert
			 $value="Pregunta a un experto";
		  else if($key=="1528")//self service form
			 $value="Forma de autoservicio";
			  ...
			  if(array_key_exists("recommend_0",$channel_details))
					  { 
						...
						{ 
						  ...
						  //spanish verbiage for the channel information

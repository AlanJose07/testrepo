RightNow.namespace('Custom.Widgets.ResponsiveDesign.coach_consent_validate');
Custom.Widgets.ResponsiveDesign.coach_consent_validate = RightNow.Widgets.TextInput.extend({ 
    /**
    ...
    overrides: {
		...
		else
		{
			...
			{
				customValidationFlag = 1;
				errors.push("Se requieren permisos para el usuario autorizado");	
			}
		
			if(this.signaturecheck == "")
			{
				customValidationFlag = 1;
				errors.push("Una firma es requerida");
			}
			if (this.signaturecheck != '') {
						pattern = /^\/s\/[\s\S]*$/ ;
						if(!pattern.test(this.signaturecheck)) {
							customValidationFlag = 1;
							errors.push("La firma no es valida");
						}
					}

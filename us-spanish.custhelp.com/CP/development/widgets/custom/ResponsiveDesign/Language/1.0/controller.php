...
class Language extends \RightNow\Widgets\SelectionInput {
    ...
    function getData() {
	 ...
	 $lang_display_spanish = 0;//0--not available  1-available
		...
		$rmasLang = RNCPHP\ROQL::queryObject("SELECT CO.CustomChatHours FROM CO.CustomChatHours WHERE CO.CustomChatHours.ID = 6")->next();
	   ...
	   if($this->fieldName === 'language_shk')
		{
				...
				if($items)
				{
						...
						foreach ($items as $item)
						{
								...
								{
								    $menuItems[$item->ID] = $item->LookupName ?: $item->Name;
								}else if($item->ID != 1183 and $item->ID != 1182 and $interface==29)//SPANISH interface
								{
								    $menuItems[$item->ID] = $item->LookupName ?: $item->Name;
								}

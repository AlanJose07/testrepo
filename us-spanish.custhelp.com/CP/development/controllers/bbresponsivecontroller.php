...
//logmessage($primarycontact);
logmessage($contactdate);
//logmessage($contQuery);
...
//$coachdate = date('Y-d-m', strtotime($contactdate));
...
logmessage($incidentid);
   ...
   // $datalog = "----------------------------incidentid:".$incidentid."CatId:".$val."first_name:".$first_name."last_name:".$last_name."voiceskillid:".$voiceskillid."coachdate:".$coachdate."phnumber:".$number;
...
//        $trans['incident'] = array("key" => "i_id", "value" => $incidentId);
  ...
  {
  	$CI = get_instance();
   	if($CI->session->getSessionData('service_alert_sessions_spanish'))
	{
		$CI->session->setSessionData(array('service_alert_sessions_spanish' => 2));
		$CI->session->setSessionData(array('LAST_ACTIVITY_SERVICE_ALERT_SPANISH' => time()));
	}
  }

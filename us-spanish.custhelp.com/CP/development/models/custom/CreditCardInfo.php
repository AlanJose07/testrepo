	  ...
	  try
	  {
		  ...
		  if($dataArray["First"] != "" && $dataArray["Last"] != "" ){
			...
			$card_type = RNCPHP\ConnectAPI::getNamedValues("RightNow\\Connect\\v1_2\\Update_Acnt\\credit_card_type");
				...
				foreach($card_type as $items)
				{
					if($items->ID == intval($dataArray["credit_card_type"]))
					{
						...
						$updateAccountObj->credit_card_type =RNCPHP\Update_Acnt\credit_card_type::fetch(intval($dataArray["credit_card_type"]));
					}
				}
			...
			*/
			...
			$msg["msg"] ="<b>Su solicitud de actualización de tarjeta de crédito ha sido enviada para su procesamiento</b><br/><br/>Espere 24 horas para que estos cambios se apliquen a su cuenta<br/><br/>El número de referencia para esta solicitud es:<span>#".$incidentObj->ReferenceNumber."</span>";

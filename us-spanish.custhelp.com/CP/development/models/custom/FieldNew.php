 ...
 */
 use RightNow\Utils\Connect,
  RightNow\Utils\Text,
    ...
    RightNow\Api;
class FieldNew extends \RightNow\Models\Base{
    ...
    public function sendForm($formData, $listOfUpdateIDs = array(), $smartAssistant = false) {
		...
		//$home_phone = $formData['Contact.Phones.HOME.Number']->value;
		
		
		print_r($presentFields[Contact.Phones.HOME.Number]);
        ...
        if ($presentFields['Contact']) {
            ...
            //Incident create email only (and can optionally contain other contact fields)
                ...
                if($existingContact){
					echo "contact existing";
				 //echo "hii";
				 //$con = $this->CI->model('Contact')->get(9940510);
				 
				 $contact = RNCPHP\Contact::fetch( $existingContact );
				 
				 $con_id =$contact->ID; 
				 echo "CONTACT ID".$con_id;
				...
				$homephone = $formData['Contact.Phones.HOME.Number']->value;
				$contact->Phones[0]->Number = $homephone;
				AbuseDetection::check();
				$contact->save();
				echo "before model call";
				
			//$results =	$this->CI->model('custom/Order')->phonesave($con_id,$homephone,$first,$last);
			echo "after model call";
		
			
			//$result = $this->CI->model('custom/IncidentModel')->create($formData, $smartAssistant);
				
				
				
				/*//$con->result->Phones[0]->Number =  $formData['Contact.Phones.HOME.Number']->value;
				
				  $formData['Contact.Name.First']->value = $con->result->Name->First;
				  $formData['Contact.Name.Last']->value = $con->result->Name->Last;
			       //$formData['Contact.Address.Country']->value = $con->result->Address->Country->LookupName;
			      $result = $this->CI->model('Contact')->update($existingContact, $formData);
//				  
				   $action = 'Updated';
//				   $formData['Incident.PrimaryContact'] = $result->result->ID;

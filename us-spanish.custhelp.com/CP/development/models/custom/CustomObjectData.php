...
class CustomObjectData extends \RightNow\Models\Base {
	...
	function fetchSpecialFeatureData() {
		...
		$rows = RNCPHP\ROQL::query( "SELECT * FROM BB.special_feature s WHERE s.enable = 1 and s.interface = 29 order by display_order_spc" )->next();
		$i=0;
		while($row = $rows->next())
		{
	...
	function fetchShadowpopup($url) {
		//initConnectAPI('OSC_NICE', 'Yatebr2H');
		$shadow_url = RNCPHP\ROQL::query( "select shadow_popup, ID from BB.url_mapping where BB.url_mapping.URL ='".$url."' and shadow_popup.interface = 29 and shadow_popup.enable = 1 LIMIT 1" )->next()->next();
  ...
  echo $e->getMessage();   
	...
	$error_log->LogTime = "US Spanish Interface";

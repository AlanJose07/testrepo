    ...
    protected function getMenuItems(){
		...
		if($this->fieldName === 'member_type_new')
		 {
				...
				if($items)
				{
					...
					{
						if(($item->ID != 390)&&($item->ID != 398))
						{
							if($item->ID == 389)
								$menuItems[$item->ID] = 'Client(e)';
							else if($item->ID == 388)
								$menuItems[$item->ID] = 'Coach';
							else if($item->ID == 1725)
		  ...
		  if($this->fieldName === 'Country')
		 {
		  ...
		  if($this->fieldName === 'dsrm_object_data')
		 {
                $field = explode('.', $this->data['inputName']);
                $object = array_shift($field);
                $items = Connect::getNamedValues($object, implode('.', $field));
				...
				if($items)
				{
					...
					{
						if($item->ID == 1522)
							$menuItems[$item->ID] = 'E-mails marketing de Beachbody et de TBB';
						else if($item->ID == 1523)
							$menuItems[$item->ID] = 'Partage de mes informations de contact avec les coachs';
						else if($item->ID == 1526)
							$menuItems[$item->ID] = 'Autre';
						else
							$menuItems[$item->ID] =  $item->Name;
        ...
        if($items){
            foreach ($items as $item) {
				...
				if($item->ID == 1505)
				$menuItems[$item->ID] = 'Droit d’accès / Droit à la portabilité des données / Droit d’être informé(e)';
				else if($item->ID == 1506)
				$menuItems[$item->ID] = 'Droit à l’effacement';
				else if($item->ID == 1507)
				$menuItems[$item->ID] = 'Droit de rectification';
				else if($item->ID == 1521)
				$menuItems[$item->ID] = 'Droit d’opposition / Droit à la limitation du traitement';
				else
                $menuItems[$item->ID] = $item->LookupName ?: $item->Name;
            }
        }

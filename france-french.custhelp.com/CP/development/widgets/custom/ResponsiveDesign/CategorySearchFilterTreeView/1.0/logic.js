RightNow.namespace('Custom.Widgets.ResponsiveDesign.CategorySearchFilterTreeView');
Custom.Widgets.ResponsiveDesign.CategorySearchFilterTreeView = RightNow.Widgets.ProductCategorySearchFilter.extend({ 
    /**
   ...
   /* selectNode: function(node) {
		...
		//eo_category=node.value;
		//RightNow.Event.fire("get_category_id", eo_category);//subscribed in CustomPaginatorGeneric
    ...
    selectNode: function(node) {
		//alert("selectNode");
		var dep = node.depth + 1;
		var val = node.value;
		
		//alert("depth = "+dep+" value = "+val);
		
		var eo_category={value :val, depth: dep};
		RightNow.Event.fire("get_category_id", eo_category);//subscribed in CustomPaginatorGeneric
		try
		{
			document.getElementById('ygtv0').style.display = "none";

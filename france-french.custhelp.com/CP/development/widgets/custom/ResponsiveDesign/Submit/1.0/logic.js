   ...
   _onFormValidated: function() {
    ...
    _confirmOnNavigate : function(result){
		 var url=String(window.location);  
			...
			var emailindex=exploded_url.indexOf("submit_document");
			if(index!=-1)
		...
		/* if(checked_channel=="1530")
		 {
			...
			/*	var incident_ref_no=result['transaction']['incident']['value'];
				...
				div.innerHTML += "<div class='email-conf-title'><h2>Votre document a été soumis</h2></div><div class='email-conf-cont'><div>Nous examinerons votre document et vous enverrons un courrier électronique dans les deux jours ouvrables.</div><div> Vous pouvez utiliser ce numéro de référence pour le suivi:&nbsp;<b>"+incident_ref_no+"</b></div></div>";
		...
		if(checked_channel=="1530")
		 {
				...
				if(document.getElementById('hide-for-email-confirm-popup').style.display=="block")
				{
					...
					var div = document.getElementById('after-submit-email');
				div.innerHTML += "<div class='email-conf-title'><h2>Votre e-mail a été soumis</h2></div><div class='email-conf-cont'><div>Merci d'avoir contacté MYXfitness. Nous faisons de notre mieux pour répondre aux e-mails dans les 4 heures.</div><div> Si votre demande arrive après 17h PT, nous vous répondrons le lendemain matin.</div><div>  Vous pouvez utiliser ce numéro de référence pour le suivi:&nbsp;<b>"+incident_ref_no+"</b></div></div>";
		...
		if(checked_channel=="1530")//email
			{
				...
				{
					this.data.attrs.on_success_url = "/app/submit_document";
